// THE PROOF THAT THE DW_apb_i2c DRIVER KNOWS NO CHIP:
// brio/dw_apb_i2c/i2c.hpp instantiated over a SECOND chip that is not a
// chip at all - two register blocks in RAM, a reset that fills them with
// the block's reset values, an interrupt controller that counts, pads
// over a modelled wire and a ruler that counts microseconds instead of
// waiting (brio/host/sim_dw_apb_i2c.hpp) - with every verb of the
// resource, of the host engine and of the client named, both engine
// slots empty and both filled. Compiled here by the family's own
// compiler and by the host's (test/test_dw_apb_i2c/main.cpp): a file
// that needed a silicon would fail in one of the two.
...
// The IP's own arithmetic and vocabulary, judged where no chip can
// colour them.
...
static_assert(i2c_cycles_ceil(125'000'000, 50) == 7u && i2c_cycles_ceil(1'000'000, 1000) == 1u);
static_assert(i2c_timing_for(125'000'000, I2cSpeed::standard_100k)->hcnt == 486u);
