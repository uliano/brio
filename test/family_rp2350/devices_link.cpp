// The DCS serial link over this family's SpiHost: BOTH ARCHITECTURES
// and both packages must compile it (instantiation only - the function
...
// prescaler and a serial clock rate; the frame size is eight or sixteen
// bits and `cmd_len` and `len` are counted in FRAMES. Every pad named
// below is on the smaller package too.
#include <span>
