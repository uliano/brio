...
on the G0B1 against `USART2_IRQn` elsewhere) and the kernel-clock
multiplexers (USART2SEL absent on the G031). Every fact of that kind is
read off the device header in `brio/stm32g0/device_tables.hpp`, and
this fixture is what proves the probes right on every header.
