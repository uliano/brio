// Platform family smoke TU: the CH32X035 realization of the kernel's
// Platform concept (Ch32x035Platform<TB>), the 64-bit STK timebase it
...
// No idle_until: the probe must be dependent (a requires-expression on a
// fixed type is ill-formed rather than false when the call does not
// exist).
template <class Pl>
constexpr bool has_idle_until = requires { Pl::idle_until(std::optional<uint32_t>{}); };
static_assert(!has_idle_until<P>);

// The interrupt numbers ARE the vector table's word indices (RM table 7-1).
static_assert(static_cast<uint8_t>(Irq::non_maskable) == 2);
static_assert(static_cast<uint8_t>(Irq::exception) == 3);
...
static_assert(static_cast<uint8_t>(Irq::flash) == 18);
static_assert(static_cast<uint8_t>(Irq::exti7_0) == 20);
static_assert(static_cast<uint8_t>(Irq::usart1) == 32);
static_assert(static_cast<uint8_t>(Irq::usart2) == 39);
static_assert(static_cast<uint8_t>(Irq::usart3) == 42);
static_assert(static_cast<uint8_t>(Irq::usart4) == 43);
static_assert(static_cast<uint8_t>(Irq::dma1_channel8) == 44);
static_assert(static_cast<uint8_t>(Irq::tim3) == 54);
...
static_assert(delay_rate(187'500).cycles_per_us == 1);
...
void interrupt_verbs() {
    ...
    (void)Pfic::enabled(Irq::usart2);
    (void)Pfic::pending(Irq::usart2);
