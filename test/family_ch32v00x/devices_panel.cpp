// The panel driver over this family's SpiHost: every part must
...
using PanelHost = SpiHost<1>;   // the instance's own default column
using PanelLink = DcsSerialLink<PanelHost>;

/// The two tenures, filled as this stratum fills a request: the select
/// and the D/C pin, the setup time, the division of HCLK, the mode and
/// the frame - and the read one slower, because a panel's read ceiling
/// is not its write ceiling.
PanelLink::Config panel_link_config() {
    PanelHost::Request write{};
    write.cs = Pin<'C', 1>::ref();
    write.dc = Pin<'C', 0>::ref();
    write.cs_setup_us = 1;
