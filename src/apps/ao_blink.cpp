// ao_blink - the first brio-kernel firmware: two active objects talking.
//
// BlinkerAo toggles the LED on PF2, driven by its own periodic time
// event; SupervisorAo cycles the blink period (500 -> 250 -> 100 ms)
...
// ---- the blinker ------------------------------------------------------------
struct BlinkerAo : brio::Fsm<BlinkerAo, Toggle, SetPeriod> {
...
// ---- the supervisor ---------------------------------------------------------
struct SupervisorAo : brio::Fsm<SupervisorAo, Cycle> {
    static inline brio::EventQueue<Event, 2, P> queue;
    static inline brio::TimeEvent<P, SupervisorAo, Cycle> cadence{Cycle{}};
    ...
    static Status running(const Event& e) {
        return std::visit(overloaded{
            ...
            [](Cycle) {
                ...
                brio::post<BlinkerAo>(SetPeriod{periods[index]});
                return handled();
            },
            [](auto) { return unhandled(); },
        }, e);
...
int main() {
    ...
    brio::Kernel<P, BlinkerAo, SupervisorAo>::run();  // never returns
}
