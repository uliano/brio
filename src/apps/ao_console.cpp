// ao_console - the console migrated to the brio kernel: three active
// objects replacing the old polling main loop.
//
//   SerialAo   turns RX bytes into LineReceived events (ping-pong line
//              buffers, backpressure on the ring - see util/serial_ao.hpp)
//   ConsoleAo  parses and routes each line, replies via blocking print
//              (bounded, ~us at 460800), drives the blinker BY POSTING
//   BlinkerAo  owns the LED: heartbeat time event at 1 Hz, manual
//              LED ON|OFF|TOG commands arrive as posted SetLed events
//
// Kernel pack order is a CONTRACT here: ConsoleAo (line consumer) must
// precede SerialAo (line producer) so the ping-pong buffers are always
// free when SerialAo runs - see the scheduling contract in serial_ao.hpp.
//
// Connect with `pio device monitor -e ao_console` (460800 8N1) and type:
...
struct BlinkerAo : brio::Fsm<BlinkerAo, Toggle, SetLed> {
    static inline brio::EventQueue<Event, 2, P> queue;
    static inline brio::TimeEvent<P, BlinkerAo, Toggle> heartbeat{Toggle{}};
...
struct ConsoleAo : brio::Fsm<ConsoleAo, brio::LineReceived> {
    static inline brio::EventQueue<Event, 2, P> queue;
...
private:
    ...
    static void cmd_led(const Cmd& cmd, Serial s) {
        ...
        brio::post<BlinkerAo>(SetLed{mode});   // addressed command, AO to AO
...
using SerialLines = brio::SerialAo<Serial, P, ConsoleAo, 80>;

void ConsoleAo::cmd_err(const Cmd&, Serial s) {
    brio::print(s,
...
int main() {
    ...
    // Pack order = priority AND correctness: the line CONSUMER (ConsoleAo)
    // must precede the PRODUCER (SerialLines) - scheduling contract.
    brio::Kernel<P, ConsoleAo, SerialLines, BlinkerAo>::run();
}
