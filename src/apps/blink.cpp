...
// set up by init_clock_24mhz(), which falls back to the internal OSCHF if
// the crystal fails to start (CLK_PER is 24 MHz either way).
...
namespace {
using Led = Pin<'F', 2>;  // PF2
}  // namespace

int main() {
    init_clock_24mhz();  // PA0/PA1 crystal -> CLK_PER = 24 MHz (OSCHF fallback)
