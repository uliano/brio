// test_rp2350_i2c - the reference bench suite for this chip's I2C
// (rp2350/i2c.hpp over datasheet 12.2, the Synopsys DW_apb_i2c v2.03a):
// the host on a wire to the chip's OTHER instance as the client, every
// tenure shape, the vocabulary produced on the wire, the three speeds
// measured, the DMA engines, the roles inverted, this chip's pads and
// their isolation latch, and util/i2c_bus.hpp's arbiter with not one
// line changed. ONE SOURCE, BOTH ARCHITECTURES: nothing in chapter 12.2
// is a processor's, so every verdict below is expected to read the same
// on the Cortex-M33 half and on the Hazard3 half, and the numbers that
// may differ (an interrupt count, a service latency) are PRINTED and not
// judged tightly.
//
...
// BOARD, two wires between the two instances' pads and a real pull-up on
// each:
//
//   GP12 (I2C0 SDA)  <->  GP14 (I2C1 SDA)     4.7 kOhm to 3V3
//   GP13 (I2C0 SCL)  <->  GP15 (I2C1 SCL)     4.7 kOhm to 3V3
//
// The client answers 0x42 and is served from ITS OWN interrupt: what the
// host writes is taken there, what the host reads is given there (up to
// the FIFO's depth ahead), and every tenure edge the block reports is
// counted. Both instances' interrupts run on one core, so the client's
// service delays the host's and the other way round; the wire lets both.
//
// HOW THE PULL-UPS ARE PROVEN HERE, AND WHY IT IS NOT THE RP2040'S TEST.
// Erratum RP2350-E9 is live on this stepping: a pad whose input buffer is
// enabled and which nothing drives leaks enough current to sit HIGH
// against its own pull-down. So "both lines read high with every
// peripheral released" no longer proves a pull-up is fitted - a bare pad
// says the same. The erratum's other half is the instrument: A PAD DRIVEN
// LOW AND THEN RELEASED STAYS LOW. So each line is driven low, released
// with the pad's own PULL-DOWN on, and read: a 4.7 kOhm to 3V3 beats that
// pull-down and the line comes back HIGH; with no pull-up fitted the pad
// stays where it was put. That is a sound test in both directions on this
// silicon, and the letter prints both readings.
//
// THE RULER IS SYSTEM TIMER 0 (rp2350/timer.hpp). A bus rate is measured
// with no pad to spare: a tenure of N bytes is 9 x (N + 1) SCL periods,
// and the stopwatch runs around the whole tenure. THE UNSTICK HAS A RULER
// OF ITS OWN - the platform timer, which rp2350/delay.hpp counts because
// it is the one counter both architectures have - and main starts it, on
// both halves, because on the Cortex-M33 half nothing else does. Letter h
// measures the pulse train and so states which it got.
//
// What is exercised, letter by letter:
//   a  the block, WIRELESS: 9.4 as the driver states it over the whole
//      bank, the timing arithmetic at this clk_sys (table 1053's floors
//      reproduced), the reset state, the FIFO depths the block reports,
//      the refusals, the enable and the disable through IC_ENABLE_STATUS
//   b  THE WIRE: the pull-ups proven the way E9 forces; then THE SCAN,
//      every address 0x08..0x77 probed, exactly one answer at 0x42,
//      nobody-home measured as i2c_nack_addr and timed
//   c  the tenure shapes: write, read, write-then-read with the repeated
//      START counted from the far end, the probe as this silicon serves
//      it (one byte read), a 200-byte write in one tenure through the
//      pump
...
//      the client's clock stretch priced; the flush of stale bytes at a
//      read
//   e  THE THREE SPEEDS byte-exact both ways and MEASURED on the ruler
//   f  THE DMA ENGINES, kept apart from every bus verdict above so that
//      a fault in this target's new DMA cannot mask one: a 64-byte read,
//      a write-then-read, the short reads that stay on the pump
//   g  THE KERNEL: I2cBus (= BusMaster) over I2cHost, the NACK in its
//      place, the rejection, both votes, and THE TIMED BUS: a client
//      holding the clock answered by the arbiter's per-bus timeout, the
//      recover()ed engine carrying the next tenure
//   h  THE STUCK BUS: unstick() reading a healthy wire, then SDA held low
//      by a GPIO, then healthy again - with its half-bit measured
//   i  THE ROLES INVERT on the same two wires: I2C1 hosts, I2C0 listens
//   j  THE PADS, which is where this chip differs from its predecessor:
//      the isolation latch dropped by the driver's own hand-over, the
//      whole pad register read back against 12.2.1.3, and both pads left
//      isolated again by release()
//   x  (outside z) a traced read, for diagnostics
//
...
// I2C0 hosts on GP13/GP12; I2C1 listens on GP15/GP14. And the other way
// round.
...
// Everything the ISR reads and the letters write is VOLATILE: a store to
// a plain global with no opaque call after it is sunk past an inlined
// tenure, and the ISR reads the old value.
...
// ---- the wire, proven under erratum RP2350-E9 --------------------------------
//
// A level a pad happens to be sitting at is HISTORY on this stepping, so
// each line is PUT somewhere first: driven low, then released with the
// pad's own pull-down holding it there. Only an external pull-up can move
// it back up, and the 4.7 kOhm of this link beats a pad pull-down by more
// than a decade.
bool line_pulled_up(uint8_t pin) {
    switch (pin) {
        case 12: (void)Pin<12>::output(false); break;
        case 13: (void)Pin<13>::output(false); break;
        case 14: (void)Pin<14>::output(false); break;
        default: (void)Pin<15>::output(false); break;
    }
    spin_us(50);
    switch (pin) {
        case 12: (void)Pin<12>::input({.pull = PinPull::down}); break;
        case 13: (void)Pin<13>::input({.pull = PinPull::down}); break;
        case 14: (void)Pin<14>::input({.pull = PinPull::down}); break;
        default: (void)Pin<15>::input({.pull = PinPull::down}); break;
    }
    spin_us(200);
    switch (pin) {
        case 12: return Pin<12>::read();
        case 13: return Pin<13>::read();
        case 14: return Pin<14>::read();
        default: return Pin<15>::read();
    }
}

void release_wire_pads() {
    (void)Pin<12>::release();
    (void)Pin<13>::release();
    (void)Pin<14>::release();
    (void)Pin<15>::release();
...
bool wire_present() {
    const bool ok = line_pulled_up(12) && line_pulled_up(13) && line_pulled_up(14) && line_pulled_up(15);
    release_wire_pads();
    return ok;
}
...
bool wire_or_decline() {
    ...
    bench.verdict("DECLINED: the self-link's pull-ups are not fitted (GP12..GP15 do not come back high "
                  "after being driven low and released onto their own pull-down) - wire I2C0 x I2C1 with "
                  "two 4.7 kOhm to 3V3",
                  false);
    return false;
}
...
// =============================================================================
void ta_block() {
    bench.verdict("9.4 as the driver states it over the WHOLE bank: I2C0 SDA on 0/4/../44 and SCL on "
                  "1/5/../45, I2C1 SDA on 2/6/../46 and SCL on 3/7/../47, one function column and no "
                  "second one",
                  i2c_sda_pin(0, 0) && i2c_sda_pin(0, 44) && i2c_scl_pin(0, 1) && i2c_scl_pin(0, 45) &&
                      i2c_sda_pin(1, 2) && i2c_sda_pin(1, 46) && i2c_scl_pin(1, 3) && i2c_scl_pin(1, 47) &&
                      !i2c_sda_pin(0, 2) && !i2c_scl_pin(1, 13) && !i2c_sda_pin(1, 48) &&
                      !i2c_pins_valid(0, {.scl = 12, .sda = 13}) &&
                      Rp2350DwApbI2c::pad_function == PinFunction::i2c);
    ...
    bench.verdict("the timing arithmetic: the three speeds solved at this clk_sys produce their asked rate "
                  "on an ideal wire (the SPKLEN + 8 cycles the block adds subtracted from the counts)",
    ...
    bench.verdict("table 1053's floors reproduced: fast mode at 12 MHz is (LCNT 15, HCNT 6), standard at "
                  "2.7 MHz is (12, 6), fast-mode-plus needs 32 MHz, a hertz below each is refused",
    ...
    print(serial, "  I2C0 after reset: IC_CON=", hex(con), " SPKLEN=", S::timing(I2cSpeed::fast_400k).spklen,
          " SDA_HOLD=", S::timing(I2cSpeed::fast_400k).sda_tx_hold, " enabled=", S::enabled(),
          " status=", hex(S::flags()), " IC_COMP_PARAM_1=", hex(S::regs().IC_COMP_PARAM_1),
          " line=", static_cast<uint32_t>(S::irq()), crlf);
    bench.verdict("the block's reset state: a host with its client half disabled, fast speed code, restarts "
                  "enabled (IC_CON 0x65), the spike filter at 7, the hold at 1, disabled, both FIFOs empty - "
                  "and the component parameter register reading zero, as 12.2 leaves it",
    ...
    bench.verdict("the two interrupt lines are 3.2's, which are NOT the RP2040's 23 and 24: I2C0_IRQ is 36 "
                  "and I2C1_IRQ is 37, the same numbers under both architectures (3.8.4.2)",
                  static_cast<uint32_t>(DwApbI2c<0>::irq()) == 36u &&
                      static_cast<uint32_t>(DwApbI2c<1>::irq()) == 37u);
    ...
    S::clear_all();
    print(serial, "  the transmit FIFO under TX_CMD_BLOCK: full after ", depth, " entries (TXFLR ", counted,
          "), TX_OVER ", over_now ? "raised" : "NOT raised", " on the next; after the disable TXFLR ",
          S::tx_count(), crlf);
    bench.verdict("the transmit FIFO is sixteen deep (12.2.1), measured under TX_CMD_BLOCK: full at sixteen, "
    ...
    const I2cTiming asked = *i2c_timing_for(SysClock::hz, I2cSpeed::fast_plus_1m);
    print(serial, "  a configuration read back: HCNT ", back.hcnt, " LCNT ", back.lcnt, " (asked for ",
          asked.hcnt, " and ", asked.lcnt, "), TAR ", hex(S::target()), crlf);
    bench.verdict("a configuration, a timing and a target are taken with the block disabled and read back "
                  "as they were written",
                  cfg && back.hcnt == asked.hcnt && back.lcnt == asked.lcnt &&
                      back.spklen == asked.spklen && S::target() == 0x42u &&
    ...
    bench.verdict("enabled: IC_ENABLE_STATUS reports it, and the configuration, the timing, the target and "
                  "the own address are refused",
...
// =============================================================================
void tb_scan() {
    // The two readings, printed before the verdict: what a bare pad says
    // on this stepping, and what a pull-up says.
    (void)Pin<12>::input({.pull = PinPull::down});
    spin_us(200);
    const bool idle_12 = Pin<12>::read();
    const bool up_12 = line_pulled_up(12);
    release_wire_pads();
    print(serial, "  GP12 with its own pull-down and nothing done to it reads ", idle_12 ? "HIGH" : "low",
          " (erratum E9 makes that reading history and not a measurement); driven low and released onto the "
          "same pull-down it reads ", up_12 ? "HIGH - a real pull-up is fitted" : "low - no pull-up", crlf);
    if (!wire_or_decline()) {
        return;
    }
    bench.verdict("THE PULL-UPS ARE PROVEN THE WAY E9 FORCES: each of GP12..GP15 driven low, released onto "
                  "the pad's own pull-down, and back HIGH - which only an external pull-up can do on this "
                  "stepping",
                  true);
    (void)client_up();
    host_ready();
    ...
    print(serial, "  scan 0x08..0x77 at 400 kHz: ", found, " answer(s); nobody home at 0x43 is status ",
          nobody_status, " in ", nobody_us, " us; the client saw ", client_read_reqs, " read request(s), ",
          client_stops, " stop(s)", crlf);
...
void tc_shapes() {
    ...
    spin_us(200);
    print(serial, "  read 8: status ", st, ", read requests ", client_read_reqs, ", host NACK seen ",
          client_nacks, ", stops ", client_stops, ", ",
          same(client_answers, rx_buf, 8) ? "byte-exact" : "MISMATCH", crlf);
    bench.verdict("a read of eight bytes: i2c_ok, byte-exact, the client served eight read requests, saw the "
                  ...
                  st == i2c_ok && same(client_answers, rx_buf, 8) && client_read_reqs == 8u &&
                      client_nacks == 1u && client_stops == 1u);
    // A write-then-read.
    client_reset_tally();
    client_limit = 6;
    tx_buf[0] = 0xA0;
    ...
    spin_us(200);
    print(serial, "  write 2 then read 6: status ", st, ", heard ", client_count, ", restarts ",
          client_restarts, ", stops ", client_stops, ", ",
          same(client_answers, rx_buf, 6) ? "byte-exact" : "MISMATCH", crlf);
    bench.verdict("a write-then-read: the two bytes heard, ONE repeated START seen from the far end, six "
                  "bytes back exact, one STOP",
    ...
    const uint32_t took = us_now() - t0;
    spin_us(200);
    print(serial, "  write 200: status ", st, " in ", took, " us, ", i2c0_isr_entries,
          " host interrupts, the client heard ", client_count, ", overruns ", client_overruns, ", ",
          same(tx_buf, client_heard, 200) ? "byte-exact" : "MISMATCH", crlf);
    bench.verdict("a 200-byte write in ONE tenure through the pump (the FIFO refilled at its half): exact, "
                  ...
                  st == i2c_ok && client_count == 200u && same(tx_buf, client_heard, 200) &&
                      client_overruns == 0u && i2c0_isr_entries < 200u && client_stops == 1u);
    Host::release();
    client_down();
}
...
void td_vocabulary() {
    ...
    bench.verdict("a client refusing data (IC_SLV_DATA_NACK_ONLY) answers i2c_nack_data on the first byte, "
                  ...
                  nack_armed && refused == i2c_nack_data && heard_refused == 0u && taken == i2c_ok &&
                      client_count == 4u);
    // The stretch priced.
    client_reset_tally();
    client_limit = 8;
    ...
    client_stretch_us = 0;
    print(serial, "  an 8-byte read: ", plain_us, " us plain, ", stretched_us,
          " us with the client holding SCL 100 us before every byte (read requests ", client_read_reqs,
          ", given ", client_given, ")", crlf);
    bench.verdict("the client's clock stretch (RD_REQ holds SCL until a byte is given) is priced on the "
                  "wire: 100 us before each of eight bytes lengthens the read by 700 us or more",
                  plain == i2c_ok && stretched == i2c_ok && stretched_us >= plain_us + 700u &&
                      stretched_us < plain_us + 2000u);
    ...
    spin_us(200);
    print(serial, "  a two-byte read against four queued: status ", two, " [", hex(r0), " ", hex(r1),
          "], flushes ", flushes_after_two, "; the next read: status ", again, " [", hex(rx_buf[0]), " ",
          hex(rx_buf[1]), "]", crlf);
    bench.verdict("bytes queued beyond what the host takes are flushed at its NACK and the client told "
                  ...
                  two == i2c_ok && r0 == client_answers[0] && r1 == client_answers[1] &&
                      flushes_after_two >= 1u && again == i2c_ok && rx_buf[0] == client_answers[0]);
    Host::release();
    client_down();
}
...
void te_speeds() {
    ...
    for (uint8_t s = 0; s < i2c_speed_count; ++s) {
        ...
        print(serial, "  ", asked / 1000u, " kHz: write ", w_ok ? "exact" : "WRONG", ", read ",
              r_ok ? "exact" : "WRONG", "; 64 bytes in ", took, " us -> SCL ", scl_hz / 1000u, " kHz (",
              Host::scl_hz(sp) / 1000u, " on an ideal wire)", bracket ? "" : "  OUT OF BRACKET", crlf);
        if (w_ok && r_ok) {
            ++exact;
        }
        if (bracket) {
    ...
    bench.verdict("the three speeds, 100 kHz, 400 kHz and 1 MHz: sixteen bytes byte-exact both ways at each",
                  exact == 3u);
    bench.verdict("the SCL rate measured on the ruler is never above the asked rate and within a quarter "
                  "below it (the pull-ups' rise time is the difference)",
                  in_bracket == 3u);
    Host::release();
    client_down();
}
...
// =============================================================================
//
// KEPT APART FROM EVERY BUS VERDICT ABOVE. This target's DMA chapter is
// newer than this one and its own suite is what judges the controller; if
// a channel here misbehaves, the letters before this one still stand.
void tf_dma() {
    if (!wire_or_decline()) {
        return;
    }
    ...
    const uint32_t took = us_now() - t0;
    spin_us(200);
    print(serial, "  64 bytes read on the engines at 400 kHz: status ", st, " in ", took, " us, ",
          i2c0_isr_entries, " host interrupts, read requests ", client_read_reqs, ", ",
          same(client_answers, rx_buf, 64) ? "byte-exact" : "MISMATCH", ", nacks ", client_nacks, ", stops ",
          client_stops, crlf);
    if (st != i2c_ok) {
        uint8_t landed = 0;
        ...
        print(serial, "    landed ", landed, " bytes; tx channel remaining ", dma_tx_remaining, " credits ",
              dma_tx_credits, ", rx channel remaining ", dma_rx_remaining, " credits ", dma_rx_credits,
              "; I2C0 raw=", hex(i2c0_raw), " status=", hex(i2c0_status), " rxflr=", i2c0_rxflr, " txflr=",
              i2c0_txflr, " abort=", hex(i2c0_abort), crlf);
    }
    bench.verdict("a 64-byte read on the two engines: the commands poured from a fixed cell, the bytes "
                  ...
                  st == i2c_ok && same(client_answers, rx_buf, 64) && i2c0_isr_entries <= 2u &&
                      client_stops == 1u);
    client_reset_tally();
    client_limit = 32;
    client_ahead = 8;
    ...
    bench.verdict("a write-then-read on an engined host: the write on the pump, the repeated START, 32 "
                  ...
                  st2 == i2c_ok && client_count == 2u && client_restarts == 1u &&
                      same(client_answers, rx_buf, 32));
    client_reset_tally();
    client_limit = 8;
    ...
    spin_us(100);
    bench.verdict("... and the next engined read after that is clean",
                  st6 == i2c_ok && same(client_answers, rx_buf, 16));
    ...
    for (uint8_t k = 0; k < 8u; ++k) {
        ...
        const uint8_t s7 =
            tenure<DmaHost>(client_address, nullptr, 0, rx_buf, 255, I2cSpeed::fast_plus_1m, 50'000u);
        spin_us(100);
    ...
    print(serial, "  eight 255-byte reads on the engines at 1 MHz: ", exact, " exact, TX_OVER ",
          over ? "RAISED" : "never raised", crlf);
    bench.verdict("the engine's bursts never overrun the sixteen-deep FIFO: eight 255-byte reads at 1 MHz "
                  "exact, TX_OVER never raised",
                  exact == 8u && !over);
    i2c0_owner = I2c0Owner::none;
    DmaHost::release();
    client_down();
...
void tg_kernel() {
    ...
    bench.verdict("the NACK of a deaf address is delivered in its place, the tenures around it untouched",
                  kl::Probe::n == 3u && kl::Probe::replies[0] == i2c_ok &&
                      kl::Probe::replies[1] == i2c_nack_addr && kl::Probe::replies[2] == i2c_ok);
    kl::Probe::clear_tally();
    ...
    bench.verdict("the arbiter rejects what it cannot queue, immediately, and every request is still "
    ...
    print(serial, "  a read against a client holding SCL: reply ",
          kl::Probe::n != 0u ? kl::Probe::replies[0] : 0xFFu, " after ", answered_ms, " ms, SCL ",
          scl_low ? "still low" : "released", crlf);
    bench.verdict("THE TIMED BUS: a client holding the clock (a read request never served) is answered "
                  "i2c_timeout at the arbiter's 20 ms with SCL still held - no silicon on this chip answers "
                  "a held clock",
                  kl::Probe::n == 1u && kl::Probe::replies[0] == i2c_timeout && answered_ms >= 19u &&
                      answered_ms <= 25u);
    ...
    bench.verdict("... and the recover()ed engine carries the next two tenures clean",
                  kl::Probe::n == 2u && kl::Probe::replies[0] == i2c_ok &&
                      kl::Probe::replies[1] == i2c_ok && same(client_answers, kl::in_a, 8));
    bus_ao_live = false;
    Host::release();
    client_down();
}
...
void th_unstick() {
    ...
    host_ready();
    const bool ruler_up = Mtime::running();
    const uint8_t healthy = Host::unstick();
    // SDA held low by a GPIO at the far end: a short, as far as nine
    // clocks can tell. The nine pulses are TIMED, which is how this
    // letter states which ruler the unstick got: nine pulses of two half
    // bits each at 5 us a half bit is some 90 us, and a platform timer
    // that is not running would make them vanish.
    (void)Pin<14>::output(false);
    spin_us(10);
    const uint32_t t0 = us_now();
    const uint8_t held = Host::unstick();
    const uint32_t pulse_us = us_now() - t0;
    (void)Pin<14>::release();
    spin_us(10);
    const uint8_t again = Host::unstick();
    (void)client_up();
    const uint8_t probe = tenure<Host>(client_address, nullptr, 0, nullptr, 0);
    print(serial, "  unstick: healthy ", healthy, ", SDA held by a GPIO ", hex(held), " (nine clocks and a "
          "STOP took ", pulse_us, " us), released ", again, "; the probe after: ", probe,
          "; the platform timer was ", ruler_up ? "running" : "STOPPED", crlf);
    ...
    bench.verdict("THE HALF BIT IS PACED BY THE PLATFORM TIMER, which is the one ruler both architectures "
                  "have: the nine clocks and the STOP of a stuck bus take between 90 and 300 us, so every "
                  "one of the twenty waits was served and none was refused for want of a running counter",
                  ruler_up && pulse_us >= 90u && pulse_us <= 300u);
    Host::release();
    client_down();
}
...
void ti_roles() {
    ...
    spin_us(200);
    print(serial, "  I2C1 hosting on GP15/14, I2C0 listening on GP13/12: write ", w, " (",
          heard ? "exact" : "WRONG", "), read ", r, " (",
          same(client_answers, rx_buf, 16) ? "exact" : "WRONG", ")", crlf);
...
// =============================================================================
// j - the pads, and the isolation latch this chip added
// =============================================================================
void tj_pads() {
    if (!wire_or_decline()) {
        return;
    ...
    host_ready();
    const uint32_t pad12 = Gpio::pad(12);
    const uint32_t pad13 = Gpio::pad(13);
    const bool fn_ok = Pin<12>::function() == PinFunction::i2c && Pin<13>::function() == PinFunction::i2c;
    const bool iso_ok = !Pin<12>::isolated() && !Pin<13>::isolated();
    const bool ie_ok = Pin<12>::input_enabled() && Pin<13>::input_enabled();
    const uint32_t wanted = pad_value(i2c_pad_config);
    print(serial, "  after init: GP12 pad=", hex(pad12), " GP13 pad=", hex(pad13), " (the driver asks for ",
          hex(wanted), "), FUNCSEL ", static_cast<uint32_t>(Pin<12>::function()), "/",
          static_cast<uint32_t>(Pin<13>::function()), ", isolated ", Pin<12>::isolated(), "/",
          Pin<13>::isolated(), crlf);
    bench.verdict("THE HAND-OVER DROPS THE ISOLATION LATCH: both pads read back FUNCSEL 3 with ISO clear, "
                  "the input buffer on, and the whole pad register exactly what 12.2.1.3 asks for - the "
                  "pull-up, the slew limit, the Schmitt trigger",
                  fn_ok && iso_ok && ie_ok && pad12 == wanted && pad13 == wanted);
    // The pads still carry a tenure, which is what makes the readback
    // above a fact about the wire and not about a register.
    (void)client_up();
    fill_pattern(tx_buf, 4, 0xC0);
    const uint8_t st = tenure<Host>(client_address, tx_buf, 4, nullptr, 0);
    spin_us(200);
    bench.verdict("... and the pads so configured carry a tenure: four bytes heard at the far end",
                  st == i2c_ok && client_count == 4u && same(tx_buf, client_heard, 4));
    // The unstick takes the pads by hand and gives them back.
    const uint8_t cleared = Host::unstick();
    const uint32_t pad12_after = Gpio::pad(12);
    const uint8_t st2 = tenure<Host>(client_address, tx_buf, 4, nullptr, 0);
    spin_us(200);
    print(serial, "  after unstick(): GP12 pad=", hex(pad12_after), ", the next tenure ", st2, crlf);
    bench.verdict("the unstick takes both pads to SIO and hands them back to the block with the same pad "
                  "register: a tenure after it lands unchanged",
                  cleared == 0u && pad12_after == wanted && st2 == i2c_ok);
    client_down();
    Host::release();
    const uint32_t pad12_released = Gpio::pad(12);
    print(serial, "  after release(): GP12 pad=", hex(pad12_released), " FUNCSEL ",
          static_cast<uint32_t>(Pin<12>::function()), crlf);
    bench.verdict("release() puts the pads back where a reset leaves them (9.11's 0x116: isolated, the "
                  "input buffer off, a pull-down) with no owner",
                  pad12_released == PADS_BANK0_GPIO0_RESET && Pin<12>::isolated() &&
                      !Pin<12>::input_enabled() && Pin<12>::function() == PinFunction::none);
}
...
void tx_trace() {
    ...
    host_ready();
    using C = DwApbI2c<1>;
    print(serial, "  client after init: raw=", hex(C::raw_pending()), " status=", hex(C::flags()), " txflr=",
          C::tx_count(), " con=", hex(C::con()), " sar=", hex(C::own_address()), crlf);
    for (uint8_t round = 0; round < 3u; ++round) {
        ...
        const uint32_t t1 = us_now();
        spin_us(300);
        print(serial, "  round ", round, " (stretch ", client_stretch_us, "): status ", st, " in ", t1 - t0,
              " us, got ", hex(rx_buf[0]), " ", hex(rx_buf[1]), " ", hex(rx_buf[2]), " ", hex(rx_buf[3]),
              "; requests ", client_read_reqs, " given ", client_given, " nacks ", client_nacks, " stops ",
              client_stops, " flushes ", client_flushes, " errors ", client_errors, "; client raw=",
              hex(C::raw_pending()), " txflr=", C::tx_count(), crlf);
...
void banner() {
    print(serial, crlf, "test_rp2350_i2c - the RP2350 I2C (datasheet 12.2, DW_apb_i2c v2.03a) on the ",
          arch_name(), " half: I2C0 hosts on GP13/GP12, I2C1 listens at 0x42 on GP15/GP14; clk_sys=",
          SysClock::hz, " Hz", crlf);
    bench.menu();
}
...
int main() {
    ...
    // The unstick's half-bits are paced by the platform timer
    // (rp2350/delay.hpp), which the Hazard3 half's ticker starts and the
    // Cortex-M33 half's does not. Starting it here is idempotent and
    // makes the two halves the same program.
    ...
    bench.letter('j', "the pads and the isolation latch", tj_pads);
    bench.letter('x', "a traced read (diagnostics, outside z)", tx_trace, false);

    if (serial_ok) {
        brio::print(serial, brio::crlf, "boot: arch=", arch_name(), " clk=", clock_ok ? "PLL150" : "FAILED",
                    " timer0=", timer_ok ? "1us" : "FAILED", " ruler=", ruler_ok ? "1us" : "FAILED",
