...
# definition of done, the samc21 twin of cli/checks/check_family.sh).
#
...
# Negative: every test/family_samc21/neg/*.cpp must FAIL to compile for
...
#
# Usage: brio check samc21            all TUs, all variants
#        brio check samc21 pin        only TUs/negatives matching "pin"
set -u
cd "$(dirname "$0")/../.."
...
for tu in test/family_samc21/*.cpp; do
    [ -e "$tu" ] || continue
...
for tu in test/family_samc21/neg/*.cpp; do
    [ -e "$tu" ] || continue
    case "$tu" in *"$FILTER"*) ;; *) continue ;; esac
