// test_vx03_usb - the reference bench suite for the CH32V203's USB
// device controller (ch32vx03/usb.hpp over RM ch. 21) under the device
...
// (ch32vx03/bus_activity.hpp carries the measurements and the reasons).
...
// private/tools/ch32vx03_usb_host.py, driven by hand one letter at a
...
struct Descriptors {
    ...
    static constexpr auto product = usb_string_descriptor("test_vx03_usb");
...
void ty_echo() {
    ...
    if (wait_for_traffic(10000u) == 0u) {
        print(serial, "  SKIPPED, and it passes as a skip: the host sent nothing in ten "
                      "seconds. The host end is private/tools/ch32vx03_usb_host.py.",
              crlf);
...
// ===========================================================================
void banner() {
    print(serial, crlf, "test_vx03_usb - the USB device controller of RM ch. 21 under the CDC "
                        "ACM stack; HCLK ",
