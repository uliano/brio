// startup_stm32f469.cpp - vector table + reset path for the STM32F469 (and, by the same vector set, the F479, whose header adds CRYP at the one position the F469 leaves empty),
...
// (stm32f469xx.h) but no handler names at all; the spelling every STM32
// tool, template and user knows comes from ST's own startup file
// (cmsis-device-f4 Source/Templates/gcc/startup_stm32f469xx.s, the
...
// lines as <acronym>_IRQHandler, with the ACRONYM of RM0386 table 48. On this
...
// holes: position 79 (CRYP) is the F479's; the F469 has no peripheral there.
// The F469 adds two lines the F42x/F43x have not: QUADSPI at 91 and the
// DSI host at 92, the last two of its 93.
//
...
// Peripheral lines, in IRQn order (stm32f469xx.h / RM0386 table 48).
...
void QUADSPI_IRQHandler()                       __attribute__((weak, alias("Default_Handler")));  // 91
void DSI_IRQHandler()                           __attribute__((weak, alias("Default_Handler")));  // 92
...
constexpr Handler vector_table[] = {
    ...
    LTDC_IRQHandler, LTDC_ER_IRQHandler, DMA2D_IRQHandler, QUADSPI_IRQHandler,  // 88..91
    DSI_IRQHandler,  // 92
};
