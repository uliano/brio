 ...
 * adc.hpp
 *
 * The ADC of the CH32V00x (RM ch. 9): one 12-bit successive-
 * approximation converter over eight pads and two internal sources,
 * a rule group of up to sixteen conversions and an injection group of
 * four, seven internal triggers a group, three analog watchdogs of
 * which two can RESET THE CHIP, one DMA request - the STM32F1's ADC1
 * under WCH's names with a third control register of its own. In the
 * two strata every brio target uses:
 *
 ...
 *                 from the pad (DS table 2-1-1), stated here once.
 *
 *  AdcInput       the two internal sources: VREFINT on channel 8 (1.2 V
 *                 nominal, DS table 3-5) and the OPA's output on 9.
 *
 * WHAT THE F1 HAS NOT. ADC_CTLR3: a LOW-POWER bit (ADC_LP, set at reset,
 * the mode for rates under a megasample - and the mode the SAMPLE TIME
 * TABLE depends on, 9.3.4 giving two columns), a clock duty-cycle
 * knob, and two more watchdogs (AWD1 and AWD2 with their own threshold
 * registers, compare results in CTLR3 cleared by writing zero) beside
 * the F1's AWD0 - EACH OF THE THREE ABLE TO RESET THE SYSTEM when its
 * comparison fails (AWDx_RST_EN; the boot reads ADCRSTF in
 * RCC_RSTSCKR, reset.hpp's ResetFlag::adc), which is what 9's opening
 * paragraph calls "the reset and protection system". What the three
 * guard is the chapter's least clear sentence and the bench's finding:
 * under AWD_SCAN each watchdog takes ONE RANK of a scanned rule
 * sequence - the first conversion against watchdog 0's thresholds, the
 * second against watchdog 1's, the third against watchdog 2's. Also an INPUT
 * BUFFER (CTLR1.BUFEN) for sources above the 50 kOhm the sampling
 * switch tolerates. What the F1 has and this block has not: a
 * calibration - no CAL bit, no procedure, the vendor's own init runs
 * none; and a second sample-time register, the ten channels fitting
 * in SAMPTR2 alone.
 *
 ...
 * values); the datasheet bounds fADC at 48 MHz. A conversion is the
 * sample time plus 12.5 ADCCLK cycles (9.2.2).
 *
 ...
 * a measured path - a pad and a source.
 */
...
struct AdcRegs {
    ...
    uint32_t RESERVED0;           ///< 0x0c (the F1's SAMPTR1: absent, ten channels fit in one)
    ...
    volatile uint32_t CTLR3;      ///< 0x50
    volatile uint32_t WDTR1;      ///< 0x54 watchdog 1's thresholds
    volatile uint32_t WDTR2;      ///< 0x58 watchdog 2's thresholds
};
...
inline constexpr uint32_t adc_tkenable = 1UL << 24;
inline constexpr uint32_t adc_tkitune = 1UL << 25;
...
inline constexpr uint32_t adc_tgregu = 1UL << 4;
inline constexpr uint32_t adc_tginje = 1UL << 5;
...
inline constexpr uint8_t adc_channels = 10;      ///< 0..7 the pads, 8 VREFINT, 9 the OPA
inline constexpr uint8_t adc_pad_channels = 8;
inline constexpr uint32_t adc_steps = 4096;      ///< util/analog.hpp's full scale
inline constexpr uint16_t adc_vrefint_mv = 1200; ///< DS table 3-5, 1.18..1.22 V

/// The two internal sources (9.2.2).
enum class AdcInput : uint8_t { vrefint = 8, opa = 9 };
...
/// SMPx[2:0]: the sample time in ADCCLK cycles, whose table depends on
/// CTLR3.ADC_LP (9.3.4): the codes are the same, the cycles differ at
/// three of them. Named by the low-power column, the reset one.
...
/// The sample time in HALF ADCCLK cycles, so the .5 stays exact.
constexpr uint32_t adc_sample_half_cycles(AdcSampleTime t, bool low_power = true) {
...
/// tCONV in half ADCCLK cycles: the sample time plus 12.5 (9.2.2).
constexpr uint32_t adc_conversion_half_cycles(AdcSampleTime t, bool low_power = true) {
    return adc_sample_half_cycles(t, low_power) + 25u;
}

/// RCC_CFGR0.ADCPRE (3.4.2): a two-level code. The twelve dividers it
/// can produce, each by its lowest code.
struct AdcPrescaler {
    uint8_t code;
    uint8_t divider;
};
...
struct AdcConfig {
    ...
    /// vendor's own choice; /2 is 24 MHz.
    ...
    bool low_power = true;             ///< CTLR3.ADC_LP: the reset value, for rates under 1 Msps
...
/// The watchdogs (9.2.5, 9.3.14). Watchdog 0 is the F1's, with its
/// channel selection; 1 and 2 have thresholds of their own and their
/// results in CTLR3.
struct AdcWatchdogConfig {
    uint16_t low = 0;                  ///< 12 bits
    uint16_t high = 0x0FFF;
    ...
    /// AWDx_RST_EN: a comparison that fails RESETS THE CHIP.
    bool reset_on_fault = false;
};

constexpr bool adc_watchdog_config_valid(const AdcWatchdogConfig& w) {
    return w.low <= 0x0FFFu && w.high <= 0x0FFFu && w.low <= w.high &&
           (!w.channel || *w.channel < adc_channels);
}

/// The pads and their channels (DS table 2-1-1): the pad SAYS the
/// channel, so a wrong one cannot be written.
...
struct AnalogIn {
    static_assert(adc_channel_of(P::pad) != 0xFFu,
                  "brio AnalogIn: this pad is not an ADC input on the CH32V006 (PA2, PA1, "
 ...
 *   Adc::init(clock, {.prescaler_code = brio::adc_prescaler_for(48'000'000, 24'000'000)->code});
 *   Adc::sample_time(Vin::channel, brio::AdcSampleTime::cycles28_5);
 *   Adc::select(Vin{});
...
struct Adc {
    ...
    static constexpr uint8_t opa_channel = 9;
    static constexpr Irq irq() { return Irq::adc; }
    /// Table 8-2: the rule group's request reaches DMA channel 1.
    ...
    /// False for a config the chapter refuses or an ADCCLK above the
    /// datasheet's 48 MHz.
    template <typename Clock>
    static bool init(Clock clock, const AdcConfig& c = {}) {
        ...
        if (c.dma) { c2 |= adc_dma; }
        r.CTLR2 = c2;
        r.CTLR3 = c.low_power ? (r.CTLR3 | adc_lp) : (r.CTLR3 & ~adc_lp);
    ...
    /// The 12 bits whatever the alignment.
    static uint16_t result_counts() {
        const uint16_t v = result();
        return cfg_.left_aligned ? static_cast<uint16_t>(v >> 4) : static_cast<uint16_t>(v & 0x0FFFu);
    }
    ...
    /// too (table 9-3's 111).
    static void trigger(AdcTrigger t, bool opa_instead_of_pad = false) {
        AdcRegs& r = regs();
    ...
    static void injected_trigger(AdcInjectedTrigger t, bool opa_instead_of_pad = false) {
        AdcRegs& r = regs();
    ...
    static int16_t injected_result(uint8_t slot) {
        ...
        if (cfg_.left_aligned) {
            // SIGNB in bit 15, the datum in 14..3.
            return static_cast<int16_t>(static_cast<int16_t>(raw & 0xFFFFu) >> 3);
        }
        return static_cast<int16_t>(raw & 0xFFFFu);   // SIGNB replicated in 15..12
    }
    ...
    static bool watchdog0(const AdcWatchdogConfig& w) {
        ...
        r.CTLR1 = c1;
        r.CTLR3 = w.reset_on_fault ? (r.CTLR3 | adc_awd0_rst_en) : (r.CTLR3 & ~adc_awd0_rst_en);
        return true;
    }
    static void watchdog0_off() {
        AdcRegs& r = regs();
        r.CTLR1 &= ~(adc_awden | adc_jawden | adc_awdie);
        r.CTLR3 &= ~adc_awd0_rst_en;
    }
    /// Watchdogs 1 and 2 (WDTR1/WDTR2): thresholds and the reset enable.
    /// THEY GUARD RANKS, under the watchdog scan (measured, the vendor's
    /// own example's arrangement): with AWD_SCAN set and a SCANNED rule
    /// sequence, watchdog 0 compares the FIRST conversion against
    /// WDHTR/WDLTR, watchdog 1 the SECOND against WDTR1, watchdog 2 the
    /// THIRD against WDTR2 - 9.3.15's "only applicable to watchdog
    /// channel 1" is that rank. Without the scan they compare nothing.
    /// The results are CTLR3's AWDx_RES, write-zero-to-clear; the AWD
    /// flag of STATR is watchdog 0's alone and stays down under the scan.
    static bool watchdog(uint8_t n, uint16_t low, uint16_t high, bool reset_on_fault = false) {
        if (n < 1u || n > 2u || low > 0x0FFFu || high > 0x0FFFu || low > high) {
            return false;
        }
        AdcRegs& r = regs();
        ...
        r.CTLR3 = reset_on_fault ? (r.CTLR3 | rst) : (r.CTLR3 & ~rst);
        return true;
    }
    static bool watchdog_result(uint8_t n) {
        const uint32_t bit = n == 0u ? adc_awd0_res : n == 1u ? adc_awd1_res : adc_awd2_res;
        return (regs().CTLR3 & bit) != 0u;
    }
    static void clear_watchdog_result(uint8_t n) {
        const uint32_t bit = n == 0u ? adc_awd0_res : n == 1u ? adc_awd1_res : adc_awd2_res;
        regs().CTLR3 = regs().CTLR3 & ~bit;   // RW0 beside RW bits: the store keeps the rest
    }
    /// AWD_SCAN (9.3.14): one watchdog per rank of the scanned rule
    /// sequence (the note on watchdog()).
    static void watchdog_scan(bool on) {
        AdcRegs& r = regs();
        r.CTLR3 = on ? (r.CTLR3 | adc_awd_scan) : (r.CTLR3 & ~adc_awd_scan);
    }
...
static_assert(adc_conversion_half_cycles(AdcSampleTime::cycles3_5) == 32u);      // 16 cycles
static_assert(adc_conversion_half_cycles(AdcSampleTime::cycles239_5) == 504u);   // 252 cycles
static_assert(adc_conversion_half_cycles(AdcSampleTime::cycles28_5, false) == 64u);   // 19.5 + 12.5 in the other column
