 ...
 * sleep.hpp
 *
 * util/power.hpp's depth ladder on CH32V203 silicon: `Ch32v203SleepSite`,
 * which arms one of this family's modes and puts the clock tree back
 * afterwards, and `Ch32v203TimedSleepSite`, which additionally keeps
 ...
 * is ch32v203/pwr.hpp's. The POLICY - when a program may stop - is
 ...
 *    (measured: ch32v203/bus_activity.hpp), so a DMA channel or the USB
 ...
 *    path makes the same test on its own (ch32v203/platform.hpp), so a
 ...
 *    after it (ch32v203/platform.hpp) - because that is the only place
 ...
 * `Ch32v203TimedSleepSite` LIFTS it, inside arm() and disarm() alone,
 ...
 *    needs no clock (2.3.5, and ch32v203/rtc.hpp's `arm_wake()`). It is
 ...
 * ch32v203/rtc.hpp's counter elsewhere, and must bind the vector:
 *
 *     extern "C" BRIO_CH32_INTERRUPT void rtc_alarm_handler() { Site::isr(); }
 *
 * WHAT THE SILICON SAID OF ALL THIS is in docs/ch32v203/sleep.md, with
 ...
 *
 *   using Site = brio::Ch32v203SleepSite<Clock>;
 *   brio::PowerManager<P, Site, ...> power;
 *
 * `C` is the application's Clock task or DynamicClock (ch32v203/
...
struct Ch32v203SleepSite {
    static_assert(sleep_site_config_valid(cfg),
                  "brio Ch32v203SleepSite: RAMLV is valid only with the low-power regulator "
     ...
     * `Pwr::wakeup_pin(true)`, the alarm through ch32v203/rtc.hpp) and
...
static_assert(SleepSite<Ch32v203SleepSite<Clock<ClockSource::internal, 8'000'000>>>);

// ---- the timed site ---------------------------------------------------------

/**
 * Ch32v203TimedSleepSite's knobs.
 *
...
struct Ch32v203TimedSleepSite {
    static_assert(timed_sleep_config_valid(cfg),
                  "brio Ch32v203TimedSleepSite: the RTC's prescaler is twenty bits and its "
    ...
    static_assert(timed_sleep_tr_hz(cfg) >= P::ticks_per_second,
                  "brio Ch32v203TimedSleepSite: the RTC tick is the ruler this site places "
    ...
    static_assert(requires {
                  ...
                  "brio Ch32v203TimedSleepSite: a timed site repairs a timebase that STOPS "
                  "in a Stop (the STK ticker, resynchronized through advance()); this "
                  "platform's timebase has no such verb");

    Ch32v203TimedSleepSite() = delete;

    using Plain = Ch32v203SleepSite<C, site_cfg>;
     ...
     * settled. The site still works as a plain Ch32v203SleepSite in that
