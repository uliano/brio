/*
 * parts/ch32v303rc.hpp
 *
 * The CH32V303RC as a table of facts: 256 KB of zero-wait flash out of a
 * 480 KB array, 64 KB of SRAM, LQFP64M with fifty-one pins bonded, the QingKe
 * V4F core with its floating-point unit, and WCH's CH32V30x_D8 device class.
 * The rules this file obeys are parts/ch32v203c8.hpp's; read that one first.
 *
 ...
 * long whose tail is not the CH32V203's at all (startup_ch32v203.S,
 ...
 *
 * THE VC ON FEWER PINS. What separates this part from the CH32V303VC is the
 * package alone - table 2-1-1 gives both the same resources but the FSMC,
 * whose address and data pads the LQFP64M does not bring out - so its facts
 * are the VC's but for the pads and that one block.
 *
 ...
 * ld/ch32v303rc.ld state the split a part leaves the factory in, 256K of
...
inline constexpr const char* part_name = "CH32V303RC";
...
/// being DMA2's eleventh channel (startup_ch32v203.S).
...
// ---- the pads (datasheet 3.2, table 3-1's LQFP64M column) ------------------
/// Which pins each port bonds, as a mask. Fifty-one in all: three whole ports
/// and three pins of port D - PD2 is pin 54, and PD0/PD1 are pins 5 and 6,
/// which come up as OSC_IN/OSC_OUT and are handed to port D by software (the
/// datasheet's note 4). Port E is not bonded.
inline constexpr uint16_t port_pins(char port) {
    return port == 'A' ? 0xFFFFU :
           port == 'B' ? 0xFFFFU :
           port == 'C' ? 0xFFFFU :
           port == 'D' ? 0x0007U :   // PD0, PD1, PD2
           0x0000U;
}
