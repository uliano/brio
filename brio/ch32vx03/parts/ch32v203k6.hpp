/*
 * parts/ch32v203k6.hpp
 *
 * The CH32V203K6 as a table of facts: 32 KB of flash, 10 KB of SRAM,
 * the CH32V203K8's LQFP32 pinout, and WCH's CH32V20x_D6 device class.
 * The rules this file obeys are parts/ch32v203c8.hpp's; read that one
 * first.
 *
 * THE PART THE TWO DOCUMENTS DISAGREE ABOUT. The datasheet V2.8 gives
 * it a column of its own in table 2-1 (32K/10K, LQFP32, a copy of the
 * K8's line otherwise), while its own ordering table names only
 * CH32V203K8T6, and the reference manual V2.3 - the older document -
 * lists seven parts under CH32V20x_D6 without this one. The class is
 * derived rather than quoted, and derived from the rule the manual
 * states for every CH32 part: D6 is "32KB or 64KB" of flash, D8 is
 * 128KB or 256KB. 32 KB makes this D6, which is also what every one of
 * its neighbours in the table is.
 *
 * Everything else here is the K8's, less the memories: the same
 * package, the same bonding, and the 32 KB die's instances (two USARTs,
 * one SPI, one I2C), which are what the C6 has too.
 */
...
inline constexpr const char* part_name = "CH32V203K6";
...
/// part is CH32V20x_D6 by the manual's own size rule (see the file
/// header). A register description that names another class is about
/// other silicon; a driver asks the facts below and names the class
/// only where the manual's note is a list of classes.
