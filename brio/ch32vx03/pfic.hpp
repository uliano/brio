 ...
 * follows the project's CH32VX03_HPE option (ch32vx03/CMakeLists.txt):
 ...
 * caller-saved registers pushed by the hardware on entry and popped on
 * MRET (QingKe V4 manual 3.2), and gcc emits no prologue at all; with it
 * off, the attribute is the plain `interrupt` and the handler saves what
 * it clobbers itself. The two must agree: a fast handler under an HPE
 * that is off returns into a program whose registers it clobbered. One
