 ...
 * ch32vx03/clock.hpp programs ADCPRE in `init()` and publishes
 ...
 * bus master but the core gets a cycle (docs/ch32vx03/dma.md), so an
 ...
 * clock tree and ch32vx03/clock.hpp's `Clock::init()` programs it, so
