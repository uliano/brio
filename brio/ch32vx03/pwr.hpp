 ...
 * chapter (table 2-2); their map is ch32vx03/device.hpp's, because the
 ...
 * instruction is the kernel idle path's, ch32vx03/platform.hpp - and
 ...
 * docs/ch32vx03/README.md), so this file offers the three READS and no
 ...
 * ch32vx03/sleep.hpp is what arms it. VBAT itself is a pin and not a
 * register.
 *
 * The numbers behind the measured statements above, and the rest of
 * what the silicon answered, are in docs/ch32vx03/sleep.md.
 */
...
struct Pwr {
     ...
     * the kernel's idle path (ch32vx03/platform.hpp). `mode()` reads
     ...
     * this stratum (ch32vx03/sleep.hpp's ladder does not reach it).
     *
     ...
     * ch32vx03/pin.hpp's - and its RISING edge is the wake. Returns
    ...
    /// smaller part is a question docs/ch32vx03/sleep.md's gap list
    /// carries.
