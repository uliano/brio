 ...
 *
 * THE SECOND SYSTICK TICKER IN BRIO. This file is samc/ticker.hpp's core
 * line for line (the erratum guard that file also carries is a SAM C21
 * fact and is not copied); the armv6m/ factoring pass that follows the
 * bring-up is where the two become one - see nvic.hpp's header.
 *
...
public:
    ...
    static bool init(C clock) {
        static_assert(clock_follows<C, BasicTicker>(),
                      "brio BasicTicker: SysTick is clocked from HCLK, so a dynamic "
                      "clock must list the ticker among the users it rebases - see "
                      "the caveat in this file's header");
        ...
        // CLKSOURCE = 1: the processor clock (HCLK). The alternative is
        // HCLK/8 (RM0444 5.2: HCLK8), which nothing here needs; CALIB is
        // never read - the reload comes from Clock::hz.
     ...
     * Stop caveat above calls designed work; its intended caller is a
     * timed sleep site that measures on the RTC how long the counters
     * stood still (the samc precedent, SamTimedSleepSite). Thread context
     * only, under the guard. The caller owes the FROZEN span, not the
     * whole slept span: advancing time the handler already counted would
     * mature events EARLY, which brio's time contract forbids.
     */
    ...
    /// deliberately: an escape hatch, not a metrology verb.
    static void pause() {
