 ...
 * samc21/osc32kctrl.hpp; see ticker.hpp for the one consequence a
...
// live in samc21/nvm.hpp: CTRLB.RWS is NVMCTRL's register. The QUESTION is
...
struct Oscctrl {
    ...
    /// samc21/evsys.hpp owns the fabric and knows nothing of oscillators.
...
struct FdpllConfig {
    ...
    /// (samc21/sleep.hpp). Clear - the reset value - lets it stop there,
...
struct Fdpll {
    ...
    static bool init(const FdpllConfig& cfg, uint32_t spins = 4'000'000UL) {
        ...
        // lock time again at the next wake (samc21/sleep.hpp).
