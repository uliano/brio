 ...
 *    only, which is why samc21/ac.hpp uses AC_GCLK_ID. 1.8.9 (the DAC
...
enum class Ref : uint8_t {
    ...
    /// (samc21/supc.hpp's `VrefLevel`) and not here. THE TWO PATHS DIFFER
    ...
    /// The DAC's output (samc21/dac.hpp). Bench-verified and ratiometric:
...
enum class AdcWindow : uint8_t {
     ...
     * silicon question and docs/samc21/adc.md carries the measured answer.
     */
...
enum class AdcInput : uint8_t {
     ...
     * on this family that pad IS an AIN pad (docs/samc21/dac.md).
     */
...
/// meaning, and the same refusal samc21/ac.hpp makes.
...
public:
     ...
     * THEM. samc21/nvm.hpp typed the four fields (two per converter) when
     ...
     * possible (samc21/ac.hpp established the same thing for the
