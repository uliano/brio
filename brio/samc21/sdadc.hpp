 ...
 *    samc21/adc.hpp's is. The bits that exist: SWRST, ENABLE, CTRLC,
...
enum class SdadcRef : uint8_t {
    ...
    /// The DAC's output (samc21/dac.hpp, CTRLB.IOEN). ERRATUM 1.8.10 is
...
/// meaning - the same refusal samc21/adc.hpp, samc21/dac.hpp and samc21/ac.hpp
/// all make.
...
public:
     ...
     *
     * A BOOL AND NOT A VOID STORE, unlike samc21/adc.hpp's `select()`:
     ...
     * the last value read - the same shape samc21/adc.hpp has, for the same
     * reason.
     */
