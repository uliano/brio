 ...
 *    same way for stored records, which is why samc21/nvm_flash.hpp puts
 *    the heap there.
 *
 ...
 *    `write_cell` split, and samc21/nvm_flash.hpp is where the two meet.
 *
 ...
 * computes them, so `init` is normally reached through samc21/clock.hpp
...
/**
 * CTRLB.RWS. Kept as its own type, and kept under the name samc21/clock.hpp
...
struct NvmUserRow {
    ...
    /// SUPC.BODVDD mirrors, in force from power-on (samc21/supc.hpp reads
