 ...
 * `SysTickInterruptGuard` (samc21/ticker.hpp - the file that owns the
 * register) is it.
 *
 ...
 * is SUPC.VREG.RUNSTDBY = 1 (`Vreg::run_standby(true)`, samc21/supc.hpp),
 ...
 * samc21 stratum has been bitten by): 1.8.1 (Idle does not stop the
...
enum class VregStandbyMode : uint8_t {
    ...
    /// SUPC.VREG.RUNSTDBY (samc21/supc.hpp's `Vreg::run_standby(true)`)
 ...
 * wants the slack back measures its own board (samc21/freqm.hpp is the
 ...
 * using this site must not drive samc21/rtc.hpp elsewhere, and the app
