 ...
 * - so `samc21/nvm.hpp`'s `NvmUserRow` and this header describe the same
...
struct BodVdd {
     ...
     * set it?
     *
     * This is the same cross-check `samc21/reset.hpp` makes for the
     * watchdog, from the other end: `samc21/nvm.hpp` reads the user row,
...
struct VrefConfig {
    ...
    /// docs/samc21/ac.md's gap list has been waiting for.
