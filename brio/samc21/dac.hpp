 ...
 * samc21/adc.hpp spells its chapter's:
 *
 ...
 *    docs/samc21/dac.md carries what the silicon actually does.
 ...
 *    the separate verb, exactly as samc21/adc.hpp's `select()` is (the
 ...
 * disagreement; docs/samc21/dac.md carries the measurement that settles
 * it.
 *
 * `DacRef` is this converter's OWN vocabulary and deliberately not
 * `brio::Ref`, which samc21/adc.hpp owns for REFCTRL.REFSEL: on this
...
#include "samc21/device_tables.hpp"
...
enum class DacRef : uint8_t {
    ...
    /// (samc21/supc.hpp's `VrefLevel`) and not a knob of this peripheral.
    /// THE DEVICE HEADER CALLS THIS VALUE `INT1V`, a fixed 1.0 V
    /// reference this family does not have; 41.8.2 is the authority on
    /// what it means and docs/samc21/dac.md on what it measures.
...
/// meaning - the same refusal samc21/adc.hpp and samc21/ac.hpp make.
...
struct DacConfig {
    ...
    /// (docs/samc21/dac.md). EOEN is for the PAD.
...
public:
     ...
     * bus. This is the same shape samc21/adc.hpp's `select()` has, for
     ...
     * either reading - and docs/samc21/dac.md carries what the silicon
