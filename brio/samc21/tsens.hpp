 ...
 *    ARGUMENT (the samc21/freqm.hpp `reference_hz` pattern - a ratio
 ...
 *    OFFSET, which is where it is actually applied; docs/samc21/tsens.md
 ...
 * (docs/samc21/tsens.md): the conversion takes 699 ms instead of 3.7 ms -
 ...
 * extends it and `tsens_value_fits()` bounds it - and docs/samc21/tsens.md
...
enum class TsensWindow : uint8_t {
    ...
    /// refuses either threshold order for this mode; docs/samc21/tsens.md
...
/// meaning - the refusal samc21/dac.hpp, samc21/adc.hpp and samc21/sdadc.hpp
/// all make.
...
public:
     ...
     * can be forgotten.
     *
     * NO PATH IS REFUSED, and that is the difference from samc21/dac.hpp's
     * and samc21/sdadc.hpp's identical-looking verbs: table 29-3 gives
