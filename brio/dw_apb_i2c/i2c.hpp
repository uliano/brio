 ...
 * i2c.hpp
 *
 * THE SYNOPSYS DESIGNWARE I2C (DW_apb_i2c), written once for every
 * family that carries it: an IP STRATUM, a directory named for a
 * peripheral DESIGN rather than for a silicon, sitting where a core
 * stratum sits - above util/, below the families
 * (docs/dw_apb_i2c/README.md, and the rule in docs/design/overview.md:
 * "A core stratum sits between util/ and the families that share a
 * core").
 *
 * Three layers, the arrangement every brio bus driver has:
 *
 *  DwApbI2cBlock<Chip, n>  the RESOURCE: the register block behind the
 *                          family's gate, the configuration under the
 *                          block's rule (IC_CON, the counts, the
 *                          addresses and the hold times take a write
 *                          only with ENABLE clear), the command
 *                          register, the flags, the thirteen interrupt
 *                          sources with their read-to-clear registers,
 *                          the abort source decoded. It decides nothing.
 *  DwApbI2cHost<Chip, n, pins, TxEngine, RxEngine>
 *                          the ENGINE util/i2c_bus.hpp's I2cBus drives:
 *                          the Request is the other strata's VERBATIM -
 *                          {addr, tx span, rx span, reply, speed}, one
 *                          bus tenure that is a write, a read, a
 *                          write-then-read with a repeated START, or the
 *                          probe - and the outcome vocabulary is
 *                          i2c_bus.hpp's, read off IC_TX_ABRT_SOURCE.
 *  DwApbI2cClient<Chip, n, pins>
 *                          the target side: a polled surface and one ISR
 *                          body reporting one event per call,
 *                          deliberately thin, because a client is a
 *                          protocol and which protocol is the
 *                          application's.
 *
 ...
 * request. WHAT A FAMILY OWES IT is the `DwApbI2cChip` concept below,
 * satisfied by one traits type the family's own header defines; that
 * header also keeps the PUBLIC NAMES apps use (`DwApbI2c<n>`,
 * `I2cHost<n, pins, ...>`, `I2cClient<n, pins>`) as aliases of these
 * three templates, so nothing above ever spells a Chip. Every register
 * bit below is spelled as the block's own description gives it, in this
 * file's words; a family whose vendor device description names the same
 * bits checks the two against each other once, in its own header.
 *
 * THE MACHINE THIS BLOCK PRESCRIBES IS A COMMAND FIFO, not an event
 ...
 * that entry. AN EMPTY TRANSMIT FIFO WITH NO STOP HOLDS SCL LOW (the bus
 * stalls until the next entry), which is why the last entry of every
 * tenure carries STOP and the pump keeps the FIFO ahead of the wire.
 * Nothing stands for an address-only frame: the controller cannot issue
 * START, the address and STOP with no data entry, so THE PROBE ON THIS
 * BLOCK IS A ONE-BYTE READ - START, the address with the read bit, one
 * byte taken and not acknowledged, STOP - the answer still being the
 * acknowledge on the address (i2c_nack_addr when nobody is home). A
 * NACK, a lost arbitration and every other failure come back as ONE
 * interrupt, TX_ABRT, with the reason in IC_TX_ABRT_SOURCE, and the
 * FIFOs are flushed and held so until IC_CLR_TX_ABRT is read.
 *
 * THE SCL TIMING is two counts per speed (HCNT, LCNT in ic_clk cycles,
 * the rate the family names), the spike filter (SPKLEN), and the SDA
 * hold and setup times, under the block's floors (LCNT at least SPKLEN +
 * 7, HCNT at least SPKLEN + 5) and WITH THE CYCLES THE BLOCK ADDS (the
 ...
 * specification's) bound the split, and a clock too slow to meet them
 * makes the speed unreachable - answered i2c_rejected.
 *
 ...
 * across the word, so a command is written as a halfword), the bytes
 * come back through the receive engine, and the first command (RESTART)
 * and the last (STOP) are the pump's - the last written on TX_EMPTY when
 * the engine's block left no room, because the request is a level the
 * DMA banks credits on while the FIFO is empty and spends in a burst
 * that can fill it. A write phase stays on the pump: its entries are the
 * request's bytes with a flag each, and a byte buffer cannot carry the
 * flag.
 *
 * NOT built, each with its reason in docs/dw_apb_i2c/README.md: the
 * high-speed mode, SMBus, the stuck-bus timeouts, the bus clear of the
 * blocks that have one. The slots for DMA engines ARE here, empty, so
 * the type's shape does not move when a family fills them.
 */
 ...
 */
struct DwApbI2cAbsentEngine {
    DwApbI2cAbsentEngine() = delete;
 ...
 * `ic_clk_hz(clock)` - it is a template over the family's OWN clock
 * types, of which this file knows none - so it has a concept of its own,
 * `DwApbI2cChipClock`, checked at the two call sites that need it, the
 * host's and the client's init().
 */
template <typename C>
concept DwApbI2cChip =
    requires {
        /// The register block of one instance, as the family's device
        /// description spells it: the member names are the block's own
        /// (IC_CON, IC_DATA_CMD, IC_TX_ABRT_SOURCE, ...).
        typename C::Regs;
        ...
        /// The family's pin-set type, one pad per line: which pads exist
        /// and which function routes an I2C to them is the family's
        /// table, never this file's.
        typename C::Pins;
        /// What the family's DMA calls a peripheral request.
        typename C::DmaRequest;
        /// The family's microsecond ruler, as a value computed once per
        /// clock change: the unstick drives the wire by hand and needs
        /// half a bit of standard mode.
        typename C::SpinRate;
    ...
    requires(volatile uint32_t& reg, uint32_t bits, typename C::Pins pins, uint8_t instance,
             typename C::Irq line, typename C::SpinRate rate, uint32_t hz, uint32_t us) {
        /// Instance 0 answers every per-instance question; so does every
        /// other instance the family has. Each is a template on the
        /// instance number so the answer is a compile-time constant - a
        /// register address, a line, a request.
        ...
        /// read-modify-write: the enable, the abort and the command
        /// block are single bits of IC_ENABLE, and the interrupt mask is
        /// touched from two contexts.
        C::set_bits(reg, bits);
        C::clear_bits(reg, bits);
        /// The pads: is this pin set legal for this instance, and which
        /// pad carries each line. Then the PAD ITSELF as a type, with
        /// the function code that routes an I2C to it and the electrical
        /// setup the two lines want - and the five verbs the unstick
        /// drives an open-drain line with by hand, the output enable
        /// being the drive over an output register holding a zero.
        { C::pins_valid(instance, pins) } -> std::same_as<bool>;
        { C::scl_pad(pins) } -> std::same_as<uint8_t>;
        { C::sda_pad(pins) } -> std::same_as<uint8_t>;
        C::template Pad<0>::function(C::pad_function, C::pad_config);
        C::template Pad<0>::release();
        C::template Pad<0>::input(C::open_drain_pull);
        C::template Pad<0>::clear();
        { C::template Pad<0>::read() } -> std::same_as<bool>;
        C::template Pad<0>::drive_low();
        C::template Pad<0>::release_drive();
        /// Two engines of one host must not name one DMA channel; what
        /// "the same channel" means is the family's DMA's business.
        { C::template engines_distinct<DwApbI2cAbsentEngine, DwApbI2cAbsentEngine>() }
            -> std::same_as<bool>;
        /// The line's two verbs, on the family's own controller.
        C::Interrupts::enable(line);
        C::Interrupts::disable(line);
        /// The microsecond ruler: the factor for a rate, computed at
        /// every clock change, and the wait itself.
        { C::spin_rate(hz) } -> std::same_as<typename C::SpinRate>;
        C::spin_us(rate, us);
    };

/**
 * WHICH CLOCK OF THIS FAMILY'S TREE IS ic_clk - the half of the contract
 * that is a template over the family's clock types and so can only be
 * checked where a clock is in hand (the two init()s).
 */
template <typename C, typename Clock>
concept DwApbI2cChipClock = requires(Clock clock) {
    { C::ic_clk_hz(clock) } -> std::same_as<uint32_t>;
};
...
/// The three speeds the block runs (no high-speed mode here).
...
/// IC_CON: the role, the speed and the seven behaviour bits.
struct I2cControl {
    static constexpr uint32_t host_mode = 1u << 0;
    static constexpr uint32_t speed_lsb = 1;
    static constexpr uint32_t speed_bits = 0x3u << speed_lsb;
    static constexpr uint32_t ten_bit_client = 1u << 3;
    static constexpr uint32_t ten_bit_host = 1u << 4;
    static constexpr uint32_t restart_enable = 1u << 5;
    static constexpr uint32_t client_disable = 1u << 6;
    static constexpr uint32_t stop_det_if_addressed = 1u << 7;
    static constexpr uint32_t tx_empty_late = 1u << 8;
    static constexpr uint32_t hold_when_rx_full = 1u << 9;
    /// IC_CON.SPEED's two codes: standard is 1, fast and fast-mode-plus
    /// share 2.
    static constexpr uint8_t speed_standard = 1;
    static constexpr uint8_t speed_fast = 2;
};

/// IC_CON.SPEED: standard is 1, fast and fast-mode-plus share 2.
constexpr uint8_t i2c_speed_code(I2cSpeed s) {
    return s == I2cSpeed::standard_100k ? I2cControl::speed_standard : I2cControl::speed_fast;
}

/// The minimum ic_clk a speed needs: the rate at which the block's own
/// floors (SPKLEN + 5 and SPKLEN + 7 cycles, with the spike filter at
/// the specification's 50 ns) still fit the specification's high and low
/// times. Each family's data sheet tabulates the same three numbers.
...
/// The block's arithmetic. The wire period is hz / speed cycles, split
/// into a high of at least the specification's minimum (and at least
/// what HCNT's floor allows) and a low of the rest, the low taking the
/// larger share (3/5, the vendor's ratio) when the period leaves room;
/// the cycles the block adds (SPKLEN + 7 to the high, 1 to the low) are
/// subtracted from the programmed counts. Nullopt when the clock is
/// below the speed's floor, when the split cannot meet both minima, or
/// when the hold could not fit the low period (IC_SDA_HOLD's rule: at
/// most LCNT - 2).
constexpr std::optional<I2cTiming> i2c_timing_for(uint32_t hz, I2cSpeed speed) {
    ...
    // The floors, as the data sheets' own minimum-clock rows have them
    // (HCNT = SPKLEN + 5, LCNT = SPKLEN + 7 at every speed's minimum
    // clock; the block's "larger than" is read as those rows set it):
    // high >= 2 x SPKLEN + 12, low >= SPKLEN + 8.
...
/// What a timing really produces on an ideal wire (the rise and fall
/// times at zero): hz over the two periods the block generates.
...
constexpr uint32_t i2c_con_of(const I2cConfig& c) {
    uint32_t v = static_cast<uint32_t>(i2c_speed_code(c.speed)) << I2cControl::speed_lsb;
    if (c.role == I2cRole::host) {
        v |= I2cControl::host_mode | I2cControl::client_disable;
    }
    if (c.ten_bit_host) { v |= I2cControl::ten_bit_host; }
    if (c.ten_bit_client) { v |= I2cControl::ten_bit_client; }
    if (c.restart_enabled) { v |= I2cControl::restart_enable; }
    if (c.stop_if_addressed) { v |= I2cControl::stop_det_if_addressed; }
    if (c.tx_empty_late) { v |= I2cControl::tx_empty_late; }
    if (c.hold_when_rx_full) { v |= I2cControl::hold_when_rx_full; }
    return v;
}

/// IC_DATA_CMD: the byte, the read command and the two flags an entry
/// carries, and the first-data-byte status a read reports.
struct I2cData {
    static constexpr uint32_t byte = 0xFFu;
    static constexpr uint32_t read = 1u << 8;
    static constexpr uint32_t stop = 1u << 9;
    static constexpr uint32_t restart = 1u << 10;
    static constexpr uint32_t first_data_byte = 1u << 11;
};
...
constexpr uint16_t i2c_write_entry(uint8_t byte, I2cCmd c = {}) {
    return static_cast<uint16_t>(byte | (c.restart ? I2cData::restart : 0u) |
                                 (c.stop ? I2cData::stop : 0u));
}
constexpr uint16_t i2c_read_entry(I2cCmd c = {}) {
    return static_cast<uint16_t>(I2cData::read | (c.restart ? I2cData::restart : 0u) |
                                 (c.stop ? I2cData::stop : 0u));
}

/// IC_ENABLE: the enable, the abort and the command block.
struct I2cEnable {
    static constexpr uint32_t enable = 1u << 0;
    static constexpr uint32_t abort = 1u << 1;
    static constexpr uint32_t command_block = 1u << 2;
};

/// IC_ENABLE_STATUS: what the hardware says about a disable.
struct I2cEnableStatus {
    static constexpr uint32_t running = 1u << 0;
    static constexpr uint32_t client_disabled_while_busy = 1u << 1;
    static constexpr uint32_t client_rx_data_lost = 1u << 2;
};

/// IC_STATUS's flags.
struct I2cFlag {
    static constexpr uint32_t activity = 1u << 0;
    static constexpr uint32_t tx_not_full = 1u << 1;
    static constexpr uint32_t tx_empty = 1u << 2;
    static constexpr uint32_t rx_not_empty = 1u << 3;
    static constexpr uint32_t rx_full = 1u << 4;
    static constexpr uint32_t host_active = 1u << 5;
    static constexpr uint32_t client_active = 1u << 6;
};
...
/// IC_CLR_* register.
struct I2cInterrupt {
    static constexpr uint32_t rx_under = 1u << 0;
    static constexpr uint32_t rx_over = 1u << 1;
    static constexpr uint32_t rx_full = 1u << 2;       ///< the receive FIFO at or above RX_TL + 1
    static constexpr uint32_t tx_over = 1u << 3;
    static constexpr uint32_t tx_empty = 1u << 4;      ///< the transmit FIFO at or below TX_TL
    static constexpr uint32_t rd_req = 1u << 5;        ///< a host reads this client: SCL held until a byte is given
    static constexpr uint32_t tx_abrt = 1u << 6;       ///< the transaction failed: the source says why
    static constexpr uint32_t rx_done = 1u << 7;       ///< the host did not acknowledge a byte this client gave
    static constexpr uint32_t activity = 1u << 8;
    static constexpr uint32_t stop_det = 1u << 9;
    static constexpr uint32_t start_det = 1u << 10;
    static constexpr uint32_t gen_call = 1u << 11;
    static constexpr uint32_t restart_det = 1u << 12;
    static constexpr uint32_t all = 0x1FFFu;
};

/// The reasons IC_TX_ABRT_SOURCE names, as the resource decodes them.
struct I2cAbort {
    static constexpr uint32_t addr_noack = (1u << 0) |    ///< no ACK on a 7-bit address
                                           (1u << 1) |    ///< no ACK on a 10-bit address's first byte
                                           (1u << 2) |    ///< no ACK on its second
                                           (1u << 4);     ///< no ACK on the general call
    static constexpr uint32_t data_noack = 1u << 3;
    static constexpr uint32_t arb_lost = 1u << 12;
    static constexpr uint32_t client_flush = 1u << 13;      ///< a read arrived with stale bytes queued
    static constexpr uint32_t client_arb_lost = 1u << 14;
    static constexpr uint32_t client_read_cmd = 1u << 15;   ///< a client wrote a READ command
    static constexpr uint32_t user_abort = 1u << 16;
    static constexpr uint32_t flush_count_lsb = 23;
};

/// util/i2c_bus.hpp's code for an abort source.
...
/// IC_SDA_HOLD's two halves: the transmit hold in the low sixteen bits,
/// the receive extension in the eight above them.
struct I2cHoldField {
    static constexpr uint32_t tx = 0xFFFFu;
    static constexpr uint32_t rx = 0xFFu << 16;
    static constexpr uint32_t rx_lsb = 16;
};

/// IC_TAR and IC_SAR: ten bits of address, and IC_TAR's SPECIAL bit -
/// the general call instead of the address.
struct I2cAddressField {
    static constexpr uint32_t address = 0x3FFu;
    static constexpr uint32_t special = 1u << 11;
};

/// IC_TXFLR and IC_RXFLR: how many entries are in a FIFO.
struct I2cLevelField {
    static constexpr uint32_t count = 0x1Fu;
};

/// IC_DMA_CR's two request enables, and the field of the two watermark
/// registers IC_DMA_TDLR / IC_DMA_RDLR.
struct I2cDmaControl {
    static constexpr uint32_t rx = 1u << 0;
    static constexpr uint32_t tx = 1u << 1;
    static constexpr uint32_t level = 0xFu;
};
...
template <DwApbI2cChip Chip, uint8_t n>
struct DwApbI2cBlock {
    static_assert(n < Chip::instances, "this family has no DW_apb_i2c with that number");
    DwApbI2cBlock() = delete;
    ...
    static bool enabled() { return (regs().IC_ENABLE & I2cEnable::enable) != 0u; }
    /// What the hardware says (IC_ENABLE_STATUS.IC_EN): a disable takes
    /// until the bus activity under way completes.
    static bool running() { return (regs().IC_ENABLE_STATUS & I2cEnableStatus::running) != 0u; }
    static void enable() { Chip::set_bits(regs().IC_ENABLE, I2cEnable::enable); }
    /// Disable and wait, bounded, for the hardware to report it: true
    /// when it did. A host mid-command with no STOP in sight never
    /// disables (the block's own note) - abort() first.
    static bool disable(uint32_t spins = 100'000u) {
        Chip::clear_bits(regs().IC_ENABLE, I2cEnable::enable);
        while (running() && spins-- != 0u) {
        }
        return !running();
    }
    /// IC_ENABLE.ABORT: STOP after the entry under way, the transmit
    /// FIFO flushed, TX_ABRT raised with ABRT_USER_ABRT. A host verb;
    /// ignored unless enabled.
    static void abort() { Chip::set_bits(regs().IC_ENABLE, I2cEnable::abort); }
    static bool aborting() { return (regs().IC_ENABLE & I2cEnable::abort) != 0u; }
    /// The two facts a disable reports about a client tenure it cut.
    static bool client_disabled_while_busy() {
        return (regs().IC_ENABLE_STATUS & I2cEnableStatus::client_disabled_while_busy) != 0u;
    }
    static bool client_rx_data_lost() {
        return (regs().IC_ENABLE_STATUS & I2cEnableStatus::client_rx_data_lost) != 0u;
    }
    ...
    static bool timing(const I2cTiming& t) {
        ...
        regs().IC_SDA_HOLD = (regs().IC_SDA_HOLD & I2cHoldField::rx) | t.sda_tx_hold;
    ...
    static bool sda_rx_hold(uint8_t cycles) {
        ...
        regs().IC_SDA_HOLD = (regs().IC_SDA_HOLD & I2cHoldField::tx) |
                             (static_cast<uint32_t>(cycles) << I2cHoldField::rx_lsb);
        return true;
    }
    static I2cTiming timing(I2cSpeed s) {
        ...
        t.sda_tx_hold = static_cast<uint16_t>(regs().IC_SDA_HOLD & I2cHoldField::tx);
    ...
    static bool target(uint16_t addr, bool general_call = false) {
        ...
        regs().IC_TAR = (static_cast<uint32_t>(addr) & I2cAddressField::address) |
                        (general_call ? I2cAddressField::special : 0u);
        return true;
    }
    static uint16_t target() { return static_cast<uint16_t>(regs().IC_TAR & I2cAddressField::address); }
    /// The client's own address (IC_SAR); refused while enabled.
    static bool own_address(uint16_t addr) {
        if (enabled()) {
            return false;
        }
        regs().IC_SAR = static_cast<uint32_t>(addr) & I2cAddressField::address;
        return true;
    }
    static uint16_t own_address() { return static_cast<uint16_t>(regs().IC_SAR & I2cAddressField::address); }
    ...
    static uint8_t tx_count() { return static_cast<uint8_t>(regs().IC_TXFLR & I2cLevelField::count); }
    static uint8_t rx_count() { return static_cast<uint8_t>(regs().IC_RXFLR & I2cLevelField::count); }

    /// One IC_DATA_CMD entry (i2c_write_entry / i2c_read_entry).
    static void push(uint16_t entry) { regs().IC_DATA_CMD = entry; }
    /// One received byte.
    static uint8_t pop() { return static_cast<uint8_t>(regs().IC_DATA_CMD & I2cData::byte); }
    /// Whether the last pop() was the first byte after an address phase.
    static bool first_data_byte() { return (regs().IC_DATA_CMD & I2cData::first_data_byte) != 0u; }
    ...
    static void interrupts(uint32_t mask, bool on) {
        if (on) { Chip::set_bits(regs().IC_INTR_MASK, mask); } else { Chip::clear_bits(regs().IC_INTR_MASK, mask); }
    }
    ...
    /// Returns the source it stood for.
    ...
    static void dma_requests(bool tx, bool rx) {
        regs().IC_DMA_CR = (tx ? I2cDmaControl::tx : 0u) | (rx ? I2cDmaControl::rx : 0u);
    }
    /// The DMA watermarks (IC_DMA_TDLR / IC_DMA_RDLR), 0 at reset and
    /// left there: the block requests single transfers.
    static void dma_levels(uint8_t tx, uint8_t rx) {
        regs().IC_DMA_TDLR = tx & I2cDmaControl::level;
        regs().IC_DMA_RDLR = rx & I2cDmaControl::level;
    }
    ...
    static void command_block(bool on) {
        if (on) { Chip::set_bits(regs().IC_ENABLE, I2cEnable::command_block); }
        else { Chip::clear_bits(regs().IC_ENABLE, I2cEnable::command_block); }
    }
};
 ...
 * spans empty is the PROBE, served as a one-byte read (the file header:
 * this controller has no address-only entry) - the ACK on the address is
 ...
 * serving a READ phase of three bytes or more (the file header): the
...
class DwApbI2cHost {
    using S = DwApbI2cBlock<Chip, n>;
    static_assert(Chip::pins_valid(n, pins),
                  "these pads cannot carry this instance's lines: an I2C's SCL and SDA pads are "
                  "fixed per instance, each under the one function that routes the I2C to it - "
                  "see the family's own pin table");
    ...
    static_assert([] {
        if constexpr (TxEngine::present) {
         ...
         "a narrow write is replicated across it), the receive engine uint8_t bytes");

    using SclPad = typename Chip::template Pad<Chip::scl_pad(pins)>;
    using SdaPad = typename Chip::template Pad<Chip::sda_pad(pins)>;
...
public:
    ...
    /// tag, and the family's traits say which of its rates is ic_clk.
    /// The three speeds are solved here and at rebase(); one the clock
    /// cannot produce is marked unreachable (speed_ok()) and a Request
    /// naming it is answered i2c_rejected. False when not even
    /// standard_100k is legal (a clock under 2.7 MHz) or the block did
    /// not come up.
    template <typename Clock>
    static bool init(Clock clock) {
        static_assert(DwApbI2cChipClock<Chip, Clock>,
                      "the family's chip traits must say which rate of this Clock type is ic_clk "
                      "(DwApbI2cChipClock's ic_clk_hz)");
        static_assert(clock_follows<Clock, DwApbI2cHost>(),
        ...
        SclPad::function(Chip::pad_function, Chip::pad_config);
        SdaPad::function(Chip::pad_function, Chip::pad_config);
        status_ = i2c_ok;
        phase_ = Phase::idle;
        S::enable();
    ...
    /// ic_clk changed (DynamicClock fan-out): the three rows solved
    ...
    /// The ENGINE's phase, not the wire's.
    ...
    /// open-drain through the family's own software path (the pad driven
    /// low or released to the pull-up), with the pads reclaimed from the
    /// peripheral for the duration. RECOVER() FIXES THE PERIPHERAL, THIS
    /// FIXES THE WIRE. Returns the pulses it took a stuck client to
    /// release SDA (0 = the wire was never stuck), or 0xFF when nine
    /// pulses and a STOP left SDA low - a short, not a client. A healthy
    /// wire is left alone.
    static uint8_t unstick() {
        Chip::Interrupts::disable(S::irq());
        SclPad::input(Chip::open_drain_pull);
        SdaPad::input(Chip::open_drain_pull);
        // Open drain by hand: the output held low, the output enable is
        // the drive.
        SclPad::clear();
        SdaPad::clear();

        uint8_t released_at = 0xFF;
        bool free_now = SdaPad::read();
        if (!free_now) {
            uint8_t pulses = 0;
            for (uint8_t i = 0; i < 9u && released_at == 0xFF; ++i) {
                SclPad::drive_low();
                spin_half_bit();
                SclPad::release_drive();
                spin_half_bit();
                ++pulses;
                if (SdaPad::read()) {
                    released_at = pulses;
            ...
            SdaPad::drive_low();
            spin_half_bit();
            SdaPad::release_drive();
            spin_half_bit();
            free_now = SdaPad::read();
        } else {
            released_at = 0;
        }
    ...
    /// the block enabled forever, and the family's reset is the one
...
private:
    ...
    static inline typename Chip::SpinRate spin_rate_{};
};
 ...
 * then: a byte written to it lands in the receive FIFO (RX_FULL), a read
 * of it raises RD_REQ and HOLDS SCL until a byte is given - the clock
 * stretch is the block's own; the host's NACK at the end of a read is
 * RX_DONE; a repeated START while addressed is RESTART_DET; the STOP is
 * STOP_DET, raised for this client's tenures alone
 * (STOP_DET_IFADDRESSED). A read arriving with bytes still queued from
 * an earlier one flushes them and says so (ABRT_SLVFLUSH_TXFIFO); bytes
 * queued beyond what the host takes are flushed at its NACK the same
 * way. There is no address-match event: the tenure is known by its first
 * byte or its first read request.
 *
 *   extern "C" void isr_i2c1() { act(Peer::service()); }
 */
template <DwApbI2cChip Chip, uint8_t n, typename Chip::Pins pins>
class DwApbI2cClient {
    using S = DwApbI2cBlock<Chip, n>;
    static_assert(Chip::pins_valid(n, pins),
                  "these pads cannot carry this instance's lines - see the family's own pin table");
    using SclPad = typename Chip::template Pad<Chip::scl_pad(pins)>;
    using SdaPad = typename Chip::template Pad<Chip::sda_pad(pins)>;

public:
    DwApbI2cClient() = delete;
    using Resource = S;
    static constexpr typename Chip::Pins pin_set = pins;
    ...
    /// Bring the instance up as a bus target. `clock` is the app's Clock
    /// tag, and the family's traits say which of its rates is ic_clk.
    /// False when the block did not come up or the speed's timing cannot
    /// be met at this clock.
    template <typename Clock>
    static bool init(Clock clock, const Config& cfg) {
        ...
        const auto t = i2c_timing_for(Chip::ic_clk_hz(clock), cfg.speed);
        if (!t) {
            return false;
        }
        I2cConfig c{};
    ...
    /// One event per call, the flag consumed where the block consumes
...
// =============================================================================
// The block's own arithmetic, pinned at compile time
// =============================================================================
...
// The floors reproduced: at 12 MHz fast mode is the block's own minimum
// (low 16, high 14 cycles); at 2.7 MHz standard mode is (13, 14); a
// hertz below either is refused, and fast-mode-plus needs 32 MHz.
...
static_assert(i2c_con_of({}) == (I2cControl::host_mode | I2cControl::client_disable |
