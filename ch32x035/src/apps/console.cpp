// console - the brio kernel console on the CH32X035: three active objects
// over USART2, and the kernel this repository is built around on the
// series' QingKe V4C.
//
...
// every other target compiles. Only the last lines are this target's: the
// clock type, the transport, and the two vector bindings.
//
...
// serial_port.hpp.
//
// The board is WCH's CH32X035 evaluation board in its QFN20 edition (a
// CH32X035F8U6): no crystal - the part has no oscillator for one - and two
// LEDs on the header P4 that reach a pad only through a jumper, each
// hanging from 3.3 V so that it lights with its pad LOW. This program
// drives PA0, the pad WCH's own GPIO example drives, so with P4's LED1
// jumpered there `LED ON` darkens it. The console is USART2 on PA2 (TX)
// and PA3 (RX), the board's "Serial port 2", wired to the WCH-LinkE's own
// serial; USART1's default TX pad is not bonded by this package. Connect at
// 115200 8N1 and type:
//   HELP | LED ON|OFF|TOG | UPTIME | BEATS | ERR
//
// Between keystrokes the CPU sleeps in the idle hook, woken by the system
// counter or the USART. No polling anywhere.
//
// build: boards = x035r8,x035c8,x035g8u,x035g8r,x035f8,x035f7,x033f8
...
// The clock: the HSI undivided - 48 MHz, the top of this series' range and
// its only root, and the ONE truth about HCLK every driver below derives
// from.
...
using Serial = brio::Uart<2, P>;   // USART2 on PA2/PA3, rings 64/64
constexpr Serial serial;           // tag for print(serial, ...)

constexpr uint32_t console_baud = 115200;

using Led = brio::Pin<'A', 0>;     // P4's LED1, jumpered (see header)
...
private:
    ...
    static void cmd_help(const Cmd&, Serial s) {
        brio::print(s, "commands: HELP | LED ON|OFF|TOG | UPTIME | BEATS | ERR", brio::crlf);
    }
...
int main()
{
    const bool clock_ok = SysClock::init();            // HSI undivided -> 48 MHz
