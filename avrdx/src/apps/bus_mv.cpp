// bus_mv - a bench voltmeter for an I2C bus: this board measures, with
// its own ADC, everything the desk has no meter for. VDD and VDDIO2
// come free through the internal dividers; the two bus nodes need one
// jumper each onto PORTD's analog pins:
//
//     PD1 (AIN1) -> the SDA node        PD2 (AIN2) -> the SCL node
//
...
int main() {
    ...
    for (;;) {
        ...
        // The node truth this board always has: its own digital taps.
        // If PA2 reads 1 while PD1's mV are low, the PD1 jumper is not
        // on the node.
