...
- Firmware today: `test_stm32_platform` (stm32g0).
...
position E on a direct USB port, running `test_stm32_platform`
(stm32g0) - the third architecture's bring-up day, closed by its
platform + reset suite; the SAM boards are as described below (now at
3.3 V), with C's probe temporarily unplugged.**
