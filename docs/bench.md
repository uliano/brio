...
**Today's end state (2026-09-07, late): THE SELF-LINK'S SIX WIRES HAVE
MOVED TO THE SAM.** Step (b) of the G0 bus campaigns is on the desk: the
Nucleo-G0B1RE at position E and ONE SAM C21 on position D's probe are
tied by the six-wire link of "The G0-to-SAM bus link" above (SPI1 to
SERCOM1, I2C1 to SERCOM3, the I2C pull-ups unchanged, a dedicated GND),
so the Nucleo's own SPI1-to-SPI2 and I2C1-to-I2C2 self-links are BROKEN
and both bus suites now run their peer halves instead - `test_stm32_spi`
z 38/38 and `test_stm32_i2c` z 54/54, each twice including a run from a
cold flash, with the self-link letters skipping themselves after a probe
and claiming nothing. E last ran `test_stm32_i2c`; D last ran
`twi_peer` (reflash it `spi_peer` before the SPI half). The AVR boards
stay unplugged, and only one SAM board is plugged in at all. What
...
topology probe and touched by nothing else; the SAM board is as
described above (now at 3.3 V). THE RTC DOMAIN IS ON THE
...
| The Nucleo-G0B1RE's self-link (the G0 bus campaigns' step a) | SPI1 PB3/PB4/PB5/PA15 to SPI2 PB10/PC2/PD4/PB12; I2C1 PB8/PB9 to I2C2 PA11/PA12 with 2.2k pull-ups to 3V3 | see "The Nucleo-G0B1RE's self-link" below; verified over SWD. **NOT FITTED TODAY** - the same six G0 pads carry the row below instead |
| The G0-to-SAM bus link (the G0 bus campaigns' step b) | E.PB3/PB4/PB5/PA15 to D.PA17/PA19/PA16/PA18 (SPI); E.PB8/PB9 to D.PA23/PA22 (I2C, the same 2.2k pull-ups) plus a dedicated GND | see "The G0-to-SAM bus link" below; the self-link's six wires MOVED to the SAM end |
...
| `spi_peer` | The INSTRUMENT half of the SPI campaign, ONE protocol and TWO firmwares: the avrdx original (board B or A, SPI0 ALT1) and the samc21 port (board C or D, SERCOM1 function C) - one blocking loop that shifts whatever the DUT clocks, decodes a command frame, acknowledges it and becomes for a bounded moment whatever the DUT needs at the other end. Both are DARK LISTENERS (MISO driven for exactly one answer window, after a frame that checked out) holding the shared select wire up with their own pull-up, and both serve `host_burst` (the roles invert: the instrument clocks a bounded burst so the DUT's client half is testable). The samc21 port serves an exchange through its own DMA ENGINES by default - hardware reload, boundary 6 MHz - and through its polled one-ahead pump when the command's spare bit (`spilink::spare_polled_pump`) asks, so a suite can measure both; its ident label is the die serial's first word, its regimes map onto PLOADEN (the AVR's BUFEN/BUFWR do not exist here), and the AVR-only experiments (wrcol, feed_tx, mspi) answer cfg_failed. Console (observability only): `?` help, `i` status and counters, `0` back to the dark client, `3` command trace |
...
| `twi_peer` | The INSTRUMENT half of the I2C campaign, ONE protocol and TWO firmwares: the avrdx original (TWI0, PA2/PA3) and the samc21 port (SERCOM3 fn C, PA22/PA23, core at 48 MHz - the clean wire's bet, won). A blocking loop around a polled client answering exactly the command address (0x6B): decodes a frame, acks (or naks a bad checksum), then becomes what the DUT needs - a client that stretches by a commanded hold per byte (on the samc21 port the hold is simply the wait before answering: AMATCH/DRDY stretch by construction), NACKs the n-th byte, goes deaf, answers the general call; a stuck client holding SDA from PORT released by counted SCL edges; the bus HOST for a bounded burst (on the samc21 port a MODE SWITCH - one SERCOM is host or client, never the AVR's combined both). The samc21 port's ident label is the die serial's first word; its serving of the command channel's reads is the stratum's first exercised client-transmit path (I2cs::end_transaction was born for its closing NACK). Console (observability only): `?` help, `i` status, `0` back to command mode, `3` trace |
