...
- Firmware today: `test_stm32_tim` (stm32g0). NB from the FLASH
...
position E on a direct USB port, running `test_stm32_tim` (stm32g0),
with `test_stm32_nvm`'s heap blocks and `test_stm32_journal`'s values
both still live in bank 2 - the TIMER suite, like the EXTI one before
it, is wireless and writes no flash, so the storage day's proof stands
untouched; the SAM boards are as described below (now at 3.3 V), with
C's probe temporarily unplugged. NOTHING IS WIRED TO THE NUCLEO:
`test_stm32_tim` moves its own pads (PA5 = LD4, PA6, PA7, PA8) and
`test_stm32_exti` moves PA0, PB0, PB3, PB7 and PA8, so any wire on
those header pins would be the one thing that breaks either.**
