...
position E on a direct USB port, running `test_stm32_spi` (the SPI
campaign's own suite, and the first firmware on this desk to use the
self-link), with `test_stm32_nvm`'s
...
SERIAL and SPI suites all write no flash, so the storage
day's proof stands untouched; PA15 is left an output HIGH (the
deselected chip select) and the other seven self-link pads under their
alternate functions; the SAM boards are as described below (now at
...
in analog; `test_stm32_fdcan` moves PB8 and PB9 ALONE (FDCAN1_RX and
FDCAN1_TX on AF3, the same two pads proven free again by their own pull
and left in analog), and runs FDCAN2 with no pad at all, because
