...
- Firmware today: `test_stm32_dma` (stm32g0). NB from the FLASH
...
position E on a direct USB port, running `test_stm32_dma` (stm32g0),
with `test_stm32_nvm`'s heap blocks and `test_stm32_journal`'s values
both still live in bank 2 - the DMA suite, like the TIMER and EXTI ones
before it, is wireless and writes no flash, so the storage day's proof
stands untouched; the SAM boards are as described below (now at 3.3 V),
with C's probe temporarily unplugged. NOTHING IS WIRED TO THE NUCLEO:
`test_stm32_dma` claims only PA5 (LD4), `test_stm32_tim` moves PA5, PA6,
PA7 and PA8, and `test_stm32_exti` moves PA0, PB0, PB3, PB7 and PA8, so
any wire on those header pins would be the one thing that breaks them.
The DMA suite's letter `u` is the ONE thing on this desk that wants a
host peer, and the peer is `tools/uart_stress.py` on the board's own
virtual COM port - no wire either.**
