...
- **Board D alone carries a 32.768 kHz crystal on PA00/PA01
  ...
  cross-checks.
- Firmware today: C = `spi_peer` (the samc21 port), D = `test_samc_spi`.
...
- Firmware today: `test_stm32_fdcan` (stm32g0). NB from the FLASH
...
**Today's end state (2026-09-07): the Nucleo-G0B1RE is
position E on a direct USB port, running `test_stm32_i2c` (the I2C
campaign's own suite, the second half of the self-link's first step),
with `test_stm32_nvm`'s
...
deselected chip select), the other seven SPI self-link pads under their
alternate functions and the four I2C ones (PB8, PB9, PA11, PA12) at AF6
open-drain, which on a bus with pull-ups is the idle state; the SAM boards are as described below (now at
3.3 V), with C's probe temporarily unplugged. THE RTC DOMAIN IS ON THE
