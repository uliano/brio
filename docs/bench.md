...
**Today's end state: board A runs `test_avr_tcd`, board B runs
`twi_peer`.** The TCD suite is wireless and touches no pin the I2C bus
uses, so board B can stay where it is; `test_avr_twi` goes back on
board A whenever the TWI matrix is wanted (it needs no wire of its own
either - both boards already tap the desk's one I2C bus - and it
commands board B IN BAND over that bus, so the whole matrix runs as a
plain `run A y`; `test_avr_twi z` scores its full 175 with the peer
