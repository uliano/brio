...
| `test_samc_i2c` | **Bench test suite** (keep passing) for the SERCOM in I2C mode - `samc/i2c.hpp`, both register sets - born on the cross-architecture bundle desk (39/39 vs the AVR `twi_peer`, core forced to 6 MHz by the crosstalk wall, the client letter declining) and PROMOTED by the SAM-SAM clean pair: the peer board runs `twi_peer` (avrdx or samc - one `twi_link.hpp`), BOTH cores at 48 MHz, 10 letters / 39 verdicts, 39/39 warm and cold. Letters: the block and the bus state machine with erratum 1.17.16 unreproduced in I2C mode; the command channel (every command TWO tenures of the engine under test); write/read/write-then-read plus the general call, the repeated start seen as two address matches from the client's side; the vocabulary on the wire (a deaf peer's i2c_nack_addr on write AND empty probe, a commanded 3rd-byte i2c_nack_data); commanded clock stretching (2 ms/byte = exactly 16 ms for 8 bytes); THE THREE SPEEDS with FAST-MODE-PLUS ON THE WIRE (25 tenures x 16 bytes in 40/12/6 ms at 100k/400k/1M - the stratum's first Fm+) plus the refused-never-slowed proof on a throwaway 6 MHz core claim; the held wire's parked START with its timeline (and the silicon's refusal to fire it on a phantom release - the fact that also walls off a two-node arbitration race); THE CLIENT ROLE back in its data verdicts (the foreign 100 kHz host's burst byte-exact through the 48 MHz-core client - the exact configuration the bundle deafened); unstick() 0 pulses clean / 4 at the peer's 4th-edge release; THE KERNEL LETTER - I2cBus (= BusMaster) over I2cHost with the NACK delivered in its place, NOT ONE LINE of util/ or kernel/ changed. Findings in [samc/i2c.md](samc/i2c.md) |
