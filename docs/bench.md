...
- Firmware today: `test_stm32_journal` (stm32g0). NB from the FLASH
...
position E on a direct USB port, running `test_stm32_journal`
(stm32g0), with `test_stm32_nvm`'s heap blocks and this suite's
journal values both live in bank 2 and both proven across a reflash of
the other app - the third architecture's storage day; the SAM boards
are as described below (now at 3.3 V), with C's probe temporarily
unplugged.**
