...
- Firmware today: `test_stm32_analog` (stm32g0). NB from the FLASH
...
both still live in bank 2 - the RTC, SLEEP, ANALOG, DMA, TIMER and EXTI
suites are all wireless and write no flash, so the storage day's proof
stands untouched; the SAM boards are as described below (now at 3.3 V),
...
`test_stm32_rtc` and `test_stm32_sleep` move NO pad at all - they arm
