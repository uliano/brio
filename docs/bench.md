...
position E on a direct USB port, running `test_stm32_lptim` (stm32g0,
the first suite of the second tails batch), with `test_stm32_nvm`'s
...
and SERIAL suites are all wireless and write no flash, so the storage
day's proof stands untouched; the SAM boards are as described below (now at
...
NOTHING IS WIRED TO THE NUCLEO, and the analog campaign is the reason
that matters most: PA4 is
