...
- Firmware today: `test_stm32_sleep` (stm32g0). NB from the FLASH
  ...
  erase of bank 2 from firmware, which nothing in the tree does.
...
position E on a direct USB port, running `test_stm32_serial` (stm32g0),
...
in analog - and its host-assisted letter `v` moves the CONSOLE's own
