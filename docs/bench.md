...
both still live in bank 2 - the ANALOG suite, like the DMA, TIMER and
EXTI ones before it, is wireless and writes no flash, so the storage
day's proof stands untouched; the SAM boards are as described below (now
at 3.3 V), with C's probe temporarily unplugged. NOTHING IS WIRED TO THE
NUCLEO, and the analog campaign is the reason that matters most: PA4 is
...
PA7 and PA8, and `test_stm32_exti` moves PA0, PB0, PB3, PB7 and PA8. The
