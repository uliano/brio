...
position E on a direct USB port, running `test_stm32_sleep` (stm32g0),
...
LPTIM and CRC suites are all wireless and write no flash, so the storage
day's proof stands untouched; the SAM boards are as described below (now at 3.3 V),
with C's probe temporarily unplugged. THE RTC DOMAIN IS NOW ON THE LSE
CRYSTAL and its calendar keeps running across every reset and reflash -
`test_stm32_rtc` and `test_stm32_sleep` both leave it there, and
`test_stm32_rtc`'s letter `a` wipes the domain (BDRST) whenever it has
to move the clock select, which costs the five TAMP backup registers.
...
them back in analog; `test_stm32_crc` moves NO PAD AT ALL - a checksum
...
DMA suite's letter `u` is the ONE thing on this desk that wants a host
peer, and the peer is `tools/uart_stress.py` on the board's own virtual
COM port - no wire either. TWO MEASUREMENTS THE ANALOG SUITE MAKES ABOUT
