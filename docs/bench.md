...
- Programmer/debugger: Atmel-ICE over UPDI, AVR port, 6-pin ISP
  ...
  `cli/bench/bench_boards.py` updated if the move becomes standing). The
  probe-to-board pairing is the cable and the console-to-board
  pairing the USB socket, so both are facts about the desk, not
  about the chips: `cli/bench/bench_boards.py` is the truth and the
...
- Identity: the 96-bit UID at 0x1FFF7590 is recorded in
  `cli/bench/bench_boards.py` (no suite letter checks it yet).
...
- Identity: the 96-bit UID at 0x1FFF7590 reads `00700051 4b305010
  20383659` (recorded in `cli/bench/bench_boards.py`), and its FIRST WORD
...
2. **Identity** - `cli/bench/bench_boards.py`, the bench **manifest**: a
