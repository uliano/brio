...
- Desk positions **C** and **D** in the manifest, driven by
  `tools/bench.py` like any other board (`flash`/`run`/`console`);
...
- Firmware today: **only ONE of the two boards is on the desk**, on the
  ...
  `tools/bench_boards.py` records the measurement in a comment and
...
- Identity: the 96-bit UID at 0x1FFF7590 reads `00700051 4b305010
  20383659` (recorded in `tools/bench_boards.py`), and its FIRST WORD
...
peer, and the peer is `tools/uart_stress.py` on the board's own virtual
