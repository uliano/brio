...
- Programmer/debugger: Atmel-ICE over UPDI, AVR port, 6-pin ISP
  ...
  `bench/bench_boards.py` updated if the move becomes standing). The
  probe-to-board pairing is the cable and the console-to-board
  pairing the USB socket, so both are facts about the desk, not
  about the chips: `bench/bench_boards.py` is the truth and the
  USERROW label in each banner is the cross-check.

## The SAM C21 boards

TWO of the user's own C21J rev 1.1 boards (custom KiCad, not the
Xplained Pro): ATSAMC21J18A, silicon rev F (DSU DID 0x11010500, read
over SWD on both). Desk positions **C** and **D**; everything below
...
- Desk positions **C** and **D** in the manifest, driven by
  ...
  to be given by hand.
- **Board D alone carries a 32.768 kHz crystal on PA00/PA01
  (XOSC32K)** - so far unexercised anywhere in this stratum (every
  32 kHz figure to date is an internal RC). It is the future real
...
- Firmware today: **only ONE of the two boards is on the desk**, on the
  ...
  `bench/bench_boards.py` records the measurement in a comment and
...
- Identity: the 96-bit UID at 0x1FFF7590 is recorded in
  `bench/bench_boards.py` (no suite letter checks it yet).
...
The G0 stratum's SECOND SILICON, and the G0B1RE's bus peer: an ST
**Nucleo-G071RB** with an STM32G071RB - Cortex-M0+, 128 KB single-bank
flash, 36 KB SRAM, DBGMCU_IDCODE 0x20006460 (DEV_ID 0x460 = G071/G081,
REV_ID 0x2000; OpenOCD prints "STM32G07/G08xx - Rev B"), FLASHSIZE
0x0080 = 128 KB, all read over SWD before a line of brio ran on it.
Desk position **F**. Everything below was verified at the bench.

- Target voltage **3.24 V**, the same rail class as E, so the two
  boards' pads face each other directly.
- CPU on **HSI16 through the PLL at 64 MHz**: `probe` blinked PA5
  (sampled over SWD alternating 0x00/0x20) with RCC_CFGR reading 0x12,
  SW = SWS = PLLRCLK - the stratum's `Clock<pll, 64 MHz>` on a second
  die, first try. No HSE crystal is fitted and the HSE root is
  unbuilt, so every peer this board runs reports `xtal=none`.
- LED **LD4 on PA5**, button B1 on PC13 (not exercised).
- Console: the ST-LINK's **virtual COM port on USART2, PA2 (TX) / PA3
  (RX), AF1**, 115200 8N1, addressed by `/dev/serial/by-id` (ST-LINK
  serial 066FFF534871754867235316). The `console` app answers on it:
  `STM32G071RB brio console (clk=PLL64, tick=SysTick), type HELP` and
  `HELP` replies with its four commands - the kernel, the ticker, the
  three AOs and the USART transport all alive on the new die.
- Programmer/debugger: the on-board **ST-LINK/V2.1** through OpenOCD's
  `interface/stlink.cfg` + `target/stm32g0x.cfg`, named by the same
  serial. Single-client.
- Identity: the 96-bit UID at 0x1FFF7590 reads `00700051 4b305010
  20383659` (recorded in `bench/bench_boards.py`), and its FIRST WORD
  is what the peers put on the wire as their ident label - so
  `test_stm32_spi`'s and `test_stm32_i2c`'s peer letters name this
  board by `00700051`.
- Its errata sheet is **ES0418**, not the G0B1's ES0548, and IT HAS BEEN
  READ AT THE BENCH: this die is **revision B** (REV_ID 0x2000; Y is
  0x2002), and every item of the sheet a letter can reach is answered in
  [stm32g0/vendor/README.md](stm32g0/vendor/README.md)'s "The second
  silicon" table. The one that bites is **2.2.4**, the EXTI-to-DMAMUX
  trigger routing, which costs `test_stm32_serial` four verdicts.
- **THE LSE CRYSTAL IS FITTED AND RUNS.** Its RTC domain was found EMPTY
  (RCC_BDCR 0x0000 0000 - LSEON clear, not merely unselected), and
  `test_stm32_rtc`'s letter `a` claimed the crystal with the ordinary
  one-way RTCSEL write: LSERDY rose inside the driver's bounded wait on
  the first try, and the crystal measures **32736 Hz** against the core
  (E's reads 32703 - both of them the core's own 1 % trim). LSI measures
  **32295 Hz** through the watchdog and 32323 Hz through a TIM16 capture
  (E: 32536 and 32586).
- No second flash bank, so no storage attic: the NvHeap and NvJournal
  backends answer bad_geometry here and the two storage suites stay
  the G0B1RE's alone.
- **FOURTEEN OF THE SEVENTEEN BENCH SUITES RUN ON THIS BOARD**, each of
  ...
  [stm32g0/README.md](stm32g0/README.md)'s "The second silicon". The two
  bus suites run with E as their peer, the roles of the link exchanged.
- Firmware today: `blink`.
- The board number was never verified, so no MB number and no morpho
...
- **THE DEBUG PORT CAN GO SILENT, AND ONLY UNPLUGGING THE BOARD BRINGS
  ...
  `bin/brio`'s programmer kind **`stlink_msd`** drops the
...
2. **Identity** - `bench/bench_boards.py`, the bench **manifest**: a
...
avrdude over UPDI; `c21j` means a SAM C21 built by `samc21/` and written
...
`build-cmake/apps_samc21.json`) and the board type picks which to read:
...
brio list                 # devices, probes, manifest (console present?)
brio flash A test_avr_pin # cmake --build --target <app> + avrdude over UPDI
brio flash C test_samc_dma # ... or + OpenOCD over SWD, by board type
brio run C z              # drive the console, judge "ALL: N pass, M fail"
brio console A            # print device path + speed (attach a monitor)
brio duo A:a B:script.txt # instrument peer scripted, then the DUT
brio fuses A              # read the fuses; name=value pairs write them
brio fuses C              # ... the SAM's NVM User Row, same verb
```
...
the same budget: native + `brio check avrdx` at every change
...
```bash
$PY brio fuses A                        # read, with the geometry spelt out
$PY brio fuses A bootsize=128 codesize=0
```
...
```bash
$PY brio fuses C                        # read, every field decoded
$PY brio fuses C bodvdd_hysteresis=1    # write, then reset, then read back
$PY brio fuses C --rewrite              # write the row back unchanged
```
...
are the same fields `samc21/nvm.hpp`'s `NvmUserRow` reads from the
firmware side, which is where their meanings are documented
([samc21/nvm.md](samc21/nvm.md), [samc21/reset.md](samc21/reset.md),
[samc21/supc.md](samc21/supc.md)); the two must move together.
...
bus suites run their peer halves instead - and they do it **in EITHER
direction**, whichever board hosts: `test_stm32_i2c` z 54/54 and
`test_stm32_spi` z 38/38, twice each including a run from a cold flash,
on E with F as the peer AND on F with E as the peer, with the self-link
letters skipping themselves after a probe and claiming nothing. (The
peer's MISO pad runs at HIGH speed - at the driver's very-high the
falling-edge modes of letter `o` slipped in one burst of eight, see
stm32g0/spi.md; letter `x` with F hosting found 0 of 40 bursts slipped.)
FOURTEEN of the seventeen suites now build for both boards
(`// build: boards = g0b1re,g071rb`) and are measured on both -
[stm32g0/README.md](stm32g0/README.md)'s "The second silicon" carries
the table of scores and of what skips where. **BOTH BOARDS ARE LEFT ON `blink`**, which touches PA5 and nothing else
and leaves every shared pad at its reset state; before that F last ran
`test_stm32_crc` and E `test_stm32_serial` (the boot-banner captures),
and earlier in the day F's `test_stm32_rtc` letter `w` spent its five
backup registers by design. The AVR boards and both SAM boards stay unplugged. What
follows is the storage day's state, which none of this touched: E sits
on a direct USB port with `test_stm32_nvm`'s
heap blocks and `test_stm32_journal`'s values both still live in bank 2
- the RTC, SLEEP, ANALOG, DMA, TIMER, EXTI, LPTIM, TICKLESS, CLOCK, CRC,
SERIAL, SPI and I2C suites all write no flash, so the storage
day's proof stands untouched; PA15 is left an output HIGH (the
deselected chip select), PB3, PB4 and PB5 under their alternate
functions and PB8/PB9 at AF6 open-drain, which on a bus with pull-ups
is the idle state, while the six self-link pads at the far end
(PB10, PC2, PD4, PB12, PA11 and PA12) are left in analog by the
topology probe and touched by nothing else; F is as described in its own
section above, and the peer it runs leaves PB3/PB4/PB5 and PB8/PB9 under
their alternate functions with PA15 an input under its own pull-up. THE
RTC DOMAIN IS ON THE
LSE CRYSTAL AGAIN: the desk found it EMPTY on 2026-09-05 (RCC_BDCR
0x0003 - the board had been unplugged), and `test_stm32_tickless`
selected the crystal the way `test_stm32_serial` does; the calendar
keeps running across every reset and reflash - `test_stm32_rtc` and
`test_stm32_sleep` both leave it there, and `test_stm32_rtc`'s letter
`a` wipes the domain (BDRST) whenever it has to move the clock select,
which costs the five TAMP backup registers. **A DOMAIN THAT HAS BEEN POWER-CYCLED COMES UP
UNSELECTED, NOT WRONG**: this board's VBAT is tied to VDD, so unplugging
it clears RCC_BDCR to 0x0003 (LSE running, RTCSEL = 00, RTCEN clear),
and choosing LSE from there is the ordinary one-way write of 5.4.23 and
NOT a BDRST. `test_stm32_serial` does exactly that at boot - it claims
the crystal when the domain is on LSE already OR has no clock at all,
and sets the calendar only when INITS says it was never set - which is
how the crystal came back after the day the desk lost power. A suite
that finds a domain latched onto something ELSE reports it and its
sleeping letters decline.
**AND THE DOMAIN'S MOVE TO LSE TOOK LSI DOWN WITH IT, which is a DESK
fact and not a driver one.** 5.4.24: LSIRDY stands whenever the IWDG,
the RTC or the LSE clock-security system asks, whatever LSION says - and
while the RTC ran on LSI (RCC_BDCR 0x8200) every program on this board
got a running LSI for free. With the domain on LSE (0x8103) nothing asks
any more, so LSI is OFF unless a program starts it or arms the IWDG.
`test_stm32_tim` starts it explicitly and is unaffected;
**`test_stm32_dma`'s letter `i` does NOT, and it captures LSI through
TIM16_TISEL** - so that letter reads 0 blocks drained and its `z` scores
53 of 54 on a board whose last reset came from a flash. PROVEN, not
guessed: setting RCC_CSR.LSION by hand over SWD (`mmw 0x40021060 1 0`)
makes the same image score 54/54 with four blocks of 32 captures 30 us
apart. The one-line repair - `Rcc::lsi_enable(true)` +
`lsi_wait_ready()` in that letter - was applied at the campaign's
review AFTER the byte-identity gate had been run with the DMA image
untouched, so it is the serial commit's one declared mover (z 54/54
again on the fixed image).
**THE SELF-LINK IS WIRED NOW** (the table above), and the campaigns that
predate it are unaffected because none of them owns both ends of any
wire - with ONE MEASURED EXCEPTION, which is the I2C pair and not the
SPI one: **the 2.2 kOhm pull-up on PB9 beats an internal pull-down**, so
`test_stm32_analog`'s DAC letter, which walked PB9 as the EXTI line 9
trigger on the precondition that the pad follows its own pull, scored
137 of 139 the day the wires arrived. Proven over SWD with a control:
PB9 reads 1 under an internal pull-down where PB3, an SPI wire end with
no external pull-up, reads 0 under the same. THE LETTER NOW WALKS PC9
(the same EXTI line, a pad no wire touches) and the suite is 139/139
again; the rule it teaches is that a pad with an external pull-up is no
longer a pad a suite can walk, and the wiring table above is the list.
The SPI wires cost nothing (`test_stm32_exti` 89/89 and `test_stm32_lptim`
82/82 after the wiring, PB3 and PB5 being wire ends of theirs). Otherwise NOTHING ELSE IS WIRED TO THE NUCLEO, and the
analog campaign is the reason that matters most: PA4 is
DAC1_OUT1 AND ADC_IN4 on one bond pad, so the two converters meet inside
the package, and a wire on that header pin would put a second source on
the node. `test_stm32_analog` moves PA0, PA1, PA4 and PA5;
`test_stm32_dma` claims only PA5 (LD4), `test_stm32_tim` moves PA5, PA6,
PA7 and PA8 and, since letter `l`, PB6, PB7, PB8, PB9, PB13, PB14 and
PB15 (TIM4's four channels at AF9, TIM15's three outputs at AF5 and
TIM17's pair at AF2, each pull-walked first and left as a plain input),
and `test_stm32_exti` moves PA0, PB0, PB3, PB7 and PA8;
`test_stm32_lptim` moves PB0, PB5, PB6 and PB7 (LPTIM1's four signals on
AF5), PC0, PC3 and PD6 (LPTIM2's three on AF2, letter `j`) and PA1 (the
comparator's precharged input), leaving every one of them back in
analog; **BOTH `test_stm32_tim` and `test_stm32_exti` now spend
SYSCFG's UCPD1 dead-battery strobe** before they touch PA8 (and, in the
tim suite's letter `l`, PB15), which is what makes those pads follow
their own pull; `test_stm32_clock` puts PA9 under USART1 as an
open-drain single wire with its pull-up and leaves it there (the
serial suite's loop-back pad); `test_stm32_tickless` moves NO PAD AT ALL (both
LPTIMs count with no pin, TIM2 is a free-running counter); `test_stm32_serial` moves PA9 and PA10 (USART1's TX
and RX on AF1 - the single-wire loop and the bit-banged line), PB3 and
PB4 (USART1_RTS_DE_CK and USART1_CTS on AF4 - and PB3 is ONE PAD WITH
THREE JOBS: RTS, the RS-485 driver enable and the synchronous clock),
PB9 (IR_OUT on AF0), PC1 (LPUART1_TX on AF1) and PC6 (LPUART2_TX on
AF3), each proven free by its own pull first and every one of them left
in analog; `test_stm32_fdcan` moves PC4 and PC5 ALONE (FDCAN1_RX and
FDCAN1_TX on AF3 - PB8/PB9 until the I2C self-link's pull-ups arrived -
proven free by their own pull and left in analog), and runs FDCAN2
with no pad at all, because
internal loop-back needs none - and its host-assisted letter `v` moves the CONSOLE's own
PA2/PA3 onto AF6 for two legs and puts them back;
**BOTH BUS SUITES ALSO DRIVE THE FAR END OF THE SELF-LINK FOR A FEW
HUNDRED MICROSECONDS AT BOOT**, which is the topology probe: each host
pad is driven both ways against the far pad's own internal pull and
every one of them is left in analog afterwards, so a suite knows which
of the two wirings it is on instead of assuming.
`test_stm32_spi` moves the self-link's EIGHT pads and no others - PB3,
PB4, PB5 and PA15 at the host end (SPI1's four signals at AF0, and PA15
is a plain GPIO chip select for every letter but the three that measure
the hardware NSS arrangements and the one that runs I2S1's word select),
PB10 (AF5), PC2 (AF1), PD4 (AF1) and PB12 (AF0) at the client end - plus
PB4 and PC2 as I2S1_MCK and I2S2_MCK for one leg of the I2S letter, on
the same MISO wire; it leaves PA15 an output HIGH (the deselected chip
select) and the rest under their alternate functions;
`test_stm32_i2c` moves the I2C self-link's FOUR pads and no others - PB8
and PB9 at the host end and PA11 and PA12 at the client end, all AF6
open-drain, plus the same four briefly as open-drain GPIO for
`unstick()`'s nine clocks; it writes no flash and does not reset the RTC
domain, and its letter `j` deliberately does NOT enter Stop (the reason
is in [stm32g0/i2c.md](stm32g0/i2c.md): both ends of this bus stop
together);
`test_stm32_crc` moves NO PAD AT ALL - a checksum
unit has no pin - and `test_stm32_rtc` and `test_stm32_sleep` move none
either - they arm
WKUP1 (PA0) for one verdict and disarm it again, and set a Standby pull
on PA0 with APC never applied. The
DMA suite's letters `u` and `w` and the SERIAL suite's letters `y`, `w`
and `v` are the things on this desk that want a host
peer, and the peer is `brio stress` on the board's own virtual
COM port - no wire either. The serial suite added one op to that
protocol, `poke`: the script waits out HALF the announced window, sends
a handful of bytes and waits out the rest, which is what a board that is
ASLEEP in the middle of its own window needs. It also taught the board a
rule the sink legs had never needed: the script's collect phase runs AT
THE OP'S OWN RATE after the pump and only ends on 300 ms of silence, so
a board that wakes early and reports at once gets its report read at
9600 and eaten - every host leg now waits out its whole announced window
plus 900 ms before speaking. **THE DMA SUITE'S LETTER `u` USED TO LOSE
ITS TALLY, and the repair was to stop pumping past the bridge's
ceiling** (2026-09-07). Its ladder now ends at 921600 - the rate this
VCP is measured to carry byte-exact in both directions - so `u` is six
legs, and the two rungs above it moved into a new letter `w`, outside
`z` and outside every verdict. `uart_stress.py` declines a leg
announced above 921600 the same way, running it PASSIVELY (the port
follows the rate and drains, nothing is pumped) unless it is given
`--beyond-vcp`; that ceiling is its default only for the ST-LINK port,
because the CH340s on the other boards carry 3 Mbaud and are measured to
(`test_samc_uart` letter `k`). The mechanism was never a slow letter: a
pump the wire will not take is delivered by the host's operating system
AFTER the board is home at 115200, where the console's ring reads it as
menu LETTERS and eats the closing "-> N pass" line with it. Measured
after the change: three consecutive runs of `u`, six legs reporting
each time and the tally read every time. TWO MEASUREMENTS THE ANALOG SUITE MAKES ABOUT
THIS BOARD, both worth knowing before anything is jumpered: PA5 carries
LD4 and STILL follows a 40 kohm internal pull between the rails, and the
lamp's path is NOT a load the DAC's buffer can feel (the two DAC
channels agree to 5 mV at every code) - so whatever drives LD4 here is
high impedance, not a diode with a series resistor to ground. And VREF+
sits at VDDA as far as anything inside the chip can see, which is why
the VREFBUF is read and never enabled.**
...
through its probe and watching which port emits traffic). Firmware:
C = `twi_peer` (samc21), D = `test_samc_i2c` - the I2C campaign's
positions (the SPI pair spi_peer/test_samc_spi flashes back in a
minute when that bench is wanted; the wires for both stay fitted). D
is the DUT because C's probe link is the flakier and the peer needs
flashing least often. BOTH AVR BOARDS ARE UNPLUGGED -
A last ran `twi_peer`, B `test_avr_opamp` with the five live `NvHeap`
blocks still in flash (the OPAMP campaign reflashed B five times over
them and `test_avr_nvheap v` found all five EXACT afterwards - the
page-selective default erase doing what the table above says). Every
A-B and A-C two-board half is therefore idle and needs its wiring and
peer firmware back; so are the two open AVR-side questions phase F
left (the spi_peer select-wait wedge, `test_avr_spi y` after the
run_exchange edit). Reflashing B is what costs the heap its blocks:
the default page-selective erase keeps them, a `--erase` does not.

**Wires fitted today: the five-wire SAM-SAM SPI link, board C to
board D, STRAIGHT THROUGH** (verified conductor by conductor over SWD
alone - both ends halted, each pin driven high from one side against
pull-downs and read on the other, only its own bit moving - the
technique is in the session memory, here with no UPDI half at all):

| C | signal | D |
|---|---|---|
| PA16 = SERCOM1/PAD[0] | MOSI | PA16 |
| PA17 = SERCOM1/PAD[1] | SCK | PA17 |
| PA18 = SERCOM1/PAD[2] | SS/CS | PA18 |
| PA19 = SERCOM1/PAD[3] | MISO | PA19 |
| GND | reference | GND |

Both boards SERCOM1 function C - deliberately symmetric: the same
four wires carry either board as host (DOPO row 0x0) or client (row
0x2), the fact test_samc_spi proved on the A-C bench, so roles swap
without rewiring.

**And the I2C pair (2026-09-02, its own short separated wires - NOT
part of the SPI run):**

| C | signal | D |
|---|---|---|
| PA22 = SERCOM3/PAD[0] | SDA | PA22 |
| PA23 = SERCOM3/PAD[1] | SCL | PA23 |

with EXTERNAL PULL-UPS fitted on the node (verified present and
stronger than a C21 internal pull-down by the SWD wire check, which
also proved continuity and isolation open-drain style before any
firmware touched the pins). Keeping this pair OUT of any bundle is
what the filterless-I2C finding demands, and the clean pair is the
measured proof it suffices: both cores at 48 MHz, Fm+ on the wire
([samc21/i2c.md](samc21/i2c.md)'s headline, second half).
...
**The desk runs at 5 V** (VDDIO2 from the same rail), which is what
the protocol suites need. The rows below are the wirings the reference
suites and their peers ask for; each one says whether it is fitted.
The breadboard experiments that once shared this board (a display bus,
a DAC-to-ADC loop, a traffic-light testbed) are dismantled and live
with their own wiring under `experiments/`.
...
ends of any wire, so their letters are unaffected BY THE WIRE - but two
of them are affected BY THE PULL-UPS, which is a different thing and the
rule the SPI campaign already learned on PB9 alone:

**A PAD WITH AN EXTERNAL PULL-UP IS NO LONGER A PAD A SUITE CAN WALK.**
2.2 kOhm beats the port's own ~40 kOhm both ways, so PB8 and PB9 read 1
under an internal pull-down as readily as under a pull-up, and any
letter whose precondition is "this pad follows its own pull" fails on
them. Measured after the I2C campaign, with the images proven
byte-identical to the ones that passed before the wires (so it is the
desk and not the code):

- `test_stm32_fdcan` scored 94/96 - its letter `a` precondition (PB8
  and PB9 were FDCAN1_RX/TX) and its stuck-dominant letter, which pulls
  RX down internally to make the bus dominant and could not any more:
  THE SUITE NOW LIVES ON PC4/PC5 (FDCAN1_RX/TX on AF3, the Arduino
  D1/D0, pads no wire touches; its edge counter on EXTI line 5) and is
  96/96 again;
- `test_stm32_tim` scored 117/118 - letter `l`'s pull-walk over the
  seven port-B pads reported `PB8 0 PB9 0` where the other five follow
  their own pull: THE LETTER NOW JUDGES PB8/PB9 DRIVABLE (held up by
  the external pull and sinking when driven, which is all a PWM output
  needs - TIM4's channels are on PB6..PB9 alone) and is 118/118 again.

Both were the pull-ups and nothing else, proven with the images
byte-identical to the ones that had passed. The SPI half of the link
costs nothing (`test_stm32_exti` 89/89, `test_stm32_lptim` 82/82,
`test_stm32_spi` 89/89, `test_stm32_analog` 139/139 once its DAC letter
moved to PC9).

### The G0-to-SAM bus link

STEP (b) OF THE G0 BUS CAMPAIGNS: the same six G0B1 pads the self-link
used, moved to a SAM C21 so that both bus drivers meet an INDEPENDENT
SECOND CHIP instead of a second peripheral of their own die. Both boards
run at 3.3 V; the two 2.2 kOhm pull-ups fitted for the I2C self-link
serve this bus unchanged, and a dedicated GND wire ties the two boards.

**NOT FITTED TODAY** - the six wires have moved on to the G0-to-G0 link
below, and the SAM board is not plugged in at all. The wiring is
recorded because the pads are the same six and a SAM peer is one
re-jumpering away; `spi_peer` and `twi_peer` exist for that end too, and
the suites ask the peer which one answered.

| Signal | G0B1RE (position E) | SAM C21 (position D) |
|---|---|---|
...
The instrument is `spi_peer` or `twi_peer` on the SAM (one board, one at
a time - the two peers use different SERCOMs but the same console and
the same flash), commanded IN BAND over the very bus under test through
`avrdx/src/apps/spi_link.hpp` and `avrdx/src/apps/twi_link.hpp`. Those
two headers are now compiled by THREE architectures' apps and copied by
none.

**THE SELF-LINK AND THIS LINK EXCLUDE EACH OTHER**, because the six G0
pads are the same six. Neither suite assumes which one is on the desk:
`test_stm32_spi` and `test_stm32_i2c` PROBE for the self-link at boot -
each host pad driven both ways against the far self-link pad's own
internal pull, exactly the wiring-check technique, in firmware this time
- and print the answer in their banner. Then EACH SET OF LETTERS SKIPS
ITSELF ON THE OTHER DESK, with the reason printed and no verdict claimed
either way (the `test_avr_serial` letter `q` treatment of a wiring its
desk does not have): the self-link letters when the probe says no, the
peer letters when it says yes. The one thing that still FAILS LOUDLY is
a peer that does not answer with the wires in place - that is firmware
to flash, not a desk the suite was not built for.

Scores with THIS wiring: `test_stm32_spi` z = **38/38** (letters `a` and
`m` wireless, `n`..`r` on the wire; `b`..`l` skipped) and
`test_stm32_i2c` z = **54/54** (letters `a` and `m` wireless, `n`..`r`
on the wire; `b`..`l`, `x` and `y` skipped). With the self-link back the
peer letters skip in their turn and the score returns to the campaign's
89/89 and 142/142 - by construction, since nothing but the skip changed
in those letters; that half has not been re-run since the wires moved,
and the day it is, this line becomes a measurement instead of an
arithmetic claim.
...
self-link at boot - each host pad driven both ways against the far
self-link pad's own internal pull, in firmware - and print the answer in
...
crystal), LUT1 PC0..PC3, LUT2 PD0..PD3, LUT3 PF0..PF3; TCB0/TCB1 ALT1
sit on PF4/PF5 = the console USART2.

## Apps

Three kinds of app share `src/apps/`: demos and bench tools (`blink`,
`events0`, `xtal_probe`, ...), disposable; the **peers** (`spi_peer`,
`twi_peer`, ...), the far end of a two-board suite; and the **bench
test suites** named `test_<target>_<subject>` (`test_avr_analog`), which
are the reference tests of the drivers they cover and must keep passing
through every restructuring. Breadboard experiments that were assembled
once live under `experiments/`, each with its own README, and are not
listed here.
...
| `i2c_scan` | I2C stack test: address sweep 0x08..0x77 every 2 s on TWI0 through I2cBus + the I2cHost<0> engine, ACKs printed as found |
...
| `test_stm32_platform` | **Bench test suite** (keep passing) for the STM32G0's platform and its failing half - `stm32g0/reset.hpp` + `stm32g0/delay.hpp` over `stm32g0/platform.hpp` - 6 letters in `z` / 53 verdicts, 53/53, NOTHING TO WIRE (the console is the Nucleo's own ST-LINK VCP and every clock it measures is inside the chip). The PRIMASK critical section with its nesting, the idle hook returning on the tick, the SysTick VAL arithmetic `delay.hpp` is built on checked against the interrupt count, and `delay_us` at least-never-early with its one-tick cap. Then the two watchdogs WITHOUT EVER STARTING EITHER: chapter 28's keyed registers with the start key never written - where the finding is that an update never completes until it IS written (5.2.14, and no part of ch. 28 says so) - and the whole of chapter 29's timing with WDGA clear, since the counter free-runs and EWIF rises anyway, though the INTERRUPT does not. Letter `i` sits OUTSIDE `z` because it reboots the board six times: a software reset, an IWDG time-out that measures LSI at 32536 Hz and then proves the reset STOPPED the watchdog (28.3.1's "except upon a reset" is literal, against the family lore), a WWDG window violation, a panic through `ResetReporter`, a deliberate HardFault, and a WWDG time-out that carries the early-wakeup interrupt's only hardware proof - 26/26. Run it with `bench.py run E i --app test_stm32_platform --expect="pass," --timeout 200`. Findings in [stm32g0/reset.md](stm32g0/reset.md) and [stm32g0/platform.md](stm32g0/platform.md)  **On the second silicon**: 53/53 in `z` and 26/26 for letter `i` on the Nucleo-G071RB, where the IWDG times LSI at 32295 Hz. |
| `test_stm32_nvm` | **Bench test suite** (keep passing) for the STM32G0's FLASH - `stm32g0/flash.hpp` (RM0444 ch. 3) and `util/nv_heap.hpp` over `stm32g0/nvm_flash.hpp` - 10 letters in `z` / 85 verdicts, 85/85 four times including a cold run, NOTHING TO WIRE. The geometry against the size register (512 KB, two banks of 128 pages) with the bank mapping and `nSWAP_BANK`'s one consequence - an erase is bound to the PHYSICAL bank - and the linker's own boundary read back as `__brio_rom_end`; the option bytes decoded and printed and NEVER written; the keyed unlock with no wrong key ever produced; a page erase at 22039 us and a double word at 88 us, both inside DS13560 table 48; **EOP proven not to rise with EOPIE clear**, which settles 3.7.4 against 3.3.8's own step 7; **read-while-write measured** (174260 turns of a bank-1 polling loop completed inside one bank-2 erase); fast programming at **683 us a row against the datasheet's 1.7 ms typical**; the FLASH interrupt through the NVIC; and the heap's mount/alloc/seal/find/rewrite on the third silicon. A `z` run costs 15 page erases against DS13560 table 49's 10 kcycle budget, which letter `w` prints. Letter `s` sits OUTSIDE `z` because it reboots the board four times, one per malformed program of 3.3.8 - 12/12 - and it is where the campaign's sharpest finding lives: a misstep that sends a WHOLE double word's worth of accesses raises its flag and is over, while one that sends a legal first word and no second raises NOTHING and WEDGES CFGBSY until the next system reset. Letter `v` (5/5, outside `z`) verifies the blocks after a reflash. Findings in [stm32g0/nvm.md](stm32g0/nvm.md) |
| `test_stm32_journal` | **Bench test suite** (keep passing) for `util/nv_journal.hpp` in the STM32G0's bank-2 attic - 7 letters in `z` / 52 verdicts, 52/52 four times including a cold run, NOTHING TO WIRE. The journal's THIRD silicon and the first at the 2048/8 geometry the host suite `test_nv_journal` has been sweeping all along because it is this part's. A read-only mount in 5.3 ms; the byte core, the typed twin, latest-seq-wins and the three refusals; the halves ping-ponging on real flash (a collection is exactly two page erases); a save of 16 bytes at 420 us and NO erase, a collection at 46.3 ms for six ids, `save_reserved()` at 629 us and zero erases - and the reserve intact after every one of eighty consecutive saves; the NO-STALL claim measured (176009 bank-1 turns inside the collection's erase), which is what makes an ordinary save legal from the main loop here; and COEXISTENCE - a heap block in the lower pages byte-exact while the journal collects twice above it. Letter `p` sits OUTSIDE `z` (11/11, it reboots): a real panic whose record crosses the reset IN FLASH - and the finding is WHICH PATH writes it, since with C_DEBUGEN cleared panic()'s BKPT escalates into HardFault before the Reporter is ever called, so the fault body is what carries it. Letter `v` (5/5) verifies the values after a reflash. Findings in [stm32g0/nvm.md](stm32g0/nvm.md) |
| `test_stm32_exti` | **Bench test suite** (keep passing) for the STM32G0's EXTI - `stm32g0/exti.hpp` (RM0444 ch. 13), the pin senses `stm32g0/pin.hpp` deliberately does not have - 9 letters in `z` / 89 verdicts, 89/89 FIVE times including a cold run, NOTHING TO WIRE and nothing written to flash. The stimulus is measured before anything rests on it (letter b): five pads proven to follow their own internal pull - PA8 only after the letter spends SYSCFG's UCPD1 dead-battery strobe, which is what its once-flaky leg had been meeting - and then the fact that makes this family's wireless bench easier than the SAM's - **the EXTI sees a pad its owner is DRIVING**, because the multiplexer selects a PORT and not a pin function and the input buffer stays live in output mode, where analog mode is the one that blinds a line. Then the four senses counted exactly (8 up + 8 down edges -> 8/0, 0/8, 8/8, 0/0 in the two SEPARATE pending registers), the software trigger through the real vector on a line with no trigger edge at all, the multiplexer proven to be the one-pin-per-line rule (PA0's edge is the line's and PB0's is invisible, and swapping EXTICR swaps the answers exactly), the three shared vectors each dispatching only its own lines - with the confinement STAGED (EXTI0_1 ran while line 7 stood pending, saw the bit and left it), the masks (with `EXTI_IMR` clear eight edges leave NO pending bit, so 13.3.1 is literal and a line cannot be polled without arming it) and an **EXTI EVENT returning the core from WFE in 0 us against 798 us for the masked control**, with nothing to acknowledge. Letter `h` puts `ExtInt` inside a REAL KERNEL: twelve edges, twelve events received by an active object, zero dropped. Letter `u` reads the user button - held HIGH by an external pull-up, so a press is a FALLING edge - and judges only what the board decides. Findings in [stm32g0/exti.md](stm32g0/exti.md)  **On the second silicon**: 89/89 on the Nucleo-G071RB, whose IMR1 comes up holding 13.5.12's PRINTED 0xFFF80000 against an implemented mask of 0xFEAFFFFF. |
| `test_stm32_tim` | **Bench test suite** (keep passing) for the STM32G0's TIMERS - `stm32g0/tim.hpp` (RM0444 ch. 21..25) and, through them, `util/pwm_channel.hpp` and `util/meter_sampler.hpp` on their THIRD silicon - 12 letters in `z` / 118 verdicts, 118/118 FIVE times including two from a cold flash, NOTHING TO WIRE and nothing written to flash. Four wireless techniques carry it: **a timer counting a timer** (TIM2 publishes on TRGO, TIM3 takes it as its own clock over ITR1 - 6400 updates in 6400156 CPU cycles, so an edge-aligned period is exactly ARR + 1 - or as a GATE when TRGO is OC1REF, which measures a DUTY CYCLE inside the chip: 250/500/750 per mille asked came back 250/500/749, the same numbers the pad's own sampling gives from a mechanism sharing nothing with it); **a pad read while a peripheral drives it** (LD4's PWM sampled through GPIOA's IDR at 0/250/500/750/1000 per mille, and its edges counted by an EXTI line - which extends the EXTI campaign's finding from output mode to ALTERNATE-FUNCTION mode); **a capture with no pad anywhere** (TIM16's TISEL selects LSI, 1964-tick intervals putting LSI at 32586 Hz against the watchdog's own 32536 Hz, the capture prescaler multiplying exactly at /2, /4, /8); and **a capture pad walked by its own pull** for PWM input mode (period 1012 / width 305 ticks against 1010 / 305 us on the cycle stopwatch, and the counter reset ON the edge so a period reads as its own tick count and not one less - the opposite of the samc21 TC). Also: a centre-aligned period measured to be **2 x ARR and not 2 x (ARR + 1)** (1998.1 ticks, the two candidates two ticks apart and told apart); TIM1's complementary pair **never both high in 60000 readings of one instant**, with a 2000 ns measured dead time against the 2000 ns DTG asks for; MOE, a software break, a BREAK INPUT ON A PAD (TIM1_AF1 comes up with BKINE set) and AOE putting MOE back by itself; the repetition counter dividing 801 updates to 201; two timers on ONE vector and TIM1 alone on two; `TIMx_SR` proven `rc_w0`; and letter `j`, **MeterSampler inside a real kernel** - 32535 capture interrupts, 9 samples published, 9 received, and published + missed + leftover accounting for every store exactly. ES0548 2.7.2 staged eight times WITH A CONTROL and **did not reproduce**. TWO METHOD LESSONS PAID FOR: a sampling loop must not BRANCH on what it reads (a branch makes its duration depend on the state and biased a 50 % pair to 566/383 per mille), and a polling capture loop must have the CONSOLE DRAINED first (a transmit interrupt makes it miss an edge, which reads as an interval that is a MULTIPLE of the true one). Letter `l` closes the chapter's instance list: **the six timers this suite had never counted** - TIM4, TIM6, TIM7, TIM14, TIM15 and TIM17 - each counting PCLK at its prescaler to 0 per mille against SysTick, each update reaching the vector the RESERVE derives from what shares it (lines 16, 17, 18, 19, 20 and 22, four of them shared with a timer, the DAC, an LPTIM or an FDCAN line - a wrong name there is a silent `Default_Handler` spin), and a PWM off a pad for the four that have channels: TIM4's FOUR at once on PB6..PB9 at AF9 (200/400/601/801 per mille asked 200/400/600/800), TIM14's one on **PA7 at AF4, the very pad letter `g` drives as TIM1_CH1N at AF2** - one pad, two timers, the AF number choosing - and TIM15's and TIM17's complementary pairs on port B, never both high in 40000 samples of one instant, **each half at its own duty LESS the dead time exactly** (64 DTG ticks = 64 per mille off each: 236 and 635 for a duty of 300, 385 and 487 for 450). TIM6 and TIM7 are what 23.2 says and no more - `CR2.MMS` accepted, no channel, no slave controller, no compare register - and their TRGO's only consumers here are the DAC and the ADC, which `test_stm32_analog` measures. Findings in [stm32g0/tim.md](stm32g0/tim.md)  **On the second silicon**: 114/118 on the Nucleo-G071RB, which has no TIM4 - so TIM3 owns its vector and letter `l` is a census of five. |
| `test_stm32_rtc` | **Bench test suite** (keep passing) for the STM32G0's RTC DOMAIN - `stm32g0/rtc.hpp` (RM0444 ch. 30, ch. 31's backup registers, and the RCC_BDCR half of ch. 5 that only this domain can reach) - 14 letters in `z` / 125 verdicts, **125/125 three consecutive times including a cold run from a fresh flash**, NOTHING TO WIRE and nothing written to flash. THE INSTRUMENT IS THE POINT: TIM16's input multiplexer reaches LSI, LSE and the RTC's own wake-up signal (25.6.18), so a 64 MHz capture channel weighs all three against the core with no pad and no reference. Findings: **THE NUCLEO'S X2 CRYSTAL IS FITTED AND RUNS** (32703 Hz measured, 1983 ppm off nominal - which is HSI16's own 1 % trim and not the crystal's), closing the one board fact the bring-up left unverified; **LSI at 32586 Hz**, confirming from a period capture what `test_stm32_platform` derived from an IWDG time-out (32536 Hz) by a wholly different route; **an unfiltered capture of an internal clock line is not a measurement, AND ITS ERROR IS TWO-SIDED** - a missed edge lengthens an interval (seen at 62188 ticks) and an OVER-capture shortens one, the bare leg's shortest sample sitting 4, 5, 49, 353 and 754 ticks below the filtered median over five runs, so neither a mean nor a MINIMUM estimates this period and only the median does; that was campaign 10's fix for a letter `c` that failed about two runs in three inside `z` and passed alone, and it is the capture path and not ES0548 2.2.1; **what TISEL calls "RTC wake-up" is the masked interrupt LINE and not a pulse**, so a late acknowledgement deletes the next interval - serviced through the RTC's own vector the four divided clocks come out exact to 2100 ppm; **THE SMOOTH CALIBRATION DOES NOT REACH THE DIVIDED RTCCLK WAKE-UP CLOCKS**, 1022 ppm of swing on ck_spre where 30.3.13 promises 975 and EXACTLY ZERO on RTCCLK/16; an alarm landing AT its match and not a period after it (the SAM's counter compare does the opposite); the calendar's every boundary exact in one second each, with 29 February present in a leap year and absent in a common one; ADD1H/SUB1H effective in the NEXT second (30.6.7, and a suite that reads straight back reports a working register as broken); the two locks measured to have DIFFERENT REACH (DBP covers the backup registers, the RTC's own 0xCA/0x53 key does not); and ES0548 2.9.1 staged sixteen times unguarded and NOT reproduced, recorded as such. Letter `v` sits OUTSIDE `z` because it reboots the board: five backup words and the calendar survive a software reset bit for bit (4/4). Campaign 10b's four new letters close the chapter's own tail. **RTC_REFIN (letter `k`) drags the calendar onto an outside reference** built by the CORE on a GPIO-driven PB15: a calendar second measures 64131086 TIM2 ticks on the crystal alone, 64006056 against a 50 Hz reference whose fifty periods are 64000000 by construction, 64004301 against a 60 Hz one - and the CONTROL is the strongest part, a HALTED reference giving the crystal's second back with REFCKON still set, so the correction is an EDGE and not a mode. The correction is quantized at ONE ck_apre period (3.9 ms of spread against 12000 ticks uncorrected), and it can make the sub-second counter SKIP ITS ZERO, which reads as a second measured at exactly twice its length until the wrap detector is changed to an ARRIVAL at PREDIV_S. **RTC_SHIFTR (letter `l`) is timed as a LENGTH**: a delay of 64/256 makes the one second it lands in exactly a quarter longer, ADD1S alone adds a whole second and moves no sub-second, and ADD1S with SUBFS is the register's only atomic ADVANCE - with SHPF standing for a measured 47 ms, far longer than 30.6.10's wording suggests, and a shift refused at a calendar boundary because SUBFS pushes SS above its own top. **TAMPER DETECTION (letter `m`) is built and run, and two findings shaped it.** TAMP_CR1 comes out of a domain reset holding 0xFFFF0000 - ITAMP3E..ITAMP6E, FOUR internal erase sources armed by the reset value alone on a board that had never been told anything about tampering, none of them with a NOERASE bit. And **ARMING A TAMPER INPUT TAKES THE PAD, pulls and all**: a PA0 driven high reads 1 in IDR before TAMP2E is set and 0 the instant it is with MODER untouched, so no program on this board can put an EDGE on a tamper input and the established drive-the-pad technique has no twin here. What is left is a pad the OUTSIDE world holds - TAMP_IN1 is PC13 with the user button's own pull-up - over which the filtered detector starts its sample train AT THE ARMING, so the latency from arming to flag IS the filter's N/f: 2/4/8 samples at 128 Hz cost 17465/35211/66523 us against 15625/31250/62500 predicted, and two samples at 128/32/8 Hz cost 19464/43032/136944. 31.3.4's caution DOES NOT REPRODUCE (an edge detector armed over a standing active level fires in neither polarity); TAMPPUDIS is real (a free PA0 never fires with the precharge on and fires in 349215 us with it off); TAMPxMSK outranks the erase; a tamper fills the timestamp registers with no RTC_TS pad in it, and TAMPTS costs a FILTERED detector nothing because 31.3.4's three latency rows are ALTERNATIVES and not a sum. Letter `w`, outside `z` because it spends them, shows **a detected tamper erasing all five backup registers** and **the calendar's own overflow as a tamper** - 99-12-31 23:59:59 raising ITAMP5F, erasing the backups and freezing the calendar, which a write in initialization mode then starts again. Letter `n` closes the leftovers: the date and weekday alarm masks both match and a wrong weekday stays silent, and LSCOEN does not take a pad an alternate function owns (PA2 is the LSCO pad AND this console's transmit line, and the console printing the verdict is the witness). The LSE clock security system is DECLINED with its reason - 5.4.23 reads its enable as one-way and the only way back would be the domain reset this bench must not take. Findings in [stm32g0/rtc.md](stm32g0/rtc.md)  **On the second silicon**: 125/125 on the Nucleo-G071RB (plus `w` 11/11 and `v` 4/4), whose LSE crystal started first try at 32736 Hz, which has TWO external tamper inputs, and on which arming one does NOT take its pad. |
| `test_stm32_sleep` | **Bench test suite** (keep passing) for the STM32G0's STOPPING HALF - `stm32g0/pwr.hpp` (RM0444 ch. 4 whole) and `stm32g0/sleep.hpp`'s two `util/power.hpp` sites - 8 letters in `z` / 50 verdicts, **50/50 three times including a cold run**, NOTHING TO WIRE and nothing written to flash. The wall clock HAS to be the RTC (every TIM lives in the VCORE domain and stops in Stop, and SysTick rides HCLK), so the calendar runs on the crystal with PREDIV_A 0 / PREDIV_S 32767 - a 30.5 us stopwatch that keeps counting with every clock in the chip stopped - and the backstop is the IWDG, armed once at 32 s and fed, since it cannot be turned off again. THE FINDING THAT SHAPED THE DRIVER, AND ITS CORRECTION: **a Stop entered with the kernel's 1 kHz tick armed LASTS on a bare board** (250 ms of 250) and is cut to 1 ms by DBGMCU_CR.DBG_STOP, a bit that keeps HCLK running inside a Stop for the debug logic, that OpenOCD's stm32g0x.cfg sets at every connection whose examine finds the DBGMCU clock gate open, and that survives every reset but a power-on - which is how the first measurement read "0..3 ms, three runs of three" and stood for a campaign (letter c now reads the bit through its gate and judges the state it finds; bin/brio clears the register after every flash). The site pauses the ticker for the deep rungs and resumes it on disarm all the same, which costs nothing because kernel time was going to stand still anyway. Also: kernel time standing still across a Stop (250 ms of wall, 0 ms of tick); **SYSCLK dropping to HSISYS at the wake** read at the instant of waking, with `resume_clock()` putting the PLL back; the four-rung LADDER read back off the silicon, with `light` mapping to Sleep and Standby/Shutdown deliberately OFF it; **a 500 ms deadline maturing at 723 ms of WALL with the plain site and at 501 ms with the TIMED one**, six repeats of 150 ms landing at 150..151 with NOT ONE EARLY; **ES0548 2.2.4 proven not to reach an RTC wake** (250 ms of Stop with HSIDIV at /4 - the erratum's own condition); a pending interrupt making a Stop fall through in microseconds, staged under a mask because an enabled line set pending with interrupts on does not stay pending; and the per-rung WAKE LATENCY **declined with the reason** - one wall tick of difference where the datasheet says 5.6 us, and every counter fine enough to resolve it stops in Stop. Letters `s` and `u` sit OUTSIDE `z` because they reboot the board: Standby and Shutdown both come back THROUGH THE RESET VECTOR, both leave PWR_SR1 at 0x8100 and **RCC_CSR naming no reset source at all**, and both leave the RTC counting and the backup registers intact - which is how the letter resumes itself (6/6 each). Findings in [stm32g0/pwr.md](stm32g0/pwr.md)  **On the second silicon**: 50/50 on the Nucleo-G071RB, with letters `s` and `u` 6/6 each - and a SHUTDOWN wake that looks different there (SBF cleared, PWRRSTF raised, where E leaves SBF standing and no flag). |
...
| `test_stm32_lptim` | **Bench test suite** (keep passing) for the STM32G0's LOW-POWER TIMERS - `stm32g0/lptim.hpp` (RM0444 ch. 26 whole) and, over it, `stm32g0/sleep.hpp`'s THIRD `util/power.hpp` site plus `util/pwm_channel.hpp` on its FOURTH silicon - 10 letters in `z` / 82 verdicts, 82/82 three times including a cold run from a fresh flash, NOTHING TO WIRE and nothing written to flash. THE WALL CLOCK HAS TO BE THE RTC (every TIM and SysTick stop in Stop), so the calendar runs on the crystal at PREDIV_A 0 / PREDIV_S 32767 and the backstop is the IWDG at 32 s. Four wireless stimuli: **pads walked by their own internal pulls** - and letter a proves the precondition first, that A PAD HANDED TO AN LPTIM *INPUT* ALTERNATE FUNCTION STILL FOLLOWS ITS PULL (the SAM found a DRIVING function takes it away); **the RTC's ALARM A and COMP1_OUT as triggers** over table 138's internal rows, with no pad on the timer's side at all; **a comparator flipped by the DAC against a precharged PA1** as the counted input through IN1SEL; and **the DMAMUX request GENERATOR** taking LPTIM1_OUT as table 56's trigger 20, so a DMA channel with no peripheral counts the timer's own output edges with no CPU in the loop. HEADLINES: **a forbidden CFGR write LANDS ANYWAY** while the block is enabled (26.7.4 forbids it and the register takes it - what the chapter forbids is what the counter then does with it, so the refusal has to be the driver's), where a start written to a DISABLED timer really is discarded by hardware; **CFGR2 keeps EIGHT bits**, settling 26.7.9's two-bit drawing against the device header's four-bit fields; **the waveform arithmetic 26.4.10 never prints** - period ARR + 1, high time ARR - CMP + 1 (so CMP = 0 is a FULL duty), and a FLAT LOW at the forbidden CMP >= ARR; **26.4.12's lost first edges are EXACTLY FIVE** on both polarities, against COUNTMODE = 1 losing none; **26.7.8's double read has a second reason the chapter does not name** (at one count per CPU cycle two reads NEVER agree - the value has MOVED, not gone incoherent - so `count()` gives nothing there); the output ceiling of kernel/2 measured EXACT at 16385 Hz against 16384; all three encoder sub-modes exactly as table 144 says; **through a Stop only LSE and LSI keep counting - HSI16 STOPS**, because a counter that merely counts makes none of the clock REQUESTS 5.3 lists the LPTIMs among; and **a compare match is a PER-LAP event and not a one-shot**, which is why the sleep site has to say its alarm is never cleared. Letter `h` is the campaign's point: **a 500 ms deadline through a Stop 1 matures at 501 ms of wall and NEVER EARLY with the RTC LEFT ALONE** - the same letter puts the calendar on 30.3.4's own low-power split, arms ALARM A for the application, and still meets a 300 ms deadline. ES0548 2.8.2 is code and measured; **2.8.1 is NOT staged and no verdict pretends it was** (reproducing it needs the ENABLE clear this driver has no verb for). Letter `j` is the SECOND INSTANCE on its own three pads - PC0, PC3 and PD6 at AF2, all pull-walked first - and it is where the two tables' asymmetries stop being quotations: all four codes of `LPTIM2SEL` measured (LSE 32740, LSI 32680, HSI16/128 125260, PCLK/128 501240 counts a second), the same waveform arithmetic off PD6, table 56's OTHER trigger input (21) counting LPTIM2_OUT at 16385 edges a second, both of 26.4.12's arrangements with the same five edges lost, `COMP3_OUT` legal on LPTIM1 and refused on LPTIM2 while `TAMP_TRG3` goes the other way, the two IN1SEL codes only LPTIM2 has, **TRGFLT measured on its own for the first time** (ten 60 us blips on PC3 leave the counter at zero with EXTTRIG never set at `samples8`, one 2 ms pulse starts it, and with the filter off the first blip is a trigger) - and **ONE VECTOR WITH TWO OWNERS**, `TIM7_LPTIM2_IRQHandler` serving LPTIM2's ARRM 19 times and TIM7's update 20 in the same 20 ms window, each body returning only its own flags. Findings in [stm32g0/lptim.md](stm32g0/lptim.md) and [stm32g0/pwr.md](stm32g0/pwr.md)  **On the second silicon**: 82/82 on the Nucleo-G071RB, every letter and every verdict. |
| `test_stm32_tickless` | **Bench test suite** (keep passing) for the STM32G0's TICKLESS KERNEL TIMEBASE - `stm32g0/lptim_ticker.hpp`'s `LptimTicker` under `Stm32g0Platform<TB>` and the kernel's optional `idle_until()` - 9 letters in `z` / 47 verdicts, 47/47 five times (one from a cold flash), NOTHING TO WIRE and nothing written to flash; letter h is an LSI-clocked ticker on LPTIM2 at a STATED rate (its ticks and millis against the crystal, a wake placed on it, the directional rule's 4 % price at the default statement), letter i a ten-second Stop 1 that costs FIVE LAP WAKES of 214 us each on HSISYS with the PLL never re-locked (TIM2 on MCO = HSI16/8 as the awake-time meter) - six interrupts since the compare is PARKED ON THE LAP (the first version parked it mid-lap and paid ten); `u` (a keystroke as the foreign wake) and `x` (the latency diagnostic) sit outside `z`. The whole image is tickless: `SysTick_Handler` is bound only to prove it never runs. Instruments: TIM2 free-running on the PLL as the awake wall, the RTC's sub-second counter as the wall a Stop cannot stop (THE RTC DOMAIN IS SELECTED FOR THE CRYSTAL WHEN FOUND EMPTY - a Nucleo has no backup battery and a USB unplug empties it - and left alone otherwise), the IWDG as the backstop, a second `LptimTicker` on LPTIM2 at the crystal's own rate as the lap witness. THE FINDING THAT FIXED THE DESIGN (letter x): a CMP write lands in 2..3 counts of WHATEVER clock the counter runs on - 74..88 us undivided, 2.0..2.8 ms at prescaler /32 - and CMPM fires at the count edge AFTER equality, so the counter runs undivided and the kernel tick is its count shifted (1024 Hz), the compare goes at the deadline's first count LESS ONE, and a deadline nearer than six counts is spun through. Measured: N ticks asked, N ticks slept to the count with one CMPM each; a deadline one tick away slept 980 us; a due one returns in 7 us; with nothing armed the RTC's wake-up timer ends the sleep and the LPTIM never speaks; two arms 150 us apart - the second finds CMPOK standing, pends the vector once and declines, the handler's clear visible in 0 us, the third stores; A STORE IMMEDIATELY BEFORE A STOP 1 LANDS WITH PCLK STOPPED (292 ms on the wall, three of three - the ticker's wait before a Stop is insurance, kept); three periodics at 7/30/250 ticks for three seconds fire 438/102/12 times exactly on their ticks with 535 LPTIM wakes for 534 CMPM and no deferral; a 500-tick deadline through Stop 1 under the manager with the PLAIN site matures at 488 ms of wall with 500 ticks elapsed, six rounds of 150 ticks all at 146 ms, none early; `delay_us` exact on the interrupt-less SysTick. A compare equal to ARR matches. Findings in [stm32g0/platform.md](stm32g0/platform.md), [stm32g0/lptim.md](stm32g0/lptim.md) and [stm32g0/pwr.md](stm32g0/pwr.md)  **On the second silicon**: 45/47 on the Nucleo-G071RB, letter `i`'s awake-time meter needing an ETRSEL code that part has not got. |
| `test_stm32_clock` | **Bench test suite** (keep passing) for the STM32G0's DYNAMIC CLOCK - `stm32g0/clock.hpp`'s `DynamicClock<Rates<Fast, Mid, Slow, Pll16>, Users...>` over 64 MHz PLL / Range 1, 16 MHz HSISYS / Range 2, 2 MHz HSISYS/8 / low-power run and 16 MHz on the PLL in Range 2 (the VCO at table 47's 128 MHz ceiling), on the TICKLESS platform - 10 letters in `z` / 42 verdicts, 42/42 three times (one cold), NOTHING TO WIRE and nothing written to flash; letter i hands SysTick to the BasicTicker and measures its rebase at every rung (500 ticks per 500 ms of crystal), letter j the PLL rate in Range 2, and letter g's Stops are judged by TIM2 on MCO as the witness that a Stop happened (a Stop 0 armed from low-power run stops the clocks like any Stop); `x` (the durations broken down) sits outside `z`. The console is USART2 on HSI16 (its rebase folds to nothing), the rebased user under test USART1 as a single wire on PA9 (HDSEL) on PCLK, the ADC on PCLK/1 as the second user; THE WALL is HSI16/64 through MCO into TIM2's ETR with no pad (ETRSEL = MCO, external clock mode 2 - 250 kHz within HSI16's 1 % of the crystal, 4 us a count), the RTC's sub-second counter for the Stops, the IWDG as the backstop. Measured: every rung reads back as the type claims and the CPU is at the claimed rate on the crystal (500 x delay_us(999) = 498..501 ms at 64/16 MHz, 515 at 2); the four steps the design named as unbenched - HSIDIV under a running core, a wait-state DECREASE, Range 2's own latency column, the PLL's refusal while running - all closed; a switch costs 48 us (64 -> 16) to 384 us (2 MHz LPR -> 64); the loop byte-exact at every rung and through 72 switches; the ADC's division /1 -> /2 -> /1 as predicted with VDDA 3310..3316 mV throughout; a 50-tick periodic through ten switches never early; a Stop 1 at 64 MHz restored from the wake's ISR and at 2 MHz the part WAKING IN LOW-POWER RUN WITH HSIDIV KEPT. Findings in [stm32g0/clock.md](stm32g0/clock.md), [stm32g0/pwr.md](stm32g0/pwr.md), [stm32g0/adc.md](stm32g0/adc.md), [stm32g0/tim.md](stm32g0/tim.md)  **On the second silicon**: 40/42 on the Nucleo-G071RB, where the wall falls back to the LSE (30.5 us a count) and letter `h`'s 20 us `delay_us` cannot be judged on it. |
| `test_stm32_crc` | **Bench test suite** (keep passing) for the STM32G0's CRC calculation unit - `stm32g0/crc.hpp` (RM0444 ch. 14 whole) - 5 letters in `z` / 27 verdicts, 27/27 three times including a cold run from a fresh flash, NOTHING TO WIRE, no pad moved, no flash written. The instruments are all inside the chip: `util/crc.hpp`'s bitwise CRC-16 as the software judge of the hardware's CCITT preset, a reflected CRC-32 written in the suite (not in `util/`) as the judge of the CRC-32 preset, SysTick as the cycle stopwatch, and one MEM2MEM DMA channel as the no-CPU feeder. Measured: **all three of 14.3.3's REV_IN examples are exactly what the silicon does**, checked against a checksum the unit really computed rather than against themselves; **a polynomial changed mid-calculation gives the checksum of nothing - but UNRELIABLE DOES NOT MEAN RANDOM**, the same illegal sequence twice giving the same number, so what is lost is the meaning and not the determinism; the four catalogue check values over "123456789" at all four POLYSIZE widths (0x29B1, 0xCBF43926, 0xF4, 0x75); **`crc_ccitt_false_config` IS `util/crc.hpp`'s CRC-16 bit for bit over 4 KB**, by two mechanisms sharing only their definition; `feed(span)` proven to mean byte-by-byte feeding in all four REV_IN settings; **2056 cycles per 1000 bytes through the word path against the bitwise loop's 109216 - a factor of 53** - with the input buffer of 14.2 shown real; and **16 KB of the program's own flash checksummed three ways to one number** (CPU-fed, DMA-fed at 36.5 MB/s, and a software reference). Both refusals fire before a register is touched. `util/crc.hpp` is NOT hooked to the hardware: that is a util design question this stratum must not open. Findings in [stm32g0/crc.md](stm32g0/crc.md)  **On the second silicon**: 27/27 on the Nucleo-G071RB too, nothing gated - the one driver of this stratum that needs no fact from the reserve. |
| `test_stm32_serial` | **Bench test suite** (keep passing) for the STM32G0's SERIAL TAIL - `stm32g0/usart.hpp` (RM0444 ch. 33 whole, beyond the console personality the bring-up built), `stm32g0/lpuart.hpp` (ch. 34) and `stm32g0/irtim.hpp` (ch. 27) - 15 letters in `z` / 88 verdicts, 88/88 three times including a cold run from a fresh flash, plus three HOST-ASSISTED letters outside `z` (`y`, `w`, `v` through `brio stress`, 2 verdicts each, twice over). NOTHING TO WIRE, no flash written. TWO FACTS MAKE THE WHOLE CHAPTER STAGEABLE ON ONE BOARD: **CR3.HDSEL turns any instance into its own loop-back** (TX and RX internally connected, the pad open-drain with the internal 40 k pull-up serving as 33.5.15's external one - and a pad under an OPEN-DRAIN alternate function is proven first to still follow its own pull), and **a pad handed to the RX alternate function is an INPUT whose PUPDR the CPU moves in under a microsecond**, so at 2400 baud software puts an ARBITRARY frame on the receive line - a rate nobody was told, a rate a few per cent off, a 13-bit break, an RZI-encoded frame, a bad parity, a short stop bit, a quarter-bit glitch inside a stop bit. TIM2 free-running at 64 MHz is the ruler and a DMAMUX request generator counting EXTI edges is the no-CPU witness. HEADLINES: **three authorities on every instance** (the reserve's table 183, the header's pointer-comparison macros, the silicon) and **the row of table 184 that is NOT the FULL/BASIC split** - CR2.CLKEN sticks on all six USARTs including the BASIC ones and on NEITHER LPUART, so synchronous mode is every USART's; **a BASIC instance's PRESC is worse than absent** - the field takes the value, reads it back, and the transmitter then emits nothing at all (ES0548 2.11.2's subject, measured); **the console moved under itself** to HSI16 and to SYSCLK while its own verdict lines kept coming out at 115200 with every counter zero; **a loop cannot show what a FIFO is for** (one interrupt a byte either way - a single wire is its own pacer) and the letter says so instead of dressing it up; **the noise flag is a majority-vote disagreement and nothing else**; **the tolerance of tables 188/189 walked from both sides in all four arrangements and MEETING every row**; **ES0548 2.11.1 REPRODUCED WITH ITS CONTROL** (8 of 8 frames silently corrupted by a quarter-bit glitch in the SECOND half of a stop bit, 0 of 8 in the first); smartcard's guard time in BAUD periods and its CK at kernel/(2 x PSC), with a NACK staged from a PORT pull and the retries counted on the pad; IrDA's 3/16 pulse and its low-power width that stops moving with the baud rate; and **IRTIM's USART envelope proven ACTIVE LOW** where TIM16's is active high. The host letters add: streaming byte-exact on every kernel clock at 115200/460800/921600; **all three of 33.5.21's wake sources bringing the part out of Stop 0 with the poked bytes surviving**; **ES0548 2.2.4 REPRODUCED on a USART wake** where the RTC campaign found it did not reach an RTC one, with HSIKERON proven NOT to be the way round it; and **a whole console moved onto LPUART1 on the console's own pads at AF6**, clocked by the 32768 Hz crystal at 9600 and by HSI16 at 115200, byte-exact and back. Findings in [stm32g0/usart.md](stm32g0/usart.md), [stm32g0/lpuart.md](stm32g0/lpuart.md) and [stm32g0/irtim.md](stm32g0/irtim.md)  **On the second silicon**: 83/88 on the Nucleo-G071RB (host letters `y`/`w`/`v` 2/2 each), whose USART3 is BASIC and which has no LPUART2 - and where ES0418 2.2.4 takes the four verdicts that count edges with no CPU. |
| `test_stm32_analog` | **Bench test suite** (keep passing) for the STM32G0's ANALOG BLOCK - `stm32g0/adc.hpp` (RM0444 ch. 15), `stm32g0/dac.hpp` (ch. 16), `stm32g0/vref.hpp` (ch. 17) and `stm32g0/comp.hpp` (ch. 18), and through them `util/analog.hpp` and `util/analog_sampler.hpp` on their THIRD silicon - 18 letters in `z` / 139 verdicts, 139/139 three times including a cold run from a fresh flash, NOTHING TO WIRE and nothing written to flash. ONE SUITE FOR FOUR CHAPTERS because the silicon joins them: the DAC's only route to the ADC is a PAD (PA4 is DAC1_OUT1 and ADC_IN4 at once - the ADC's connectivity figure lists no DAC among its nineteen inputs), its only internal route is a COMPARATOR's inverting input, and VREF+ is the one rail all three measure against. Four wireless techniques: **the zero-length wire** (the DAC read back through the ADC on one bond pad - monotonic, straight to 2 counts of 4096 end to end); **the factory values as the scale** (VREFINT_CAL and TS_CAL1/2 giving VDDA 3310 mV and the junction temperature 32.6 C with no meter, once the raw reading is rescaled to the 3.0 V the calibration assumes); **an internal threshold against a PRECHARGED pad** (7.3.13 disconnects a pad's pull in analog mode, so the comparator's stimulus is a node driven to a rail by GPIO and then handed over - it holds past the 100 ms cap the letter puts on it); and **a comparator measured with no pad on either side** (its edges counted twice over, 5 EXTI interrupts on line 17 and 5 TIM1 captures through TIM1_TISEL). HEADLINES: **conversion time exact to the CPU cycle** in six configurations (a DMA channel counts 256 conversions so no reader is in the loop, and CKMODE = PCLK/2 makes the prediction an integer); **A FORBIDDEN WRITE IS NOT ONE THING** - CFGR1 takes an AWD1 bit with ADEN set while AWD2CR ignores the identical write in silence, which is how the watchdogs became disabled-state verbs and how ES0548 2.6.2's own door was found; **VREFBUF_CSR reads 0x0 through the closed SYSCFG gate**, which is not its reset value but the pulled-down mode, so an application reading it before opening that gate concludes VREF+ is being grounded; **the DAC's output buffer cannot reach the rails** and switching it off says by how much (46/3276 mV against 3/3308 on a 3312 mV supply); and **one TIM6 TRGO pacing BOTH converters** with a DmaLoopEngine feeding the DAC and a DmaPingPongEngine draining the ADC - zero samples off the played table over six blocks, every seam stepping by exactly 8, once the launch block is spent (16.4.8 makes the first datum the CPU's) and the ADC's sampling window is made longer than the DAC's settling time so its sample-and-hold closes on a settled value. Also: the oversampler against a noise floor MEASURED first; the sequencer in both faces with its CCRDY handshake; ES0548 2.6.3 reproduced with a control and 2.6.4 measured at one cycle; `AnalogSampler` in a real kernel walking three inputs, 60 samples, none mislabelled; and the blanking window proven a real gate with TIM1's OC4 and no wire. Letters `m` and `n` (campaign 10) then take the comparators as far as one board goes. **A FLOATING PAD DOES NOT STAY AT ITS RAIL**: released from VDD and read continuously it relaxes in a few milliseconds to about 1.22 V - a third of the supply - and SITS there, reproducible to a count between one release and the next (1511 then 1512), with the control showing the conversions are part of what holds it (ten milliseconds of looking elsewhere cost five hundred counts). That level is enough to put **the DAC on silicon as a threshold** (`internal_unbuffered`, INMSEL 4, PA4 not even claimed - VALUE 1 at DAC code 0 and 0 at 4095), to put `dac_channel2` there as the upper limit of a window, and to reach **THE WINDOW'S INSIDE STATE** at last - all three states, above both limits, between them and below both, by moving the window instead of the signal. The **two propagation delays** differ by a real measured time (687..1062 ns high-speed against 1109..1218 ns medium-speed, a gap of 406..484 ns where table 68 prices it at 270). **THE OFFSET AND THE HYSTERESIS ARE STILL DECLINED, and now with the number that declines them**: the band between a falling and a rising crossing is 88..93 mV WITH HYST CLEAR, where it should be zero and where the largest level table 68 offers is 30 - so the band is the node's own motion, the node being a sawtooth held up by the very conversions that read it. One wire from PA4 to PA1 settles both, and nothing is faked. Letter `n` closes the rest with no analog at all: **COMP2 and COMP3 on their OWN plus pads** (PB4/PB6 and PB0/PC1) and COMP3's EXTI line 20 delivering eight interrupts for eight edges; **COMP1_OUT on PA6 at AF7**, the pad's input register following VALUE and the EXTI line of that same pad counting its edges - the exti campaign's finding extended from the CPU to a peripheral; and **three more of the five blanking sources** (TIM2_OC3, TIM3_OC3, TIM15_OC2) proven real gates, with TIM1_OC5 declared unreachable because `tim.hpp` builds four channels a timer and TIM1's fifth is the combined-PWM pair it does not build. WINOUT stays as it was: it changed neither CSR.VALUE nor the EXTI line with the bit standing and the pair made to disagree. Letter `o` takes the DAC's own tail the same way: **both wave generators run one SOFTWARE step at a time** (a triangle at MAMP 5 climbing exactly 63 codes and turning AT the top, read back through the zero-length wire as 64 ADC counts; the noise generator's MAMP proven a MASK at 15 and 489 codes of span), **16.4.12's offset calibration run as the procedure it is** (CAL_FLAG low at trim 0 and crossing at 14 where this die's factory trim is 7, with the factory value restored bit for bit and calibration refused in an unbuffered mode), and **SAMPLE-AND-HOLD WITHOUT LSI PROVEN TO DEGRADE IN SILENCE** - with LSIRDY read clear the pad carries its value to within a count exactly as the plain buffered mode does, the times land, BWST never stands at either state of the clock, and no bit an application can read tells an armed mode from a degraded one; `dac.hpp`'s own comment said the opposite and is corrected. THE VREFBUF IS NEVER ENABLED: 17.1 forbids driving VREF+ where a board ties it to VDDA, and nothing inside the chip can tell that case from a free pin. Campaign 10b's three letters take the two converters' own tails. **Letter `p`: every remaining row of the DAC's trigger multiplexer**, one event at a time - TIM1/TIM2/TIM3/TIM7/TIM15 TRGO on one software update each, both LPTIM outputs started for one edge, and EXTI 9 through a pull-walked PB9 - plus **the DMA underrun caught**, a converter asking for a DMA with no channel armed raising DMAUDR on the trigger after the first and carrying it to the shared TIM6/DAC/LPTIM1 vector once per unserved trigger; the flag has to be read with the interrupt OFF, because a handler that serves it clears it. **Letter `q`: the ADC's low-power pair and its unrun modes** - WAIT proven by a pair of runs one bit apart (a continuous converter nobody reads OVERRUNS, the same one with WAIT stalls and the READ is what releases it), AUTOFF priced at 700..900 ns of start-up per conversion, discontinuous mode converting one channel of a three-channel sequence per start with EOS only on the third, TOVS needing exactly eight triggers for one x8 result, and all seven remaining hardware triggers fired once each - with two facts table 75 does not carry: TIM1's TRGO2 needs no new verb because MMS2's reset value makes the UPDATE event the trigger output, and TIM1_CC4 is a COMPARE and not an EGR strobe (a software CC4G into a stopped timer moved nothing). The asynchronous clock's other root is measured at last and it took a discovery: **`Adc::configure()` writes ADC_CCR's prescaler and NOT RCC_CCIPR's ADCSEL**, so a letter that changed the config alone measured the same conversion time from both roots; written properly, HSI16/4 is 2883 ticks against SYSCLK/4's 753. **Letter `r`: the external inputs as a MAP** - twelve channels on twelve free pads each following their OWN pad by 449..1785 counts of 4096 while the same walk read on a different channel moves 15 - and the reason the swings are a third of full scale rather than full: **the comparator's precharged-pad technique does not carry to the converter**, an ADC's sample-and-hold being a capacitor of the same order as the pad's own, so the first conversion of a precharged pad shares its charge. A known VOLTAGE on one of those pads still needs a wire. Findings in [stm32g0/adc.md](stm32g0/adc.md), [stm32g0/dac.md](stm32g0/dac.md), [stm32g0/vref.md](stm32g0/vref.md) and [stm32g0/comp.md](stm32g0/comp.md)  **On the second silicon**: 136/139 on the Nucleo-G071RB, which has two comparators and not three; VDDA 3317 mV, the conversion times exact to the cycle, and ES0418 2.7.1 not reproduced. |
| `test_stm32_dma` | **Bench test suite** (keep passing) for the STM32G0's DMA and DMAMUX - `stm32g0/dma.hpp` (RM0444 ch. 10, 11) and, through them, `util/block_stream.hpp`'s two concepts on their SECOND silicon - 14 letters in `z` / 69 verdicts, 69/69 SEVEN of eight consecutive runs including cold ones, NOTHING TO WIRE and nothing written to flash. The eighth lost ONE verdict, letter `j`'s tightest - "no sample is off the 32000-tick step" - and nothing else in that letter; it is the flakiest estimator in the suite and [stm32g0/dma.md](stm32g0/dma.md) records the rate rather than tuning it away. THE CONSOLE IS PART OF THE EXPERIMENT: this suite's own USART2 carries a `DmaTxEngine` on channel 6 and a `DmaRxEngine` on channel 7, so every verdict line left the chip without the CPU touching a byte and every letter arrived the same way - the suite cannot pass vacuously. Five wireless techniques: memory-to-memory (no peripheral at all); **a timer as the pace and another as the payload** (TIM3's update event is the request, TIM2's free-running 32-bit CNT is what the channel reads, so consecutive samples differ by exactly TIMPCLK/rate and a captured block CHECKS ITSELF); a pad read while a peripheral drives it (a duty table played into TIM2's CCR1, read off LD4); a capture with no pad (TIM16's TISEL on LSI); and a peripheral with no pads claimed (USART1's TXE standing, with nothing leaving the die). HEADLINES: **a request is a LEVEL served on ENABLE, not an edge latched on the rise** - the opposite of the SAM's DMAC, so there is no kick() here (measured with a control); **two memory-to-memory channels ALTERNATE and the software priority does not enter into it** (10.4.4's own clause), the lower index winning all three arrangements, and the PL field's own effect DECLINED rather than dressed up; and **THE FIXED POINT** - `BlockPlayer` fits the hardware CIRC mode exactly (a player runs with its interrupt disarmed) while `BlockSource` does NOT, six elements having already landed in the half a caller was holding at a 2 MHz request rate before a handler could stop the channel, which is why the ping-pong engine is non-circular. Also: 49.7 MB/s memory-to-memory, table 51's widening and truncation as printed, a transfer error disabling the channel in hardware and gating the re-enable, three vectors confining themselves, a request generator on TIM14_OC moving exactly GNBREQ+1 a trigger, one channel pacing another through EGE, `BlockRelay` in a real kernel with zero bad samples and zero bad seams, and letter `u` (outside `z`, with `brio stress`) putting the ST-LINK VCP's ceiling at 921600 in both directions - **a ladder that now STOPS there**, with the two rungs above it moved into letter `w` (outside `z` and outside every verdict) and declined by the tool too unless it is given `--beyond-vcp`, which is what stopped letter `u` losing its closing tally to a pump the bridge could not take. Letters `k` and `l` (campaign 10) close the two gaps this document had been carrying. **THE SYNCHRONIZATION BLOCK**, built and never run, needed a stimulus that was already on the board - table 57's sync input 22 is TIM14_OC, the same signal table 56 offers the request generator - so TIM3's update at 20 kHz is the request and TIM14's OC1REF at 1 kHz the sync: in a five-millisecond window a channel that saturates its whole 64-word transfer unsynchronized moves 18 words at NBREQ 4 on the rising edge, 20 on the falling, 39 on BOTH and 5 at NBREQ 1, so SPOL is a real selector and NBREQ is the COUNT and not the count minus one. **A SYNCHRONIZATION OVERRUN IS REAL**: SOF stands when a sync edge arrives with the previous batch unserved, CFR clears it, and SOIE delivered 7 interrupts on the third NVIC line - measured in TWO legs, because the handler that would clear SOFx is on the vector the console's own engines live on, so ENABLING it is a no-op and DISABLING it takes the transmitter down (the first version of the letter did exactly that and the board went silent mid-letter). The request generator's other two polarities go the same way: 10 words rising, 10 falling, 20 on both, over ten TIM14 periods. And **THE TIMER'S DMA BURST ENGINE** (`DCR`/`DMAR`, [stm32g0/tim.md](stm32g0/tim.md)'s own first gap): four words a period played in a circle into TIM2, each row's CCR1 at half its own ARR, LD4 reading 499 per mille whatever the period does against a CONTROL of 199 where only ARR is written - the only difference between the legs being DCR's length field. **THE REGISTER MAP HAS HOLES AND THE WALK DOES NOT SKIP THEM**: DBA is a word offset from CR1, so ARR-then-CCR1-then-CCR2 is a burst of FOUR because offset 12 is a repetition counter TIM2 has not got. TWO HONEST DECLINES recorded in print: a software EXTI trigger did not reach the request generator though the hardware one did, and the PL field did not reorder two saturating channels. Campaign 10b's letters `m` and `n` close what was left. **DMA2 is five channels and not one**: every one of them moves a block at every width byte for byte, so with DMA1's seven - five for the letters and two for this console's own engines - all twelve channels of the part have carried traffic, and DMAMUX's numbering runs straight through both controllers. **10.4.5's first sense runs**: TIM6's update paces a channel that reads TIM3's CCR1 and writes TIM4's CCR1, a request from one peripheral moving data between two OTHERS, and nothing in the vocabulary names the arrangement because it is two addresses and a request id. **THE SLEEP STORY, whose two halves have opposite answers for one reason** - the controller is on HCLK, which Sleep keeps and Stop takes away: a 512-word block armed with its completion interrupt runs to the end with the CPU in WFI and its own completion is what wakes the core; and a TIM6-paced stream at 10 kHz is FROZEN by a 60 ms Stop 1 and not broken by it (CNDTR 391 before, 391 on the wake, 382 a millisecond later), the wake being an LPTIM1 compare on LSE whose VECTOR HAS TO BE BOUND - a Stop left through an EXTI direct line lands in the handler, and with none bound it lands in `Default_Handler` and never comes back. Findings in [stm32g0/dma.md](stm32g0/dma.md)  **On the second silicon**: 65/69 on the Nucleo-G071RB, which has ONE controller and no TIM4; and where ES0418 2.2.4 makes an EXTI line reach a request generator only as a pending-interrupt LEVEL. |
| `test_stm32_i2c` | **Bench test suite** (keep passing) for the STM32G0's I2C - `stm32g0/i2c.hpp` over RM0444 ch. 32 whole, BOTH ROLES IN ONE IMAGE - and, like the SPI suite, TWO INSTRUMENTS on one pair of pads: letters `a`..`m` on the Nucleo's own SELF-LINK and letters `n`..`r` (z 54/54 twice including a cold flash against a SAM C21, and 54/54 three times including a cold flash against a SECOND STM32G0 - board F running `twi_peer`'s stm32g0 port) on the bus to a PEER BOARD, commanded in band over `avrdx/src/apps/twi_link.hpp` where ONE COMMAND IS TWO TENURES of the engine under test; the peer's `ident` says which port answered. The suite probes which wiring is fitted and each set skips itself on the other desk. The peer half: the command channel at a measured 99 kHz, ten of ten round trips; write, read and the combined write-then-read that the peer counts as FOUR address matches for two tenures, plus the general call; the vocabulary as real statuses across two architectures (`i2c_nack_addr` from a client parked where nobody calls, on a write AND on the empty probe; `i2c_nack_data` at a commanded byte) and commanded stretching priced (8 x 8 bytes in 6 ms against ONE 8-byte tenure at 2 ms a byte in 16 ms); ALL THREE SPEEDS byte-exact INCLUDING Fm+ at 1 MHz on two short jumpers and their 2.2k pull-ups; and THE KERNEL LETTER with `I2cBus` over `I2cHost` against a second chip - the NACK in its place, the rejection, both votes, and A WEDGE THE PEER HOLDS answered `i2c_arb_lost` in 0 ms with SDA still low, where the self-link's own wedge is answered by the arbiter's `i2c_timeout` at 20 ms (two signatures, recorded and not reconciled). The self-link half: 13 letters / 142 verdicts, **142/142 three times including a cold flash**, on the Nucleo's own SELF-LINK (I2C1 host PB8/PB9 to I2C2 client PA11/PA12, both AF6, with the two 2.2 kOhm pull-ups). `I2cHost`'s Request is `avrdx/twi.hpp`'s and `samc21/i2c.hpp`'s VERBATIM, so letter `l` gives `util/i2c_bus.hpp` and `util/bus_master.hpp` their THIRD silicon with not one line of `util/` or `kernel/` changed. HOW A BUS RATE IS MEASURED WITH NO PAD TO SPARE: both ends of SCL are alternate functions, so the stopwatch is the CPU's own cycle counter around a whole tenure and the arithmetic is the wire's (a tenure of N bytes is 9 x (N+1) SCL periods), with the target put in NOSTRETCH so the period measured is the controller's own. Letter `a` is wireless and asks the SILICON what the header cannot be asked - an instance without SMBus forces TIMEOUTR to zero, so a write that reads back names its column of table 165 (I2C1 yes, I2C2 yes, I2C3 no) - and finds THE ENABLE PROTECTION REAL on all four PE-gated fields, the exact opposite of what the same manual's SPI chapter does. `b` is the link with the repeated START counted from the far end (two address matches, one STOP); `c` the vocabulary on the wire, where a held SDA turns out to be a PARK and not an error (BUSY alone, no ARLO, no BERR); `d` the three speeds bracketed between the register's own floor and the standard-edge prediction, the Fm+ drive, and the whole dynamic-clock ladder - **at a 2 MHz core NO speed is legal on PCLK** (ES0548 2.10.1's Sm floor is 4 MHz, above the datasheet's own table 74) **and the same bus runs byte-exact with the kernel moved to HSI16**, which is what the independent clock is for; `e` clock stretching priced (20/50/100 us per event lengthening an 8-byte read to 307/588/1057 us) and NOSTRETCH's underrun sending 0xFF; `f` 10-bit addressing with ADDCODE proven to be the HEADER (0x79 for 0x155) and OA2's mask as a real range; `g` the filters as the timing they are; `h` a 300-byte write through TCR in ONE tenure and the DMA engines both ways; `i` the PEC pinned against a bitwise CRC-8 (0x3D against 0x3D) and WHOSE hold each of the three SMBus time-outs polices, with a control on each side - its own, never a peer's; `j` the wake armed and everything around it measured, the Stop itself DECLINED because both ends of this bus stop together; `k` `unstick()` reading the wire before clocking it; `l` the kernel, where a wedged tenure is answered `i2c_timeout` at exactly the arbiter's 20 ms with SDA still low; `m` the errata. Letters `x` and `y` sit outside `z`: a POLLED trace of one tenure and the same one with the vectors live, bounded and counted - the instruments this suite needed when a wire letter wedged the board, kept because the next one will need them too. Findings in [stm32g0/i2c.md](stm32g0/i2c.md)  **On the second silicon**: 54/54 with the roles exchanged, SCL at 99/400/1000 kHz - and the G071's I2C2 has NO independent clock, so `smbus_probe()` answers yes/no where the G0B1's answers yes/yes. |
| `test_stm32_spi` | **Bench test suite** (keep passing) for the STM32G0's SPI/I2S - `stm32g0/spi.hpp` over RM0444 ch. 35 whole, BOTH ROLES IN ONE IMAGE - and it carries TWO INSTRUMENTS on one set of pads, because the desk has held both: letters `a`..`m` on the Nucleo's own SELF-LINK and letters `n`..`r` (20 verdicts) on the link to a PEER BOARD running `spi_peer`, commanded in band over `avrdx/src/apps/spi_link.hpp` - either instrument is valid and the peer's `ident` says which answered (fw 0x01xx the AVR, 0x02xx the SAM C21, 0x03xx a second STM32G0). The two wirings are the same jumpers at different ends, so the suite PROBES which one is fitted before any letter runs and each set skips itself on the other desk, with the reason printed and no verdict claimed. The peer half against the SAM C21 (z 38/38 twice including a cold flash): the command channel at PCLK/256 = 250 kHz; the four modes and both bit orders byte-exact; THE BR LADDER holding to PCLK/8 = 8 MHz and breaking at PCLK/4 WITH THE PEER'S OWN COUNT FULL AND ITS MISMATCHES ZERO - the boundary is its answer reload, not the wire. AGAINST A SECOND STM32G0 (`spi_peer`'s stm32g0 port on board F, z 38/38 twice including a cold flash): the ladder now runs to PCLK/2 = 32 MHz and HOLDS THERE - the whole register vocabulary, no boundary left inside it - with the peer's software pump measured beside it in the same letter (exact to 8 MHz, marginal at 16, slipped at 32) and no verdict claimed on that second climb; the kernel letter with `SpiBus` over `SpiHost` and A SECOND CHIP reading the bytes back (four queued in one dispatch, all spi_ok, 24 bytes exact, the rejection, both votes, `spi_timeout` and `recover()`); THE ROLES INVERTED - SPI1 as a `SpiClient` on the very pads it hosts with, the peer clocking at 2 MHz, twelve frames exact each way from the first; and letter `x` OUTSIDE `z`, the slip statistics (each mode ten times, no verdict) - the instrument that found the peer's MISO pad speed: at the driver's very-high, modes 1 and 2 (the falling-edge samplers) slip one bit mid-burst in 20 bursts of 160 while modes 0 and 3 never do; at high, 0 of 120 with the ladder unchanged ([stm32g0/spi.md](stm32g0/spi.md)). The self-link half: 13 letters / 89 verdicts, 89/89 three times including a cold flash, on the Nucleo's own SELF-LINK (SPI1 host PB3/PB4/PB5 + PA15 to SPI2 client PB10/PC2/PD4/PB12, the four wires this desk grew for the bus campaigns). Every wire verdict is a real transfer between two peripherals: the host polls in main context while the client answers under SPI2's vector ONE FRAME AHEAD (35.5.8), which is what makes one core two ends of a link. Letter `a` is wireless and carries the campaign's first finding - THE SILICON ENFORCES NONE of 35.5.7's with-SPE-clear rule (a raw write lands on all seventeen CR1/CR2 configuration fields) and a "Not used" data size is FORCED to 8-bit and not ignored; letters `b`, `c` and `d` are the link, the matrix (four modes, both orders, every frame size 4..16 with a pattern that uses every bit) and the rate ladder, where ALL EIGHT BR codes carry the link byte-exact to PCLK/2 = 32 MHz when the host paces it and a burst loop's frame period FLOORS at ~320 CPU cycles - this core cannot make a continuous SPI clock out of software, so the ceiling moves to letter `i` where both ends run on DMA channels (monotone, byte-exact to PCLK/4 = 16 MHz); letter `e` is NSS four ways (a GPIO chip select, the client's hardware input, MODF staged from the far pad, SSOE measured from the client's side at 77 cycles after the SPE store, and NSSP counted as eight pulses for eight frames on EXTI line 12); `f` is the CRC bit-exact against a bitwise reference in both lengths and both directions; `g` TI mode; `h` ES0548 2.12.1 REPRODUCED with the disable procedure as its control; `j` is `SpiBus` over `SpiHost` inside a real kernel with the client answering on the wire - the THIRD silicon for `util/spi_bus.hpp`, including the per-bus timeout and `recover()`; `k` is I2S1 master to I2S2 slave on three of the same wires; `l` the dynamic clock; `m` the wake from Sleep. Findings in [stm32g0/spi.md](stm32g0/spi.md)  **On the second silicon**: 38/38 with the ROLES EXCHANGED - the Nucleo-G071RB hosting and the G0B1RE as `spi_peer` - the BR ladder holding to PCLK/2 = 32 MHz and letter `x` finding 0 of 40 bursts slipped. |
| `test_stm32_fdcan` | **Bench test suite** (keep passing) for the STM32G0's FD CONTROLLER AREA NETWORK - `stm32g0/fdcan.hpp` (RM0444 ch. 36 whole, both instances and the 424-word message RAM behind them) - 12 letters in `z` / 96 verdicts, **96/96 four times including a cold run from a fresh flash**, NOTHING TO WIRE, no transceiver, no second node, nothing written to flash. A CAN controller is the one peripheral of this stratum whose whole purpose is another chip, and the chapter turns out to supply its own bench: **internal loop-back needs NOT ONE PAD** (filters, FIFOs, the Tx event FIFO, the DLC coding, timestamps and BOTH instances at once), **external loop-back still drives the TX pad** so a frame has a duration a CPU can time, **TEST.TX = 01 is a bit-rate meter** a DMAMUX request generator counts with no CPU in the loop, **the RX pad's own internal pull IS the bus** (up = recessive, down = stuck dominant - which is how the error machine, bus-off and the recovery sequence are reachable with nothing attached), and **the internal timestamp counter counts CAN bit times whether or not anything is on the wire**. THE HEADLINE: **36.3.4's 0xCC paragraph does not describe this part** - 64 DISTINCT bytes in one FD frame came back 64 of 64 byte-exact, none of them padding, where the chapter promises the tail padded on transmit and discarded on receive. Also measured: the registers ANSWER through a closed APB clock gate with their true reset values while a WRITE through it lands nowhere; one reset serves both modules (proven by resetting FDCAN1's registers through FDCAN2); CKDIV is one register and **FDCAN1's CCE is what opens it**, not FDCAN2's; the INIT readback costs at most 79 CPU cycles; 36.3.4's eight CCE-reset registers checked one by one, with TOCV PRESET rather than reset; every section start of figure 399; four bit rates and two dividers counted on the pad within 1 %, and the RESET NBTP measured at 4000400 bit/s where 36.4.7 prices it at 3 M from 48 MHz; **TEST.TX = 01 is a STROBE and not a level** (1 per mille of an 8 us bit, 15 of a 1 us one); **IR.TC is gated by TXBTIE and 36.4.15 does not say so**; **36.3.6's own safe read of an overwriting FIFO throws away one more message than the overwrite did**; a Tx FIFO keeping insertion order against a Tx QUEUE reversing it from the same TXBAR write; TXP measured at exactly two bit times; BRS visible as time (322 / 178 / 107 us for the same 64-byte frame at 2 / 4 / 8 Mbit/s) and TDCV putting the internal loop delay at 2 mtq; TSCV tracking TIM3's counter; **EW at TEC 96..104, EP at 128..136, bus-off after 846 us, and a recovery of 1421 bit times - which is 36.4.13's note (129 x 11) and not figure 398's (128 x 11)**; a stuck-dominant line proven to be a WAIT and not an error; **a restricted node sending NOTHING, settling 36.3.4 against 36.4.6's own contradictory description of the same bit**; and both modules looping at once on one shared vector, never hearing each other. Errata: 2.13.1 staged in both arms over 3000 FD frames and NOT REPRODUCED (recorded as that, not as a disproof), 2.13.2 UNREACHABLE BY CONSTRUCTION with TXBC's one implemented bit as the evidence. One console artifact recorded and not explained: about six bytes of one informational line in letter `k` come out corrupted, reproducibly, with two capture tools - no verdict rests on it. PADS MOVED: PB8 (FDCAN1_RX) and PB9 (FDCAN1_TX), both AF3, both proven free by their own pull first and both left in analog; FDCAN2 runs with no pad at all. Findings in [stm32g0/fdcan.md](stm32g0/fdcan.md) |
| `spi_peer` | The INSTRUMENT half of the SPI campaign, ONE protocol and THREE firmwares: the avrdx original (board B or A, SPI0 ALT1), the samc21 port (board C or D, SERCOM1 function C) and the stm32g0 port (board F, SPI1 AF0 on PB3/PB4/PB5/PA15 - the SAME PIN NAMES the DUT hosts on) - one loop that shifts whatever the DUT clocks, decodes a command frame, acknowledges it and becomes for a bounded moment whatever the DUT needs at the other end. All three are DARK LISTENERS (MISO driven for exactly one answer window, after a frame that checked out) holding the shared select wire up with their own pull-up, and all three serve `host_burst` (the roles invert: the instrument clocks a bounded burst so the DUT's client half is testable). The samc21 and stm32g0 ports serve an exchange through their own DMA ENGINES by default and through their software one-ahead pump when the command's spare bit (`spilink::spare_polled_pump`) asks, so a suite can measure BOTH boundaries: the SAM's engines break at 6 MHz, the G0's hold the WHOLE BR ladder to PCLK/2 = 32 MHz while its software pump - a reload per RXNE interrupt - holds to 8 MHz, is marginal at 16 and slips at 32. The stm32g0 port re-states its MISO pad at HIGH speed after every init that drives it: at the driver's very-high the answer edge couples into the SCK jumper and the falling-edge modes slip one bit mid-burst (20 bursts of 160 against 0 of 120 at high, the ladder unchanged), and its console `s` cycles the four speeds for the bench. The stm32g0 port's ident label is the 96-bit unique device ID's first word (`brio::DeviceUid`), its `xtal` is always 0 (no HSE is fitted on a Nucleo-64 and the stratum has no HSE root), its regimes map onto the FIFO's two preloads (the AVR's BUFEN/BUFWR do not exist here, and 35.5.8 asks for the pair), its `host_burst` maps aux16 onto the BR code that produces at most PCLK/aux16 (2 MHz for the suite's 32, against the SAM's 1.5 and the AVR's 750 kHz), and the AVR-only experiments (wrcol, feed_tx, mspi) answer cfg_failed. Console (observability only): `?` help, `i` status and counters, `0` back to the dark client, `3` command trace, and on the stm32g0 port `s` cycle the MISO pad speed |
...
| `twi_peer` | The INSTRUMENT half of the I2C campaign, ONE protocol and THREE firmwares: the avrdx original (TWI0, PA2/PA3), the samc21 port (SERCOM3 fn C, PA22/PA23, core at 48 MHz - the clean wire's bet, won) and the stm32g0 port (board F, I2C1 AF6 on PB8/PB9 - the SAME PIN NAMES the DUT hosts on, kernel clock PCLK at 64 MHz). A loop around a POLLED target answering exactly the command address (0x6B): decodes a frame, acks (or naks a bad checksum), then becomes what the DUT needs - a target that stretches by a commanded hold per byte (on both ports the hold is simply the wait before answering: the address match and every data event hold SCL by construction), NACKs the n-th byte, goes deaf, answers the general call; a stuck target holding SDA from GPIO released by counted SCL edges; the bus CONTROLLER for a bounded burst (a ROLE SWITCH on both ports - one instance is host or target, never the AVR's combined both). The stm32g0 port's ident label is the 96-bit unique device ID's first word and its `xtal` is always 0; its refusal rides TARGET BYTE CONTROL (SBC with NBYTES re-armed per byte, the decision taken at TCR before the byte's own RXNE); its one client configuration is solved for Fm+ because the DUT's speed ladder walks 100k, 400k and 1M against it and a target cannot know the coming rung; and its tally GIVES ONE BYTE BACK at the controller's closing NACK, because a target transmitter is always asked for one more byte than the controller takes and that byte never reaches the wire. Console (observability only): `?` help, `i` status, `0` back to command mode, `3` trace |
