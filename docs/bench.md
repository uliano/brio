...
banner. Today's verified mapping: A = console `usb-0:1.1` / probe
J42700051207; the second ICE J42700049508 is on the SAM board's SWD
(board C - its manifest entry names it), so board B currently has NO
programmer attached and carries whatever it last ran.

**Wires fitted today: the seven-wire PHASE F link, board A to board C**
(verified wire by wire over SWD + UPDI, each pin driven from one end
and read at the other - the technique is in the session memory):

| SAM C (SERCOM1, function C) | signal | AVR A (SPI0 ALT1 / TWI0) |
|---|---|---|
| PA16 = PAD[0] | MOSI -> | PE0 |
| PA17 = PAD[1] | SCK -> | PE2 |
| PA18 = PAD[2] | SS -> | PE3 |
| PA19 = PAD[3] | MISO <- | PE1 |
| PA22 = SERCOM3/PAD[0] | SDA <-> | PA2 |
| PA23 = SERCOM3/PAD[1] | SCL <-> | PA3 |

plus the dedicated GND (A to C) and the I2C node's 1.5k pull-ups to
+5 V. Both boards at 5 V. Board A's firmware today is `twi_peer` (the
phase F instrument - `spi_peer` for the SPI half; board C is the DUT,
the ROLES OF THE AVR CAMPAIGN INVERTED). A MEASURED FACT OF THIS
BUNDLE: its per-edge crosstalk is invisible to the AVR's filtered TWI
but not to the C21's filterless I2C - the SAM's I2C core must run at
6 MHz here and its CLIENT cannot follow a foreign 100 kHz host at all
([samc/i2c.md](samc/i2c.md)'s headline); separating the I2C pair from
the bundle is the standing fix. The AVR-to-AVR PORTE link is NOT
fitted;
every A-B two-board half (`test_avr_serial x`/`w`/`y`, the AVR SPI/TWI
`y` halves, the sleep pass) is therefore idle and needs its wiring AND
its peer firmware back. Reflashing B is what costs the heap its blocks:
...
| `test_samc_spi` | **Bench test suite** (keep passing) for the SERCOM in SPI mode - `samc/spi.hpp`, both roles - and THE FIRST CROSS-ARCHITECTURE SUITE: board C is the DUT and board A, an AVR128DB48 running `spi_peer`, is the instrument, commanded in band over the bus under test with the AVR campaign's own `spi_link.hpp` (one source file, two architectures, included by relative path). 7 letters / 61 verdicts, 61/61 cold and warm, needs the seven-wire phase F link (see Wiring). Letters: the register surface with errata 1.17.16 and 1.17.19 both NOT REPRODUCED at rev F in SPI mode (SWRST resets from the disabled state, DBGCTRL survives it); the command channel; the four-mode x two-order matrix byte-exact both ways plus the DORD mismatch as an exact two-way bit reversal; the rate ladder to the peer's polled-turnaround boundary (exact to 1 MHz back-to-back, its CLK_PER/6 electrical ceiling unreachable without inter-byte gaps); THE CLIENT ROLE via the peer's new `host_burst` op - the SAM as client on the OTHER DOPO row, PLOADEN's first character exact and the ONE-AHEAD pump the three-SCK-cycle rule forces; loop-back through the pad, the SCK ladder against the crystal, nine-bit characters, the two-deep receive buffer with BUFOVF/IBON, and CTRLB.MSSEN measured raising SS between EVERY character; THE KERNEL LETTER - SpiBus (= BusMaster) over SpiHost with queued requests, ordered replies, immediate rejection when over-full and the PrepareSleep vote, NOT ONE LINE of util/ or kernel/ changed. Findings in [samc/spi.md](samc/spi.md) |
