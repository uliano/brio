...
C = `spi_peer` (samc), D = `test_samc_spi` - the SPI campaign's
positions: D is the DUT because C's probe link through the hub is the
flakier of the two and the peer needs flashing least often. BOTH AVR BOARDS ARE UNPLUGGED -
...
without rewiring. The wires are SHORT, which is the point: the phase F
bundle's crosstalk finding ([samc/i2c.md](samc/i2c.md)'s headline)
rules this desk - the I2C pads (PA22/PA23) are deliberately NOT wired
today, and the I2C campaign gets its own short, separated pair later
(campaigns split precisely to keep every run this clean).
...
| `test_samc_i2c` | **Bench test suite** (keep passing) for the SERCOM in I2C mode - `samc/i2c.hpp`, both register sets - and the bus vocabulary's SECOND cross-architecture proof: board C is the DUT, board A runs `twi_peer`, commanded in band with the AVR campaign's own `twi_link.hpp` (every command is TWO tenures of the engine under test). 10 letters / 39 verdicts, 39/39 five times incl. cold, needs the phase F I2C pair + pull-ups (see Wiring). THE HEADLINE FINDING is the desk's own: the C21's I2C samples the wire with NO input filter, so this bundle's per-edge crosstalk kills a fast core (48/24/12 MHz die on the first tenure, 6 MHz is clean - the suite runs the core from generator 6, Fm+ declared unreachable and refused, never run slow in silence) and leaves the CLIENT deaf to a foreign 100 kHz host at any core rate (proven good against a bit-banged address; the client letter declines its data verdicts and says why). Also: erratum 1.17.16 unreproduced in I2C mode; write/read/write-then-read with the repeated start seen by the peer as two matches on one tenure; the general call; i2c_nack_addr and i2c_nack_data produced ON THE WIRE; commanded 2 ms/byte stretching = exactly 16 ms for 8 bytes; a tenure into a held-low wire PARKS and the timeline is printed; unstick() 0 pulses clean / 4 at the peer's 4th-edge release; THE KERNEL LETTER with I2cBus (= BusMaster) over the real wire - ordered replies, a NACK delivered in its place, rejection, both sleep votes - util/ and kernel/ untouched. Findings in [samc/i2c.md](samc/i2c.md) |
