...
| `test_avr_platform` | **Bench test suite** (keep passing): the PLATFORM - `AvrPlatform`, `delay.hpp` and `reset.hpp`, 9 tests / 92 verdicts, 92/92 on rev A5. Nothing to wire. Both delay paths counted in CLK_PER cycles against a 32-bit TCB pair (the folded path exact to the cycle, the runtime loop a constant 95 cycles, a DynamicClock 693), `delay_cycles` and its 16-bit chunking, the critical section's nesting and its three-cycle cost, `idle()` (wake latency, the six-cycle wake-up penalty, and a counter frozen while the CPU sleeps), the lock-free `Ring` under a 20 kHz ISR producer plus the `EventQueue`'s saturating overflow counter, `now()` across low-byte wraps against the crystal - and test `i`, which RESETS THE BOARD FOUR TIMES on purpose (watchdog time-out, a WDT window violation, a software reset from inside a panic reporter, a plain software reset) and carries its own verdicts across them in a `.noinit` token, so `z` still closes with one ALL: line. Takes about three seconds |
