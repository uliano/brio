...
- **PA8's pull check is INTERMITTENT on this desk (2026-09-03).**
  `test_stm32_tim`'s letter `a` and `test_stm32_exti` both begin by
  proving a pad follows its own internal pull, and PA8's leg failed for
  a stretch of that day and then passed for a stretch, on images the
  md5 gate proved BYTE-IDENTICAL to the ones that scored 105/105 and
  89/89 before. Over SWD, with the core halted and ~20 ms of settling,
  PA8 does follow its pull both ways (IDR bit 8 set under a pull-up,
  clear under a pull-down); the suites allow 200 us. So it is the PAD's
  own RC on this desk and not a regression - but a `test_stm32_tim` or
  `test_stm32_exti` run that scores one short at that verdict should be
  re-run before anything else is suspected.
...
storage day's proof stands untouched; the SAM boards are as described
below (now at 3.3 V), with C's probe temporarily unplugged. THE RTC
DOMAIN IS ON THE LSE CRYSTAL and its calendar keeps running across every
reset and reflash - `test_stm32_rtc` and `test_stm32_sleep` both leave
it there, and `test_stm32_rtc`'s letter `a` wipes the domain (BDRST)
whenever it has to move the clock select, which costs the five TAMP
backup registers. **A DOMAIN THAT HAS BEEN POWER-CYCLED COMES UP
...
PA7 and PA8, and `test_stm32_exti` moves PA0, PB0, PB3, PB7 and PA8;
`test_stm32_lptim` moves PB0, PB5, PB6 and PB7 (LPTIM1's four signals on
AF5) and PA1 (the comparator's precharged input), leaving every one of
them back in analog; `test_stm32_serial` moves PA9 and PA10 (USART1's TX
...
DMA suite's letter `u` and the SERIAL suite's letters `y`, `w` and `v`
are the things on this desk that want a host
...
plus 900 ms before speaking. **THE DMA SUITE'S LETTER `u` LOSES ITS
TALLY TO THAT SAME RACE**, reproducibly: all ten of its legs run and
print their figures (byte-exact to 921600, corrupt at 2 Mbaud - the
documented VCP ceiling), and then the closing "-> N pass" line is read
by a host still at 3 Mbaud and never seen, so the script reports "tally:
None". The letter's own 500 ms of quiet is shorter than the script's
collect phase, which only ends on 300 ms of silence and is extended by
every garbage byte a 3 Mbaud reader makes of a 115200 report. The repair
is the serial suite's rule - wait out the whole announced window - and
it belongs to whoever next owns that suite, for the same
byte-identity reason as the LSI one above. TWO MEASUREMENTS THE ANALOG SUITE MAKES ABOUT
...
C = `twi_peer` (samc), D = `test_samc_i2c` - the I2C campaign's
...
([samc/i2c.md](samc/i2c.md)'s headline, second half).
...
| `spi_peer` | The INSTRUMENT half of the SPI campaign, ONE protocol and TWO firmwares: the avrdx original (board B or A, SPI0 ALT1) and the samc port (board C or D, SERCOM1 function C) - one blocking loop that shifts whatever the DUT clocks, decodes a command frame, acknowledges it and becomes for a bounded moment whatever the DUT needs at the other end. Both are DARK LISTENERS (MISO driven for exactly one answer window, after a frame that checked out) holding the shared select wire up with their own pull-up, and both serve `host_burst` (the roles invert: the instrument clocks a bounded burst so the DUT's client half is testable). The samc port serves an exchange through its own DMA ENGINES by default - hardware reload, boundary 6 MHz - and through its polled one-ahead pump when the command's spare bit (`spilink::spare_polled_pump`) asks, so a suite can measure both; its ident label is the die serial's first word, its regimes map onto PLOADEN (the AVR's BUFEN/BUFWR do not exist here), and the AVR-only experiments (wrcol, feed_tx, mspi) answer cfg_failed. Console (observability only): `?` help, `i` status and counters, `0` back to the dark client, `3` command trace |
...
| `twi_peer` | The INSTRUMENT half of the I2C campaign, ONE protocol and TWO firmwares: the avrdx original (TWI0, PA2/PA3) and the samc port (SERCOM3 fn C, PA22/PA23, core at 48 MHz - the clean wire's bet, won). A blocking loop around a polled client answering exactly the command address (0x6B): decodes a frame, acks (or naks a bad checksum), then becomes what the DUT needs - a client that stretches by a commanded hold per byte (on the samc port the hold is simply the wait before answering: AMATCH/DRDY stretch by construction), NACKs the n-th byte, goes deaf, answers the general call; a stuck client holding SDA from PORT released by counted SCL edges; the bus HOST for a bounded burst (on the samc port a MODE SWITCH - one SERCOM is host or client, never the AVR's combined both). The samc port's ident label is the die serial's first word; its serving of the command channel's reads is the stratum's first exercised client-transmit path (I2cs::end_transaction was born for its closing NACK). Console (observability only): `?` help, `i` status, `0` back to command mode, `3` trace |
