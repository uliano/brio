...
| ch32vx03 | none | by decision, with the heap: what a program on this family gets is the band and its bounds. Two facts would shape a journal here if one were ever wanted - the erased cell reads 0xE339E339 and not 0xFF, which is exactly what the format uses to find the end of a half, and a half-word cell of the zero-wait window TAKES PASS AFTER PASS between erases (measured on both parts, where a cell of the tail given a second pass holds neither value), so the write-once cell the format leans on is not enforced by the array either |
| ch32x035 | none | with the heap: no medium on this series |
...
| rp2350 | none | by decision, with the heap: the `FlashMedia` is there (`rp2350/nvm_flash.hpp`, the top 64 kB of the external chip at 4096 / 256, the RP2040's geometry exactly) and nothing stands on it, so what a program gets is the band and its bounds. Nothing of this silicon forbids a journal - the geometry that carries one on the RP2040 is the same here; what is missing is the decision the NV stack's review owes, and a new family gets the medium and stops there |
| stm32f4 | none | the same reason as the heap's row: the smallest erase grain is a 16 Kbyte sector at the bottom of the bank and 128 Kbytes above it, so a half would be one or two cells and every save would stall the core for a sector's erase; and a second program into a programmed word is accepted here with no PROGERR (measured), so nothing in the array enforces the write-once cell the format leans on |
