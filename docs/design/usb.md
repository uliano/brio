...
|---|---|---|
| ch32vx03 | `Usbd<pma_bytes>` (`ch32vx03/usb.hpp`), ST's F1/F0 device peripheral under WCH's names, over a packet memory the program lays out itself; and `Usbfs<pool_bytes>` (`ch32vx03/usbfs.hpp`), WCH's own host/device block of ch. 23 in device mode, over buffers in the program's own RAM | 512 bytes of shared memory seen through a 32-bit window (one halfword every four bytes) whose top 128 the CAN's filter table takes, so the budget is a TEMPLATE PARAMETER and an endpoint past it is refused; every endpoint register written by XOR, its status and toggle fields inverting on a one while the two completion flags in the same word clear on a zero; COUNTn_RX both the buffer size the program writes and the count the hardware writes back, so arming a reception rewrites it, the control endpoint is laid out again inside the bus reset and guarded against a reception already standing, and a PACKET-MEMORY OVERFLOW IS REPAIRED and not merely counted - it leaves that size field zero and raises no completion, so the endpoint would lose every packet after it; the host's suspend answered with FSUSP, which is the condition for the wake-up event to arrive at all; the 48 MHz the tree's own USBPRE makes, asserted at compile time - and, this family's alone, A MEASURED FLOOR ON HCLK (24 MHz) and A BUS-MASTER COUNT held from the pull-up to the detach, because in a sleep of any depth here the controller cannot reach its packet memory at all; there is NO `Usbd` on the CH32V303, whose class has no device controller of ch. 21 (its clock gate is absent, measured) - its one full-speed controller is ch. 23's host/device block, which `Usbfs` realizes (as it would on the five CH32V203 parts that carry it too): a register file of bytes whose ONE status byte and ONE receive-length register serve every endpoint and are kept standing by the automatic pause the driver sets; a SETUP taken as endpoint zero's whatever that status byte's endpoint field says (it held 6 at an enumeration's first SETUP, measured); the endpoint buffers in the program's own RAM, reached by DMA, out of a pool the driver owns and hands out in claim order (64 bytes a direction), so the budget is again a TEMPLATE PARAMETER; a response the silicon never changes, so every completion is answered NAK before its flag is cleared; the data toggle kept in the endpoint's register and flipped by the driver, a repeated OUT acknowledged by the silicon and dropped by the driver; the pull-up and the device function one field, and every event of a detached block dropped, the SE0 a detach leaves reading as a bus reset; the same 48 MHz, a measured floor of its own (12 MHz of HCLK), and the same bus-master count from the pull-up to the detach, because with the core asleep this block sends IN packets full of zeros and never gets an enumeration past its descriptors |
