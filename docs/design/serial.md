...
transport: SPSC rings on both sides, ISR bodies (`rxc()`, `dre()`)
bound by the app, RXDATAH error counters (named statics, inspectable
from gdb). Ring defaults 64/256 (8-bit indices both sides). The driver
...
TX stays the blocking try_put print: the drain side is an ISR (it
...
Measured cost of write_byte: ~45-50 cycles/byte, under 10% of the
21.7 us wire time per byte.
