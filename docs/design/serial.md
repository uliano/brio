...
| ch32v00x | `Uart<n, P, rx_size, tx_size, TxEngine, RxEngine, remap>` (`ch32v00x/usart.hpp`) | the pins are the instance's own under a REMAP CODE (`afio.hpp`'s tables), the code a template parameter; one vector and `isr()`; the same engine slots with `dma_isr()`, `dma_faults()` and `harvest()`, on the channels table 8-2 gives the instance (the channel IS the request on this family); USART2 refused at code 0, whose TX is the K8 package's reset pin; BRR is PCLK over the baud, whole - no fractional divider and no kernel-clock multiplexer |
