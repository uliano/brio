...
Common to the seven: seventeen verbs spelled identically - `init(clock,
...
| rp2040 | `Uart<n, pins, rx_size, tx_size, TxEngine, RxEngine>` (`rp2040/uart.hpp`) | the PL011 with its 32-deep FIFOs: the pins are GPIO numbers under function 2, and table 279 decides which of them carry which instance; one vector and `isr()`; `write_byte()` PENDS THE LINE in the NVIC instead of writing the FIFO, because the PL011's transmit interrupt is a transition and not a level; the divisor's integer and fractional halves are latched by the LCR_H write that follows them; the same engine slots (`rp2040/dma.hpp`'s `DmaTxEngine`/`DmaRxEngine`, on any two distinct channels) with `dma_isr()`, `dma_faults()`, `harvest()`, `write_bulk()`/`read_bulk()` - the receive run read off TRANS_COUNT by `harvest()`, the FIFO emptied and the credits cleared before a run ([../rp2040/uart.md](../rp2040/uart.md)) |
| ch32v203 | `Uart<n, P, rx_size, tx_size, format, TxEngine, RxEngine, remap, opts>` = the task over `Usart<n>` (`ch32v203/usart.hpp`) | the pins are the instance's own under a REMAP CODE (`afio.hpp`'s columns, the code a template parameter, and a code of 0 writes no register at all); the FRAME is a template parameter of its own here, ahead of the engines, where the CH32V00x carries it inside its options; one vector and `isr()`; the same engine slots with `dma_isr()`, `dma_faults()` and `harvest()`, on the channels table 11-5 gives the instance (the channel IS the request on this family too, an engine elsewhere refused at compile time); `UartOptions` as the trailing parameter (a single wire that is a BUS and not a loop, the flow-control pair on a FULL instance); the divisor is the instance's OWN bus over the baud, whole - USART1 asks PCLK2 and the other three PCLK1, both below HCLK - and WHICH instances a part offers is the datasheet's table and not a header's; the fourth port is a USART4 and not a UART4, so the smartcard and the synchronous clock exist here ([../ch32v203/usart.md](../ch32v203/usart.md)) |
