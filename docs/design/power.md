...
| ch32vx03 | `Ch32vx03SleepSite` and `Ch32vx03TimedSleepSite` over `Pwr` (`ch32vx03/sleep.hpp`) | `none` AND `light` -> Sleep (this family has nothing between Sleep and Stop), `standby` -> a Stop on the MAIN regulator, `deep` -> the same Stop on the LOW-POWER one - one mode with two prices (measured: the second costs 30 us more at the wake), so `armed()` answers from the mirror the site wrote and `deep` for a Stop somebody else armed; Standby is off the ladder AND ENFORCED off it, every `arm()` clearing PDDS, because EVERY exit measured on this silicon is a power reset - the RTC alarm's line as an interrupt, the same line as an event alone, and even a PAD'S line as an event, which the chapter does not list as an exit at all; a SYSCLK restore rides `disarm()` (a Stop leaves the core on the HSI with the PLL off, and putting a PLL rate back takes about 2 ms of crystal start-up) and the tick is paused by the PLATFORM's idle hook rather than by the site, because with this core's WFE idiom a merely PENDING tick would end the sleep before it began - at the price of one dropped tick, which the timed site's witness repairs. AND THE ONE THING NO OTHER TARGET HAS: a sleep of any depth starves every bus master but the core here (measured), so the site REFUSES every rung but `none` while `bus_masters_active()` is not zero - a DMA channel while its EN is up and the USB controller from its pull-up to its detach each hold one count - and the kernel's idle path makes the same test before it sleeps at all, which is what turns "a program with USB never idles" from a rule into a mechanism. The timed site takes the RTC's counter as alarm AND witness on a 1024 Hz tick out of the crystal, the counts rounded up and the span down so a wake is late and a resync short: a 500 ms deadline met at 502 ms of wall and of kernel tick |
