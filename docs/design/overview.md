...
**Pins.** Common to all: `Pin<'A', 5>` and `PinRef`, `set` / `clear` /
`toggle` / `read` / `output` / `input(PinPull)` / `port` / `ref`, and
`duty()` (a `Pin` is a `PwmChannel` of one step). Beyond that,
`pull(PinPull)` and `is_output` are on every stratum whose pad carries
its pull in a register of its own - the QingKe families put it in the
OUTPUT register and have neither - and a `PinConfig` carries whatever
else a pad can be told, as a `configure(...)` verb on the AVR, the SAM
and the two ST families and as an argument to every configuring verb on
the two RP ones, where the pad register is written whole. What differs
is what a pad can be told to do, and how it is handed to a peripheral.
...
| ch32vx03 | `Pin` in `ch32vx03/pin.hpp` | `function(PinDrive, PinSpeed)` takes NO selector, as on the sister family - which peripheral a pad carries is the remap register's (`ch32vx03/afio.hpp`); the MODE field IS the speed, so every output verb takes one (50 MHz by default); `input(PinPull)` pulls through the output register; `release()` is the reset state, a FLOATING INPUT, and `analog()` a separate landing; `nibble()` reads a pin's four bits back; the whole-port `out_write` and `configure_pins(mask, nibble)`; the CONFIGURATION LOCK no other stratum has (`lock<Mask>()` / `lock(mask)` / `locked()` / `locked_pins()`, one way until a reset); `PinRef` with `read`/`write`/`toggle` beside `set`/`clear`; no `pull()`, `is_output` or `PinConfig` |
| ch32x035 | `Pin` in `ch32x035/pin.hpp` | `function()` takes NO argument: MODE carries no speed on this series and there is no open-drain output, so neither the sister family's drive nor its speed exists, and which peripheral a pad carries is the remap register's (`ch32x035/afio.hpp`); `input(PinPull)` pulls through the output register and ANSWERS - false for a pull-down on a pad that has none (all but PA0..PA15, PC16, PC17), `input<PinPull>()` refusing it at compile time; `output()` and `function()` refused at compile time on a pad sharing its package pin with another (the datasheet's notes 4 to 7); `release()` the floating input and `analog()` a separate landing; `nibble()`, the whole-port `out_write` and `configure_pins(mask, nibble)`, the one-way lock; ports of up to twenty-four pins, so `out_set` is two stores across the halves (BSHR and BSXR), and CFGHR is written only from a RAM copy and never read to configure (`Port::cfghr_copy()`) |
...
| ch32vx03 | `TimPwm<Tim, ch, top>`, `TimPairPwm<Tim, ch, top>` (the complementary pair with dead time, on the advanced timers alone - TIM1 everywhere, TIM8, TIM9 and TIM10 on the CH32V303RC and VC; the general-purpose and basic timers of this family have no break unit and no complementary output) (`ch32vx03/tim.hpp`) | the G0's two names on the F1's timers; the dead time is counted in tDTS - TIMxCLK divided by CTLR1.CKD and NOT by the counter's prescaler - so a pair holds the same absolute dead band at every PWM frequency; the timers take a static clock alone, as on the G0, and what they count is HCLK at every rate this stratum makes |
| ch32x035 | none | the timers are not written in this stratum; `Pin` is the one-step channel |
