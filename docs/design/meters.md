...
The meters themselves are each target's. Nothing here configures
hardware or names a timer.
...
| ch32vx03 | `TimPeriodMeter<Tim>` (period and width, PWM input mode on TI1, both channels) and `TimIntervalMeter<Tim, ch>` (`ch32vx03/tim.hpp`) | the G0's two names on the F1's timers again, with NONE of the sister family's additions - no CAPLVL, no CAPOV, no dual-edge capture on the CH32V203 (that register is the CH32V30x_D8's, and a lot's: `dual_edge_capture()` asks the die, and the CH32V303VCT6 measured had none); what this family offers a meter with nothing attached is a pad the PORT can drive, the alternate-function input being the pad's own input buffer, so the CPU makes the edges a capture measures ([the target's document](../ch32vx03/tim.md)) |
| ch32x035 | none | TIM1, TIM2 and TIM3 (RM ch. 12, 13) are not written in this stratum - TIM2 here an advanced timer with four vectors |
