...
| ch32vx03 | `Adc<n>` for the two converters and `AnalogIn<Pin>` deriving the channel from the pad (`ch32vx03/adc.hpp`); no DAC on this family; `Ref` in `ch32vx03/adc.hpp` | no package of the series brings out a VREF+ pad, so `Ref` is one word, `vdda` - the analog supply IS the scale - and `ref_mv()` takes the board's millivolts as on the RP2040, with `Adc<1>::vdda_mv()` measuring it from VREFINT and no factory word to sharpen it (the electronic signature holds the flash size and the unique id alone); the two internal sources are TAGS on their channel numbers and ONE bit wakes both, written AFTER the calibration because it forces the input buffer on for good; an INJECTED group of four preempts the regular one with a signed offset, as on the STM32F4; the input buffer carries a PROGRAMMABLE GAIN of 1, 4, 16 or 64, WCH's own addition to the F1's chapter; and the DUAL modes put two converters in lockstep with the follower's datum in the upper half of the master's register - a capability no other stratum here has, so it stays this driver's verb and earns no util shape ([the target's document](../ch32vx03/adc.md)) |
