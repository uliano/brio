...
| ch32v203 | none yet: `DmaTxEngine` and `DmaRxEngine` alone in `ch32v203/dma.hpp` | eight channels, the channel is the request again (no multiplexer), and a HARDWARE CIRCULAR MODE - so the player would be the G0's shape and the source the same pair of halves re-armed from the block-complete interrupt; born with their first block user, the ADC. One family fact shapes both before they are written: in Sleep no bus master but the core gets a cycle, so a stream on this family holds the program awake or it stops ([the target's document](../ch32v203/dma.md)) |
