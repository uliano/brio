...
| ch32x035 | `Ch32x035Platform<TB>` (`ch32x035/platform.hpp`) | the CH32V203's file on the QingKe V4C, kept close enough that a `qingke/` core stratum is a diff of facts: templated on its timebase, `atomic_width` 4, the critical section a `csrrci` on mstatus.MIE, `idle()` the WFE of the sister strata for their reason - the QingKe V4 manual's WFI wording being the same, unmeasured on this core - with a pause of the timebase when SLEEPDEEP is found set; NO count of bus masters, because no driver of this stratum starts one, and no `idle_until()`; `break_here()` is `ebreak` ([../ch32x035/platform.md](../ch32x035/platform.md)) |
