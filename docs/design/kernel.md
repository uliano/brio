...
own nature: `Reset::take_flags()` on avrdx, stm32g0, ch32v00x, ch32vx03
and stm32f4 (a
history that ACCUMULATES until read and cleared) and `Reset::cause()`
on samc21 (RCAUSE, one exclusive cause), while the two RP families read
theirs in two places at once - a chip-level word that stands for the
life of the supply beside the watchdog's own REASON. And the watchdog a
program
keeps alive is a resource per stratum under THREE names - not one verb,
