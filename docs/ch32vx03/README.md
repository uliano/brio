...
- **A CH32 I2C target loses the last byte before a late repeated
  ...
  that byte, in silence (the CH32V303VCT6's two controllers on one bus,
  at every count and both speeds); requested while that byte still
  shifts, as WCH's own example does, every byte arrives, and that is
  where the engine requests it ([i2c.md](i2c.md)).
