...
| [usart.md](usart.md) | The serial ports: up to EIGHT instances on two peripheral buses - USART1 on PB2, every other port on PB1 - every divisor asked of the instance's OWN bus, and WHICH instances a part offers taken from the datasheets' tables (one on the smallest CH32V203, four on the CH32V203C8 and RB, three on the 128 KB CH32V303, all eight on the CH32V303RC and VC, UART6..UART8 below USART2 in the map); the frame in every shape M and STOP allow, mute mode with both wakes, LIN's break, single-wire half duplex, IrDA, THE SMARTCARD and THE SYNCHRONOUS CLOCK of a FULL instance - the fourth port a USART4 on the CH32V203C8 alone, the full-USART verbs compile errors on a UART - the flow-control pair, the two DMA requests as SLOTS (UART4..UART8's on DMA2 on the CH32V303), every flag with the sequence that clears it and both interrupt sources; the pads are afio.hpp's COLUMNS, a remap code of 0 writes no register at all and the two columns on the debug port's pads are refused while the probe's port is alive; the CH32V303's lot-keyed CTLR4, M_EXT, MS_ERR and RX_BUSY as verbs that ASK THE DIE, CTLR4's address read as a word first; then the transport, with its frame, its two engine slots and its trailing options. Measured on the CH32V203C8T6: twenty-eight baud rows within one per mille on both buses with 1200 refused from the 144 MHz one (the register's sixteen bits, not the generator's floor), a start bit that is the divisor to the clock at four rates from 9600 to 3 Mbaud, the word length as a frame's low run, THE TWO HALF STOP CODES COMING OUT AS WHOLE ONES, TE's idle frame at 1146 us with TXE back in 104 us on an idle transmitter, PE delivering its byte and ORE keeping the data register's, both mute wakes with 18.10.4's two notes, a break of ten bits outside LIN and thirteen inside it with the detector at ten and eleven, THE INFRARED DECODER TAKING THE MIRROR OF ITS OWN ENCODER'S PULSE, seven clock pulses a byte with LBCL clear and eight with it set - the F1's sense and not this manual's words - a card clock of 6001500 Hz for the 6 MHz asked and only with the transmitter enabled, and the CTS hold with the RTS hand; on the CH32V303VCT6 the same letters with the same numbers, UART5..UART8 each with its gate, divisor, pads and vector (69, 87..89) and no wire, the crossed pair USART2-UART4 byte-exact, THEIR ENGINES ON TWO CONTROLLERS - DMA1 against DMA2 - carrying four kilobytes each way at 921600 baud in 44378 us with nothing counted, the debug-port columns refused with SW_CFG reading the probe's port alive, and a die with none of the lot's four features, the probe finding CTLR4's address empty and moving nothing and the WIRE agreeing - the parity bit computed and the word eight bits long with both stored raw |
| [spi.md](spi.md) | SPI and I2S: up to three synchronous serial ports - SPI1 on PB2, SPI2 and SPI3 on PB1, so one BR code is two frequencies and a program asks the INSTANCE - the STM32F1's SPI under WCH's names with no FIFO at all (one buffer each way, which is what makes a host's pump run on RXNE and a client answer ONE FRAME AHEAD), the four modes, both widths, both bit orders, the three chip-select arrangements, the simplex and one-wire line modes, the hardware CRC, the two DMA rows a SLOT each (SPI3's on DMA2), the pads as afio.hpp's COLUMNS, the high-speed read at BR /2 alone - the one code no lot of any class reads differently - with the lot's HSRXEN2 a verb that asks the die, and on the CH32V303RC and VC THE I2S FACE of SPI2 and SPI3 (the four standards, the widths, both roles and directions, the divider on SYSCLK, the flags and requests the faces share); then the host engine with the other strata's Request verbatim, the client with its dark listener, and a pad-slew verb on both. Measured on the CH32V203C8T6: eight rising edges per frame at every BR code, counted by a timer on the SPI's own clock pad; a strap clean to 36 MHz and breaking at 72; four kilobytes byte-exact; the hardware CRC equal to a bitwise reference; the engines at 36 core cycles a frame against the wire's 32; the arbiter's four transactions, its rejection and both votes - and against a SECOND BOARD on SPI2's four wires: all four modes and both bit orders exact both ways, the dummy byte of a client that did not preload, a link exact to 1.125 MHz whose break is the far end's ANSWER RELOAD and not the wire, the roles inverted with this chip as the client, a ten-second stress with no error at either end, and TWO THINGS THE CHAPTER DOES NOT SAY - that 20.2.7's recipe for clearing MODF does not clear it (fourteen sequences measured, WCH's own library on the same board among them; SPE never takes while the flag stands, and only the block's reset line puts it down) and that the CLIENT'S ANSWER EDGE is a correctness parameter in the two modes that sample on the falling edge. On the CH32V303VCT6, over four wires between SPI2 and SPI3: SPI3 as client and as host exact in every mode, order and width; THE HIGH-SPEED READ turning 36 MHz from 254 bytes of 256 wrong - the client's answer read ONE BIT LATE - to none; half-words through four channels on two controllers under one per cent over the wire's own cost at every rate; and the I2S face word for word in all four standards, both roles, both directions, every width and on DMA at 48 kHz, its clock SYSCLK to the frame (16015 counted for 16014), the first word the left channel's, and TXE READING 1 WITH I2SE CLEAR against 20.3.6.2, a datum written there going out first |
| [i2c.md](i2c.md) | The two-wire ports: the STM32F1's I2C under WCH's names, host and target, with a rise-time register the CH32V00x's block has not - so the SCL timing is THREE registers (FREQ, CKCFGR under F/S and DUTY, RTR from the WIRE's rise) and the chapter's own ceiling is FREQ's six bits: 4 to 60 MHz of PB1, which is the one peripheral of this family that cannot be used at the top of the tree. The event machine's every sequence, the receive procedure by count, 7- and 10-bit own addresses with the dual address and the general call, ACK and POS, SMBus and PEC as bits, the two DMA rows a channel each (a one-byte read stays on the pump), two vectors an instance, the pads as afio.hpp's COLUMNS with the SMBus alert pad outside them; then the host engine with the other strata's Request verbatim, the arbiter over it, and the client with a polled option and a flush(). Measured: both speeds exact at 48 MHz of PB1 and SCL read off its own pad by a timer (9989/2489/2593 ns of period, the 40 ns each high half is short of telling the wire's rise), NO enable protection at all - CKCFGR, FREQ and RTR all take a write with PE set, against 19.12.9's own sentence - an absent address as i2c_nack_addr with BUSY back in 6 us, a held SDA answered i2c_arb_lost by the silicon and unstick()'s three clocks counted on the pad, the four receive procedures byte-exact against a second chip, both duty shapes and both speeds byte-exact (the wire carrying 87 and 251 kHz, the target's stretch and not the register's rate), both DMA engines, the kernel's arbiter with a held clock answered i2c_timeout at its 20 ms limit, ARBITRATION IN BOTH DIRECTIONS and this board as the far controller's target, 25100 tenures in ten seconds with no error at either end - and two facts a recovery has to know: a DMA-served read can leave BUSY standing over an idle wire (19.12.1's own case for SWRST, and SWRST takes CTLR2's interrupt enables with it), and clearing PE does NOT let a stretching machine's pads go, only the block's reset line does; on the CH32V303VCT6, whose board wires I2C2 to I2C1, the block's numbers again and the chip's two controllers each host and target of the other - THE TARGET AS A TRANSMITTER through every receive procedure, flush() taking the extra byte each read asks for, the dual address and the general call, the DMA host's three shapes, I2C2 on a wire as the host, a target stuck mid-byte clocked out by unstick() in 4 pulses - and, against a CH32V203C8T6 board, every peer letter, ARBITRATION BETWEEN TWO CH32 CONTROLLERS in both directions among them; THE REPEATED START TAKEN WHILE THE LAST BYTE GOES OUT: requested after BTF, the CH32V303VCT6's own target lost the last written byte of every write-then-read in silence where a CH32V203C8T6 target took every one, so the engine requests it a byte earlier, where WCH's own example does; and A START ON A BUSY BUS LEAVES AT THE CONTROLLER'S TICK - about 85 us after the edge that made the bus busy and every 80 us after, not at the STOP - while one set on a free bus is committed 5 us ahead |
...
| [sleep.md](sleep.md) | PWR and the two sleep sites: three low-power modes behind one pair of bits - SLEEPDEEP, which is the CORE's, and PDDS, which is this block's, written and read as ONE PAIR - with a Stop that is ONE MODE WITH TWO PRICES (the regulator's low-power setting and the RAM's low-voltage one, whose interlock the chapter states and the driver enforces) and a Standby that is OFF THE LADDER because every way out of it is a reset; the supply monitor whose eight thresholds are worth TWO DIFFERENT TABLES of millivolts by a bit of the DIE and not of the part number, the wake-up pad PA0 with the one flag it shares with the RTC alarm, what a Standby keeps of the RAM (the class's two readings of one bit, a number that is the class's largest array and not every part's, and on the CH32V303VCT6 bits that READ BACK ZERO and still act, so the driver keeps the program's choice and every store of the register carries it), the regulator trims that live in the extended register, and the debug module's three low-power bits READ AND NEVER WRITTEN because a store to that CSR resets the part. Then the mapping onto [the power model](../design/power.md) - light to Sleep, standby and deep to a Stop on each regulator, the clock tree put back after one and the tick paused across it by the idle path - and THE COUNT OF ACTIVE BUS MASTERS, this family's answer to a sleep that starves every master but the core: a DMA channel of either controller and an attached USB controller each hold one, the kernel's idle path sleeps only at zero and a site refuses to arm above it, so "a program with USB never idles" becomes a mechanism. Plus the timed site, the RTC's alarm as the wake and its counter as the witness of the frozen span. Measured on both parts: an idle() that RETURNS AT ONCE over a working master in thirty to forty core cycles, a 65535-item block running on across eight of them (on the CH32V303VCT6 a channel of DMA2 counted and refused as well), and the CDC console enumerating and carrying bytes over each part's own controller with the ordinary kernel loop, where a bare WFI loses every packet on the CH32V203C8T6 and sends zeros on the CH32V303VCT6; a 200 ms Stop woken by the alarm with SIXTY MICROSECONDS of wake (the block raising its event one count of the ruler late - awake as well on the CH32V203C8T6, in the Stop alone on the CH32V303VCT6 - measured and subtracted), 21 to 30 us of RTC resynchronization owed at every wake, the tick advancing by none of the sleep and the tree back at the PLL rate within two milliseconds; 30 us (the CH32V203C8T6) and 61 us (the CH32V303VCT6) MORE at the wake for the low-power regulator, over sixteen Stops of each; a 500 ms deadline met at 502 and 501 ms of WALL and of kernel tick through the timed site under a real kernel; the vote round, the standing lock, the deadline guard and the refusal that is the silicon's over a running channel; a pad's line ending a Stop from outside on the CH32V203C8T6, and on the CH32V303VCT6 a wire's edge ending a Sleep in 501 us with its handler 30 timer cycles behind the edge, a Stop at every rate of a DynamicClock with the rate in force put back, and all thirty-two f-registers and fcsr through a Stop bit for bit; THREE STANDBYS ALL ENDING IN A RESET on each part - the alarm's line as an interrupt, the same as an event alone, and on the CH32V203C8T6 a PAD'S line as an event, which 2.3.4 does not list as an exit at all - with SBF at every boot, WUF at the alarm's alone, PORRSTF and never LPWRRSTF, and the .noinit token surviving; WHAT A STANDBY KEEPS on the CH32V303VCT6, each retention bit its own bank whole and nothing above 32 KB - and nothing at all with the bits written by separate read-modify-write stores; and THE INDEPENDENT WATCHDOG frozen through a Stop on both parts, its reset arriving 215, 208 and 209 ms after two-second sleeps it never interrupted, and ENDING a Standby on the CH32V303VCT6 225 ms after the alarm meant to end it was placed, IWDGRSTF beside PORRSTF |
| [usb.md](usb.md) | The CH32V203's USB device controller - the CH32V303 has none, its one full-speed controller being [usbfs.md](usbfs.md)'s: ST's F1/F0 device peripheral under WCH's names, realizing util/usb's UsbController at the packet - eight endpoint registers whose status and toggle fields are WRITTEN BY XOR while the two completion flags in the same word clear on a zero, a packet memory of 512 bytes seen through a 32-bit window (one halfword every four bytes of address space) whose TOP 128 THE CAN'S FILTER TABLE TAKES, so the budget is a template parameter and a CDC port spends 328 of the 384 that are left; COUNTn_RX both the size the program writes and the count the hardware writes back over it - which is why arming a reception rewrites it, why the bus reset guards against one already standing, and why AN OVERFLOW IS A REPAIR and not only a count, the lost packet leaving the size field ZERO and no completion behind it; the host's suspend answered with FSUSP, because a module left running never hears the wake-up; the pads that are still port A's GPIO pads with the pull-up in another chapter's register; one of the two vectors used and the other left to the double buffer this driver does not offer - and TWO COMPILE-TIME REFUSALS, the 48 MHz the tree's USBPRE must make and the measured floor of the bus under it. Measured: an enumeration to CONFIGURED in 362 to 628 ms with 328 of 384 bytes spent, ten frames in 10 ms and no overflow; a thousand idle() calls costing 0 to 1 ms over the bus-master count an attachment holds; a reconnect answered with three bus resets and the SAME address; 2.87 million bytes echoed byte-exact in ten seconds, 767 KB/s out of the bulk pair and 461 KB/s into it with no ring overrun; the line coding read back as 19200 7E2 with DTR and RTS raised; a host's suspend and its resume both counted, where a module left running reported the suspend alone; and, staged with a bare wfi under a full-speed pour, 616 packet-memory overflows in 50 ms that carried nothing where the awake window carried 22 kB - the endpoint repaired and taking 95 kB in the 200 ms after |
| [usbfs.md](usbfs.md) | The USB host/device controller of ch. 23 in device mode - the CH32V303's one full-speed controller and five CH32V203 parts' second, on the pads the part table names (PA11/PA12, PB6/PB7) - realizing util/usb's UsbController at the packet: WCH's own design and nothing of ST's, a register file of BYTES with eight endpoint numbers whose mode nibbles pack two to a byte, a buffer address, a transmit length and two response bytes each, and the endpoint buffers in the program's own RAM, reached by DMA out of a pool the driver owns and hands out in claim order; ONE status byte and ONE receive-length register for every endpoint, kept standing by the automatic pause the driver sets; a SETUP taken as endpoint zero's whatever the status byte's endpoint field says; a response the silicon never changes, so every completion is answered NAK before its flag is cleared; the data toggle kept in the endpoint's register and flipped by the driver, a repeated OUT dropped; the pull-up that is also the device enable, and every event of a detached block dropped; the wake-up line of each class; and the 48 MHz and a measured floor of HCLK refused at compile time. Measured on the CH32V303VCT6 on the board's own connector: the pair PA11/PA12, reached with port A's clock shut; an enumeration to CONFIGURED in 533 to 565 ms with 256 of the 960 pool bytes spent; the status byte naming endpoint 6 at a SETUP - a driver that believed it dropped every SETUP of an enumeration - and RX_LEN reading 1022 there, and the buffer register reading back the address's low bits; no bus reset reported over the SE0 a detach leaves once the driver drops a detached block's events, and a reconnect in 362 to 570 ms with the same address; the floor at 12 MHz of HCLK (96 to 12 carrying an enumeration and a pour whole, 6 losing bytes of the pour in three runs of seven with no flag, 1.5 carrying neither); the host's frames counted one a millisecond through the interrupt bit the manual calls reserved; three million bytes echoed byte-exact at 339 kB/s, 768 kB/s out and up to 609 kB/s in; a staged toggle mismatch costing exactly one packet; the pause holding the status byte through a held vector where without it 248 to 297 packets overran it in 20 ms; with the core in a bare wfi the OUT packets landing whole while the IN packets carried ZEROS and an enumeration stopped at its descriptors - which is why the controller holds a bus-master count from its pull-up to its detach; and a host's resume raising EXTI line 18 once and line 20 never |
...
has no device one, so a USB console here runs on the host/device
controller in device mode ([usbfs.md](usbfs.md)).
...
The chip's own USB carries the console too, as a CDC ACM port with
pid.codes' test identity 1209:0001 - on the WeAct board through the
device controller on its USB-C ([usb.md](usb.md)), on the evaluation
board through the host/device controller on P14 ([usbfs.md](usbfs.md)):
one stack and one class over two controllers, the part table choosing
which.
...
- **The CH32V303 has no USB device controller**, against RM ch. 21's
  opening, which applies that controller to the whole family. Its clock
  gate, bit 23 of RCC_APB1PCENR, is not there: that register written
  with all ones reads back 0x3A7EC9FF, every gate the manual names for
  this part answering and bit 23 alone of them silent, and the block's
  registers read zero. The part's one full-speed controller is the
  host/device one of ch. 23 (its control register reads its reset value,
  0x06), on PA11/PA12 - which the datasheet's alternate-function table
  gives the OTG_FS pair and WCH's own board labels USBFS. So `has_usbd`
  is false in its part table, and a USB console here runs on the
  host/device controller in device mode ([usbfs.md](usbfs.md)) - the
  same stack, class and descriptors over another controller, which
  drives exactly that pair (measured).
- **THE USB CONTROLLERS DO NOT SURVIVE THE CORE'S SLEEP**, and the
  manual says they should. Chapter 2's table gives Sleep as "core clock
  ...
  device controller cannot reach its packet memory. An armed bulk
  endpoint receives nothing - the host's bytes are lost and the
  ...
  idiom. WCH's own reference implementation fails the same way, which
  is what settles whose the finding is: the EVT's SimulateCDC example,
  built with the vendor's compiler and flags and unchanged but for one
  `wfi` at the end of its loop, never reaches CONFIGURED - SET_ADDRESS
  gets through, the descriptor reads do not, the overflow counted 18 to
  26 times - and the vendor's own `__WFE()` fails the same, while a
  busy-wait of the same length in the same loop works; a variant that
  sleeps only once configured loses every one of the 4096 bytes the
  host writes; and the loss happens with the core woken at least once
  per USB frame (the frame interrupt at 1 kHz and a 10 kHz timer
  armed), so it is neither the depth nor the length of the sleep. The
  debug module's keep-HCLK-in-Sleep bit could not be tried: a `csrw` to
  CSR 0x7C0 from the running program resets the part. On the
  CH32V303VCT6 the host/device controller, whose buffers are the
  program's own RAM, fails the other way round
  ([usbfs.md](usbfs.md)): with the core in a bare `wfi` between packets
  the host's OUT packets landed whole, but the IN packets the block read
  out of RAM carried ZEROS - some thirty per cent of the bytes of a
  counting stream broken, with no error at either end - and an
  enumeration stopped at the descriptor reads after SET_ADDRESS. A
  program that uses USB therefore does not SLEEP, on either part, and
  it is the platform that enforces it: an attached controller counts
  itself a bus master and `idle()` does not sleep while the count
  stands ([sleep.md](sleep.md)).
- **In Sleep, no bus master but the core gets a cycle.** A
  ...
  slows down, to no less than 24 MHz of HCLK under the CH32V203's device
  controller and 12 MHz under the CH32V303's host/device one
  ([usbfs.md](usbfs.md)). And that is a MECHANISM and not a rule the
  programmer keeps: [sleep.md](sleep.md).
...
- **The CH32V303VCT6 of the bench behaves as a lot the manual's lot
  ...
  USART_CTLR4's MARK and SPACE parity, CTLR1's M_EXT, STATR's MS_ERR and
  RX_BUSY, and SPI's HSRXEN2 - and the one restriction the notes give
  the other lots is present: DMA1 wraps a span crossing a 64 KB boundary
  inside the page it started in. A program cannot read its lot - the
  notes key on digits of a lot number no register states - so each
  feature is a verb that ASKS THE DIE, writing its bit and reading it
  back before anything else (TIMx_AUX kept no bit on TIM1 and TIM2
  alike, and the dual-edge verb answered false), and the restriction is
  kept on every lot ([dma.md](dma.md), [adc.md](adc.md),
  [tim.md](tim.md), [opa.md](opa.md), [usart.md](usart.md),
  [spi.md](spi.md)). CTLR4 and M_EXT were asked of the wire as well,
  stored raw past the verbs: the parity bit stayed computed and the word
  eight bits long, so they are absent and not write-only.
- **An absent register can be a MIRROR.** On that die EXTEN_CTR2's
  ...
  write ([opa.md](opa.md)).
- **PWR_CTLR's RAM-retention bits read back zero on the CH32V303VCT6,
  and act.** Bits 16 to 20 - R2KSTY, R30KSTY, their two VBAT twins and
  RAMLV - read 0x0 whatever was written, from the program and over the
  debug link alike, while a Standby keeps exactly the banks they name.
  So every read-modify-write of the register writes them away: with the
  bits set by one store and the next store made from a read, a Standby
  kept not one word in any bank. The register therefore has ONE store in
  the stratum, which carries the program's choice from a copy in RAM -
  the RTC chapter's DBP included - and the retention verbs answer that
  copy. On the CH32V203C8T6 the same bits read back
  ([sleep.md](sleep.md)).
- **A SETUP's status byte names no endpoint.** On the CH32V303VCT6's
  host/device controller the endpoint field of the status byte at a
  SETUP read 6 - whatever the last IN or OUT left there, or its reset
  garbage - and the receive length beside it 1022; a driver that
  believed the field dropped every SETUP of an enumeration, the setup
  packet itself sitting whole in the buffer the whole time. The chapter
  does not say so, and WCH's own sequence never looks
  ([usbfs.md](usbfs.md)).
- **The independent watchdog stops in a Stop and ends a Standby.** On
  both parts it does not count through a Stop - its reset arrives some
  200 ms after the WAKE, and on the CH32V303VCT6 with the debug module's
  freeze bits read clear - against 7.2.1's promise that it keeps working
  there; on the CH32V303VCT6 it ends a Standby as a reset, 225 ms into
  one an alarm would have ended at two seconds, IWDGRSTF beside PORRSTF,
  as 2.3.4 lists it. A program that sleeps deep still arms the RTC's
  alarm as its way back
...
- **A CH32V303VCT6 I2C target loses the last byte before a late repeated
  START.** With the host's START requested after BTF - the last written
  byte acknowledged and SCL held low - that target never raised RxNE for
  the byte, in silence (the chip's two controllers on one bus, the
  target polled by the core that also serves the host, at every count
  and both speeds); a CH32V203C8T6 target on another board, under a
  CH32V303VCT6 host, took it in all twelve of the suite's late-START
  probes, whether it took a standing byte before answering the address
  or after. Requested while that byte still shifts, as WCH's own example
  does, every byte arrives at both, and that is where the engine
  requests it ([i2c.md](i2c.md)).
- **An I2C START on a busy bus leaves at the controller's tick, not at
  the STOP.** With SDA held low and SCL standing high, a pending START
  reaches the wire about 85 us after the edge that made the bus busy, or
  a multiple of 80 us after that; one set on a free bus, or pending when
  a STOP frees it, reaches SDA some 5 us later whatever the wire does
  meanwhile. Two controllers' STARTs meet only where one STOP releases
  both or the second is set within those 5 us, and a tenure into a line
  a stuck target holds is answered at once instead of parked (measured
  on the CH32V303VCT6, a CH32V203C8T6's START on the same beat;
  [i2c.md](i2c.md)).
## Not covered yet
...
- **The lot-keyed features on a die of the other lots.** TIMx_AUX's
  ...
  EXTEN_CTR2's high-speed bits, ADC2's DMA request, USART_CTLR4's MARK
  and SPACE parity, M_EXT's short words, MS_ERR, RX_BUSY and SPI's
  HSRXEN2 are verbs that ask the die, and the CH32V303VCT6 answered no
  to each; what would measure them is a CH32V303 of a lot whose
  penultimate sixth digit is not zero.
- **The host/device controller on the CH32V203's pads.** Five CH32V203
  parts carry it on PB6/PB7 - the CH32V203C8 among them, beside its
  device controller - and `Usbfs` builds for all five; the WeAct board
  wires its one connector to the device controller's pads, so what
  would measure it is a board with a connector on PB6/PB7
  ([usbfs.md](usbfs.md)).
