...
- **The CH32V303's bus in Sleep is the CH32V203's.** A memory-to-memory
  ...
  ([sleep.md](sleep.md)), and its second controller starves the same
  way, 23 bytes across one idle() and 120 across ten on DMA2's first
  channel.
...
- **The CH32V303VCT6 of the bench behaves as a lot the manual's lot
  ...
  A program cannot read its lot, so each feature is a verb that ASKS THE
  DIE and the restriction is kept on every lot ([dma.md](dma.md),
  [adc.md](adc.md), [tim.md](tim.md), [opa.md](opa.md)).
