...
| QingKe V4 microprocessor manual | **V1.1** | `QingKeV4_Processor_Manual.PDF` | the core: table 1-1 for the V4C against the V4A, V4B and V4F, 2.2 for what a trap does to mstatus, 3.1 for the PFIC register set, 3.2 for INTSYSCR and mtvec, 3.4 for the hardware prologue, the chapter on the system counter, 6 for the sleep modes and what ends them, 8.3 for corecfgr - the register WCH's startup writes and no manual describes. A V1.5 waits among the candidates of the sibling stratum's folder ([../../ch32vx03/vendor/README.md](../../ch32vx03/vendor/README.md)) |
...
`docs/ch32x035/` is where this series' errata will be written. NO SVD:
the EVT package ships none, so the build project has no `svd/`.
...
unless the other is evidently the corrected one, and nothing below has
been measured.
...
- **RCC_RSTSCKR's reset value, manual against manual.** The register
  ...
  QFN20 has no reset pin. Nothing in the stratum depends on which.
...
- **The prefetch buffer, manual against itself**: RM 3.4.2 demands one
  ...
  bit; the vendor's clock setup does not look for one.
...
- **The chip identifier of the CH32X033F8P6, library against itself**:
  ...
  `GPIO_IPD_Unused()` switches on 0x03117000 for the same part.
