...
2. **Any other GCLK_TSENS rate rescales the answer**, by exactly
   ...
   `samc21/freqm.hpp` `reference_hz` pattern - a ratio meter cannot know
...
2. **Write-synchronized** (43.6.7): CTRLA.SWRST and CTRLA.ENABLE, and
   ...
   the near side of the store and returns bool** (the `samc21/sdadc.hpp`
...
`factory()` reads them through `samc21/nvm.hpp`; `programmed()` is a weak
...
- **An overflowed VALUE WRAPS rather than saturating.** The register held
  ...
  gets a plausible number **of the wrong sign** - and `samc21/sdadc.hpp`'s
...
- **A `MeterSource` or sampler adapter.** `util/analog_sampler.hpp`'s
  ...
  `samc21/sdadc.hpp` left open, and it is not attempted here.
