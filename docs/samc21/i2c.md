...
`test/family_samc21/i2c.cpp` + six negatives (the per-bus timeout adds a
...
the avrdx original or its samc21 port, one `twi_link.hpp` wire format
...
- **AND THE FIX WAS MEASURED, NOT JUST NAMED - with its confound
  ...
  the samc21 peer's client serves the command channel at the very rate
...
- **`I2cm<n>` / `I2cs<n>`** - the two resources: full register
  ...
  instead of stretching for a byte nobody wants; born with the samc21
