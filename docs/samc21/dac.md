...
not `brio::Ref`, which `samc21/adc.hpp` owns for REFCTRL.REFSEL: on this
...
caller and `samc21/supc.hpp`'s `VrefLevel` for INTREF.
...
- **1.8.10 DAC Output Reference Selection** (every revision) is live on
  ...
  enforced in `samc21/sdadc.hpp`, not here.
...
- **`SYNCBUSY.DATABUF` IS NOT A BUS CROSSING, and the chapter does not
  ...
  the same shape, for the same reason, that `samc21/adc.hpp`'s `select()`
