...
`sdadc.md`, `tsens.md`, `ccl.md`, `pac.md`, `dsu.md`, `divas.md`,
`mtb.md`; PM lives in
...
pointed at by absolute path in `samc21/cmake/toolchain-arm.cmake`
...
`SAMC21_MCU` cache variable) which `sam.h` dispatches on - no
...
`samc21/svd/ATSAMC21J18A.svd` feeding the same mcu-debug Peripheral
Viewer the AVR side uses. CMake Tools' Active Folder must be `samc21/`
...
host/) to the host database, `brio/samc21/.clangd` and
`samc21/.clangd` send the samc21 stratum and project to
`build-cmake/samc21j-release`, `test/family_samc21/.clangd` lets the
