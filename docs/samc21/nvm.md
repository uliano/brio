...
`samc21/nvm.hpp` (`Nvm` + `FlashWaitStates` + the factory views) and
`samc21/nvm_flash.hpp` (`RwweePartition`, `RwweeFlash` the
...
is the smaller of the two, the opposite of the AVR family's usage.
...
section + 2*ADDR" (27.8.8). Twenty-two bits could hold either
convention, so the convention is measured rather than trusted: a
page-buffer load at 0x00400100 leaves ADDR = 0x00000080 and one at
0x00020100 leaves 0x00010080. The register is also updated by hardware on
every page-buffer load, which is what makes that measurement possible.
...
BEFORE a rise and after a fall. `samc21/clock.hpp` is the caller.
...
**`RwweePartition`** (`samc21/nvm_flash.hpp`) - where the line between the
...
Both bounds are constants rather than linker symbols, because nothing the
linker places can reach into the RWWEE array - that is the property the
RWWEE array buys, and the partition does not spend it. The split is
anchored at the TOP of the array so that each user's own home is in turn
anchored to the silicon: the heap's map pair at the top of the heap's
share, the journal's halves at the top of the part.
...
From `test_samc_nvm`, whose `z` runs every letter; letter `m` sits
outside it, because it costs main-array endurance and must be asked for
by name. Nothing to wire: everything under test is inside the chip.
...
- **`util/nv_heap.hpp` runs unmodified on this target.** It mounts on the
  ...
  concept's `erase_size`/`write_cell` split is what makes this fit: the
  two numbers are 256 and 64 here against 512 and 2 on the AVR DA/DB.
- **`util/nv_journal.hpp` runs unmodified on it too, in the attic beside
  the heap** (`test_samc_journal`; letters `p` and `v` sit outside `z`).
  The measurements:
  ...
  - **the panic reserve holds on silicon**: after every completed save,
    `save_reserved()` of a maximum-size value succeeds and **costs no
    erase**, measured against the real page program;
...
- **Moving the line between the two storage classes is a one-time break
  of on-chip data, and the mount says so rather than hiding it.** The
  heap's map home sits at the top of its own share, so moving that
  ceiling moves the map: a boot on the far side of such a move finds no
  map, reports an empty heap and leaves the old blocks unreadable - the
  survival-aware mount working as designed. A journal mounting over rows
  that still carry old map bytes recognizes none of them as entries (a
  different magic) and treats them as dirty cells to collect past.
...
- **An unbound vector on this target is a silent death**, not a crash:
  the crt's default handler is a spin loop, so a program that forgets
  `SERCOM5_Handler` simply never prints anything. (On the AVR DA/DB the
  shape is the opposite - an unbound vector jumps to 0 and the program
  reboots for ever.) Halting over SWD and reading IPSR names the
  interrupt that went unserved.
...
- **The SSB command** (Set Security Bit). One-way - only a debugger chip
  ...
  debugger. `security_bit()` reads the state; nothing sets it: a one-way
  hazard gets a reader and never a writer.
...
- **A main-array FlashMedia backend, which is a decision and not a gap.**
  The RWWEE array serves both storage classes - blocks in rows 0..27,
  small values in rows 28..31 - so nothing is waiting for room, and a
  main-array backend would import back the three things the RWWEE array
  deletes: the stall (the CPU completes ONE polling turn inside a
  main-array row erase against about 2500 inside an RWWEE one), a quarter
  of the endurance (25k cycles against 100k, 45-43 and 45-44), and
  linker-symbol zone arithmetic with the reflash preflight that goes with
  it. It is built when something genuinely needs more than 8 KB of
  nonvolatile storage, and not before.
...
- (`samc21/dsu.hpp` carries the CRC32 engine - chip erase deliberately
  absent there too, for the same reason - and `samc21/pac.hpp` the write
  protection; see [dsu.md](dsu.md) and [pac.md](pac.md). Nothing here
  uses either.)
...
- Power reduction (`enter_power_reduction`, `exit_power_reduction`,
  `sleep_power`): the block reports PRM, but nothing here exercises it in
