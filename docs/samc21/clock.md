...
lives in `samc21/nvm.hpp` ([nvm.md](nvm.md)) - but the QUESTION is this
...
Resources (thin typed register views, `samc21/clock.hpp`):
...
- The CFD's event output is enabled and disabled but never CONSUMED -
  `samc21/evsys.hpp` could carry it, and the generator code is
