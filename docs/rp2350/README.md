...
| [spi.md](spi.md) | The SPI: the PL022 this chip carries at the same revision as its predecessor, over a pin table with ONE function column where the UART gained a second - and the one sentence of 12.3.1 that inverts a verdict, the transmit pad's output enable now following the block's own nSSPOE |
