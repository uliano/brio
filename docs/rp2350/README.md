...
| [spi.md](spi.md) | The SPI: the PL022 this chip carries at the same revision as its predecessor, over a pin table with ONE function column where the UART gained a second - and the one sentence of 12.3.1 that the rest of the chapter contradicts five times over and the pads refute, the transmit pad's output enable reaching no pad here either |
