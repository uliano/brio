...
reset value, every reversal, all four polynomial widths and the four
catalogue check values are bit for bit what the other die gives. The
unit has no per-part table cell of any kind and nothing here is gated,
...
the CPU-fed number and the software reference's, as everywhere else.
