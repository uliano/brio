...
| [brio/ch32v00x/pin.hpp](../../brio/ch32v00x/pin.hpp) | `Pin<'D', 5>`, `Port<'D'>`: the one-bit MODE this family has, pulls through OUTDR, the port clock opened by every configuring verb; `PinRef` (the runtime pin a bus request carries) and `Pad` (the compile-time pad name the pin tables use) |
...
- USART2 and the alternate-function REMAPS (AFIO_PCFR1): USART1 on its
  ...
  port or a moved pad.
- TIM1/TIM2/TIM3, ADC, the OPA and the watchdogs: each is a chapter of
  the reference manual with no user yet, and each is born with its
  first user and its bench measurements, the way the other three
  strata's were.
