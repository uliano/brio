...
- USART2 and the alternate-function REMAPS (AFIO_PCFR1): USART1 on its
  ...
  port or a moved pad.
- TIM1/TIM2/TIM3, ADC, I2C, SPI, the OPA and the watchdogs: each is a
  chapter of the reference manual with no user yet, and each is born
  with its first user and its bench measurements, the way the other
  three strata's were.
