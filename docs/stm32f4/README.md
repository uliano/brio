...
| [dsi.md](dsi.md) | DSI host (STM32F4): the MIPI Display Serial Interface in front of the LTDC on the F469/F479 class alone - the wrapper's regulator and PLL, the two-lane D-PHY on dedicated pads, the host's video mode packing the LTDC's frame into pixel packets and the generic interface a panel is spoken to through; the PLL's ranges as two documents state them and the solver on their intersection, the unit interval in quarter nanoseconds, the frame converted into lane byte clocks with its rounding, the lane transition times from the data sheet's maxima, two of the three errata as code. THE RULE THE MANUAL DOES NOT STATE: in video mode the generic interface lives inside the stream - a read with the LTDC stopped never completes and is answered by the first frame after it starts. Measured against the 32F469IDISCOVERY's panel, whose controller is READ over the link (an NT35510 by its three ID bytes): every DCS register written and read back, the panel's own count of link errors at zero, 60 frames a second out of the SDRAM, the tearing effect pulses counted on the pin and by the wrapper alike, the pattern generator, ULPS, and what the panel reports at the first turnaround after a reset on either side |
