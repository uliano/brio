...
| [usb.hpp](../../brio/ch32v203/usb.hpp) | USBD: ST's device controller under WCH's names, realizing util/usb's UsbController; measured: the kernel console over a CDC ACM port on the board's own USB-C, enumerated, configured and carrying bytes with no overflow - and the core must not sleep while it does, which the bus-master count now enforces (below, and [sleep.md](sleep.md)) |
...
- **Every chapter but the seventeen named above.** CAN and the second
  USB block (USBFS, on PB6/PB7, a different peripheral): each arrives
