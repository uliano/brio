...
- **Every chapter but the eleven named above.** SPI, I2C, CAN, the RTC
  and the backup domain, PWR and the sleep modes, the flash engine,
  CRC, and the second USB block (USBFS, on PB6/PB7, a different
  peripheral): this target is at first light and each arrives with its
  own document and its own suite.
