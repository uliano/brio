...
| CH32V203 (`brio/ch32v203/`) | [ch32v203/README.md](ch32v203/README.md) - Toolchain (WCH's gcc 15 again, but the full register file and the ilp32 ABI), the WeAct core board and the part table, the two-wire debug port of this family and the reset verb that does NOT start the program, the console on the probe's own serial; then what the silicon taught the stratum at first light - the clock task parking on the HSI, the PLL divider that lives in another block, the USB pads that are still GPIO pads - and a gap list that is most of the reference manual |
