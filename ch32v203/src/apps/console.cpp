// console - the brio kernel console on the CH32V203: three active
// objects over USART1, and the proof that the kernel this repository is
// built around runs unchanged on a second RISC-V core.
//
...
// every other target compiles. Only the last lines are this target's:
// the clock type, the transport, and the two vector bindings.
//
...
// serial_port.hpp.
//
// The board is a WeAct CH32V203C8T6 core board (a blue board in the
// black-pill shape): an 8 MHz crystal, a 32.768 kHz one, a blue LED on
...
// The clock: the HSI whole into the PLL times eighteen - 144 MHz, the
// top of this part's range, and the ONE truth about HCLK every driver
// below derives from. It is also the only rate on the ladder that feeds
// the USB blocks their 48 MHz (the PLL divided by three).
using SysClock = brio::Clock<brio::ClockSource::pll, 144'000'000>;
constexpr SysClock clock;

namespace {

using Serial = brio::Uart<1, P>;   // USART1 on PA9/PA10, rings 64/64
...
void Console::cmd_err(const Cmd&, Serial s) {
    brio::print(s,
                ...
                " baud=", Serial::actual_baud(SysClock::pclk2_hz),
                brio::crlf);
}
