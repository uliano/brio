...
`brio/` has eight strata; the include prefix makes a file's
...
builds are five sibling CMake projects, one per toolchain, all peers
(the repo root is not a CMake project): `avrdx/`, `samc21/`,
`stm32g0/` and `ch32v00x/` each auto-discover one `main()` per `src/apps/<app>.cpp` at configure time
