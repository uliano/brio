...
// docs/samc21/platform.md), but what every OTHER peripheral does WHILE
...
/// OSC48M's own measured rate (docs/samc21/clock.md): 47.755 MHz, 5100
...
/// WO[0]: the 8-bit NPWM shape docs/samc21/ccl.md already proved reaches
...
/// all (37.5.3, measured in docs/samc21/ccl.md), which is why the walker
...
void tb_rtc() {
    ...
    // counter by a constant four ticks (docs/samc21/rtc.md), so "what is
...
void td_clocks() {
    ...
    bench.verdict("the DPLL locks to the crystal and its output counts", dpll_up &&
                      dpll_awake > 100u);
    // THE ANSWER docs/samc21/platform.md LEFT OPEN, and it is the opposite
    ...
    // docs/samc21/osc32kctrl.md measured to be worth 44 % of the rate.
    ...
    // BY CONSTRUCTION here - samc21/clock.hpp never sets DPLLCTRLA.ONDEMAND
...
void tj_leftovers() {
    ...
    // report; docs/samc21/supc.md put this board at about 5.1 V and the
...
int main() {
    ...
    print(serial, "  the RTC on OSCULP32K measures ", ulp_hz,
          ...
          " kHz (docs/samc21/clock.md). KERNEL TIME STOPS IN STANDBY", crlf);
    if (!rulers) {
