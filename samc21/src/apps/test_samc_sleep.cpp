...
// samc21/sleep.hpp (PM, ch. 19 - SLEEPCFG, STDBYCFG and the two errata
...
//
// Reference test of those (docs/design/power.md, docs/samc21/platform.md):
// keep it passing.
//
...
// OSC48M's own measured rate from docs/samc21/clock.md (47.755 MHz, 5100
...
/// docs/samc21/clock.md: 47.755 MHz, i.e. 5100 ppm below its nominal
...
/// found in samc21/tc.hpp's read_sync() - and the driver now pays the
...
void tc_instrument() {
    ...
    // (docs/samc21/osc32kctrl.md) and would make every count below a lie.
    ...
    // rule docs/samc21/osc32kctrl.md records), so the generator moves back
...
void td_standby() {
    ...
    // every standby entry in this stratum (samc21/ticker.hpp's
...
int main() {
    ...
    print(serial, "  rulers: CLK_MAIN stopwatch at ", fine_hz / 1000u,
          " kHz (OSC48M's measured rate, docs/samc21/clock.md), RTC on OSCULP32K "
